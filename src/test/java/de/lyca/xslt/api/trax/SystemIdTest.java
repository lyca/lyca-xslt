...
/**
 * Test behavior of various types of systemId forms. <b>Note:</b> This test is
 * directory-dependent, so if there are any fails, check the code to see what
 * the test file is expecting the path/directory/etc. to be. I'm also using
 * RFC1738 as the definition of what various systemId's should look like: the
 * validation of this test is a little tricky, since it's not necessarily clear
 * exactly which forms of file: always have to be supported by the application.
 * (Or which forms of file: are supported by the underlying parser) Also, we
 * really need to write some platform-dependent tests for this area, to account
 * for 'c:\foo' type paths in Windows, and for '/usr/bin' type paths in
 * UNIX-land, as well as various links and such (although that may be beyond the
 * scope of what Xalan and it's parser do, and more into testing the JDK
 * itself).
 * 
 * Note RFC1738 specifies that file: URL's always use forward slash / as a file
 * separator ABSOLUTE-PATHS file:///e:/path/file.txt absolute path to
 * e:/path/file.txt
 * 
 * file://localhost/e:/path/file.txt path to e:/path/file.txt on localhost, but
 * goes direct to filesystem
 * 
 * file://otherhost/e:/path/file.txt absolute path to otherhost, but RFC1738
 * doesn't actually specify the protocol to use to get to the other machine to
 * actually get the file
 * 
 * RELATIVE-PATHS file:e:/path/file.txt is theoretically a relative URL w/r/t
 * the current Base URL
 * 
 * file:/e:/path/file.txt is theoretically a relative URL that starts with
 * /e:/... - not likely to be useful
 * 
 * file://file.txt theoretically a hostname (not a filename) with no path or
 * file portion
 * 
 * ILLEGAL-PATHS file://e:\path\file.txt is illegal, since RFC1738 specifically
 * says / forward slashes
 * 
...
public class SystemIdTest {
  ...
  /**
   * Internal flag for test that should do a transform. Presumably using the
   * systemId item you just tested and one of our known-good test files.
   */
  protected static final String EXPECTED_RESULT_DOTRANSFORM = "EXPECTED_RESULT_DOTRANSFORM";

  /**
   * Initialize this test - Set names of xml/xsl test files, cache user.dir
   * property.
   * 
   * @param p
   *          Properties to initialize from (unused)
  ...
  // reporter.logHashtable(Logger.STATUSMSG, System.getProperties(),
  ...
  public void test01() throws Exception {
    final String name = PACKAGE + "SystemIdTest";
    Source source = new StreamSource(ResourceUtils.getSystemID(name + ".xsl"));
    TransformerFactory transformerFactory = TransformerFactory.newInstance();
    Templates templates = transformerFactory.newTemplates(source);

    String expected = ResourceUtils.readResource(name + ".out", UTF_8);

    Transformer templatesTransformer = templates.newTransformer();
    StringWriter templatesWriter = new StringWriter();
    templatesTransformer.transform(ResourceUtils.getSource(name + ".xml"), new StreamResult(templatesWriter));
    Assert.assertEquals(expected, templatesWriter.toString());

    Transformer factoryTransformer = transformerFactory.newTransformer(source);
    StringWriter factoryWriter = new StringWriter();
    factoryTransformer.transform(ResourceUtils.getSource(name + ".xml"), new StreamResult(factoryWriter));
    Assert.assertEquals(expected, factoryWriter.toString());
  }
  ...
  // String inputDirPath = PACKAGE + "SystemIdTest";
  ...
  // EXPECTED_RESULT_UNKNOWN, null, EXPECTED_RESULT_UNKNOWN, null },
  //
  // { inputDirPath,
  ...
  // { "file:///" + userDirPath,
  // "file:///, System(user.dir), /blah (works normally)[2a]", userDirExpected,
  // null, userDirExpected, null },
  //
  // { "file://localhost/" + userDirPath,
  // "file://localhost/, System(user.dir), /blah (works normally)[2b]",
  // userDirExpected, null, userDirExpected, null },
  //
  // { userDirPath, "Just System(user.dir), /blah[2c]", EXPECTED_RESULT_UNKNOWN,
  // null, EXPECTED_RESULT_UNKNOWN,
  // null },
  //
  ...
  // "/tests/./api/" + TRAX_SUBDIR
  // + "/" + knownGoodBaseName, "file:///, System(user.dir), /./blah (???)[2d]",
  // userDirExpected, null, userDirExpected, null },
  //
  ...
  // "Just /blah relative path[3b]", userDirExpected,
  // null, userDirExpected, null },
  //
  ...
  // EXPECTED_RESULT_UNKNOWN,
  // null, EXPECTED_RESULT_UNKNOWN, null },
  //
  // { "file:/" + inputDirPath,
  ...
  /**
   * Worker method to test factory.newTemplates. Performs validation of various
   * types based on the expected value.
   * 
   * @param sysId
   *          systemId of XSL to test with
   * @param desc
   *          description of test, used in check() calls
   * @param expected
   *          either one of the EXPECTED_RESULT_* flags defined in this file, or
   *          the start of a .toString of the exception that you expect will be
   *          thrown
   * @param innerExpected
   *          the start of a .toString of the inner or wrapper exception that
   *          you expect will be thrown, presumably wrapped inside a
   *          TransformerException; if null, then this is not checked for
  ...
  // reporter.logStatusMsg("newTemplates(" + desc + "): " + sysId + ", " +
  // expected);
  ...
  // reporter.logThrowable(Logger.ERRORMSG, t, "newTemplates(" + desc +
  // ") threw");
  // }
  // // Call worker method to perform actual validation
  // validateWithSystemId(sysId, desc, expected, innerExpected, thrown,
  // transformer);
  // return templates;
  // }

  /**
   * Worker method to test factory.newTransformer. Performs validation of
   * various types based on the expected value.
   * 
   * @param sysId
   *          systemId of XSL to test with
   * @param desc
   ...
   */
  // protected Transformer testNewTransformerWithSystemId(String sysId, String
  // desc, String expected, String innerExpected) {
  // TransformerFactory factory = null;
  // Transformer transformer = null;
  // Throwable thrown = null;
  // try {
  ...
  // // Use a StreamSource with the systemId, which sets itself automatically
  // Source source = new StreamSource(sysId);
  // reporter.logStatusMsg("newTransformer(" + desc + "): " + sysId + ", " +
  // expected);
  // transformer = factory.newTransformer(source);
  // reporter.logTraceMsg("newTransformer() no exceptions!");
  // } catch (Throwable t) {
  // thrown = t;
  // reporter.logStatusMsg("newTransformer(" + desc + ") threw:" +
  // t.toString());
  // reporter.logThrowable(Logger.ERRORMSG, t, "newTransformer(" + desc +
  // ") threw");
  // }
  // // Call worker method to perform actual validation
  // validateWithSystemId(sysId, desc, expected, innerExpected, thrown,
  ...
  /**
   * Worker method to test transformer.transform(). Performs validation of
   * various types based on the expected value, using a known-good XSL file.
   * 
   * @param sysId
   *          systemId of XML file to test with
   * @param desc
   *          description of test, used in check() calls
   * @param expected
   *          either one of the EXPECTED_RESULT_* flags defined in this file, or
   ...
   *          TransformerException; if null, then this is not checked for
   */
  // protected void testTransformWithSystemId(String sysId, String desc, String
  // expected, String innerExpected) {
  // TransformerFactory factory = null;
  // Transformer transformer = null;
  // Throwable thrown = null;
  // try {
  // factory = TransformerFactory.newInstance();
  // // Use a StreamSource with the known-good systemId, which sets itself
  // // automatically
  // Source source = new StreamSource(knownGoodFileInfo.inputName);
  // reporter.logStatusMsg("Transform(" + desc + "): " + sysId + ", " +
  // expected);
  // transformer = factory.newTransformer(source);
  //
  // // Always try the transform using a new StreamSource
  // reporter.logTraceMsg("About to transform(StreamSource(" + sysId + ", " +
  // outNames.nextName() + ")");
  // transformer.transform(new StreamSource(sysId), new
  // StreamResult(outNames.currentName()));
  // } catch (Throwable t) {
  // thrown = t;
  // reporter.logStatusMsg("Transform(" + desc + ") threw:" + t.toString());
  // reporter.logThrowable(Logger.ERRORMSG, t, "Transform(" + desc + ") threw");
  // }
  //
  ...
  // int result = fileChecker.check(reporter, new File(outNames.currentName()),
  // new File(knownGoodFileInfo.goldName),
  // "(" + desc + ") transform into: " + outNames.currentName());
  // if (result == Logger.FAIL_RESULT)
  // reporter.logInfoMsg("(" + desc + ") transform failure reason:" +
   ...
   * 
   * @param sysId
   *          systemId that you were testing with; used in logging
   * @param desc
   *          description of test, used in check() calls
   * @param expected
   *          either one of the EXPECTED_RESULT_* flags defined in this file, or
   *          the start of a .toString of the exception that you expect will be
   *          thrown, only used when expected=EXPECTED_RESULT_DOTRANSFORM
   * @param innerExpected
   *          the start of a .toString of the inner or wrapper exception that
   *          you expect will be thrown, presumably wrapped inside a
   *          TransformerException; if null, then this is not checked for
   * @param thrown
   *          any Throwable that was thrown in your operation
   * @param transformer
   *          that was created from your operation; only used for transform when
   *          expected=EXPECTED_RESULT_DOTRANSFORM
   */
  // protected void validateWithSystemId(String sysId, String desc, String
  // expected, String innerExpected,
  // Throwable thrown, Transformer transformer) {
  // // Sorry for the icky if..else.. statement
  // if (EXPECTED_RESULT_UNKNOWN.equals(expected)) {
  // // Just log a message: no validation done
  // reporter.logWarningMsg("(" + desc + ") not validated!");
  ...
  // StreamSource(QetestUtils.filenameToURL(knownGoodFileInfo.xmlName)),
  // new StreamResult(outNames.nextName()));
  // int result = fileChecker.check(reporter, new File(outNames.currentName()),
  // new File(
  // knownGoodFileInfo.goldName), "(" + desc + ") transform into: " +
  ...
  // reporter.logThrowable(Logger.ERRORMSG, t, "(" + desc +
  // ") do transform threw");
  // }
  // } else {
  // // Otherwise, assume it's a string that any exception
   ...
   * 
   * @param sysId
   *          systemId that you were testing with; currently unused?
   * @param desc
   *          description of test, used in check() calls
   * @param expected
   *          the start of a .toString of the exception that you expect will be
   ...
   *          any Throwable that was thrown in your operation
   */
  // protected void validateException(String sysId, String desc, String
  // expected, String innerExpected, Throwable thrown) {
  // if (thrown == null) {
  // reporter.checkFail("(" + desc + ") No exception was thrown when expected");
  // } else {
  // reporter.check(thrown.toString().startsWith(expected), true, "(" + desc +
  // ") expected exception");
  // if ((innerExpected != null) && (thrown instanceof TransformerException)) {
  // // Also validate any innerExceptions
  // Throwable inner = ((TransformerException) thrown).getException();
  // if (inner != null) {
  // reporter.check(inner.toString().startsWith(innerExpected), true, "(" + desc
  // + ") inner expected exception");
  // } else {
  // // User specified an innerException but none
  // // was found, so fail
  // reporter.checkFail("(" + desc + ") No innerException found like: " +
  // innerExpected);
  // }
  // }
  // }
