...
<xsl:output method="xml" indent="yes"/>
  ...
  <xsl:template match="date">
    ...
    <xsl:variable name="locale" select="java:java.util.Locale.new('US')"/>
    <xsl:variable name="formatter" select="java:java.text.SimpleDateFormat.new($format,$locale)"/>
    <xsl:variable name="date" select="java:de.lyca.xslt.extensions.javaSample3.getDate($year,$month,$day)"/>
