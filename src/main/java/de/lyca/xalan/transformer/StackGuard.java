...
public class StackGuard {
  ...
  /**
   * Used for infinite loop check. If the value is -1, do not check for infinite
   * loops. Anyone who wants to enable that check should change the value of
   * this variable to be the level of recursion that they want to check. Be
   * careful setting this variable, if the number is too low, it may report an
   * infinite loop situation, when there is none. Post version 1.0.0, we'll make
   * this a runtime feature.
   */
  ...
  public int countLikeTemplates(ElemTemplate templ, int pos) {
    final ObjectStack elems = m_transformer.getCurrentTemplateElements();
    int count = 1;
  ...
  public void checkForInfinateLoop() throws TransformerException {
    final int nTemplates = m_transformer.getCurrentTemplateElementsCount();
    ...
    for (int i = nTemplates - 1; i >= m_recursionLimit; i--) {
      final ElemTemplate template = getNextMatchOrNamedTemplate(i);

      if (null == template) {
        break;
      }

      final int loopCount = countLikeTemplates(template, i);

      if (loopCount >= m_recursionLimit) {
        // throw new
        // TransformerException("Template nesting too deep. nesting = "+loopCount+
        ...
        final Object[] msgArgs = new Object[] { new Integer(loopCount), idIs,
                null != template.getName() ? template.getName().toString() : template.getMatch().getPatternString() };
