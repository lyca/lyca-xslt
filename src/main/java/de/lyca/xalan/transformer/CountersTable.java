...
/**
 * This is a table of counters, keyed by ElemNumber objects, each of which has a
 * list of Counter objects. This really isn't a true table, it is more like a
 * list of lists (there must be a technical term for that...).
 * 
 * @xsl.usage internal
 */
public class CountersTable extends Hashtable<ElemNumber, List<Counter>> {
  ...
  /**
   * Add a list of counted nodes that were built in backwards document order, or
   * a list of counted nodes that are in forwards document order.
   * 
   * @param flist
   *          Vector of nodes built in forwards document order
   * @param blist
   *          Vector of nodes built in backwards document order
   */
  ...
  /**
   * Count forward until the given node is found, or until we have looked to the
   * given amount.
   * 
   * @param support
