...
public class NodeSorter {
  ...
  /**
   * Construct a NodeSorter, passing in the XSL TransformerFactory so it can
   * know how to get the node data according to the proper whitespace rules.
   * 
   * @param p
  ...
  /**
   * Return the results of a compare of two nodes. TODO: Optimize compare --
   * cache the getStringExpr results, key by m_selectPat + hash of node.
   * 
   * @param n1
   *          First node to use in compare
   * @param n2
   *          Second node to use in compare
   * @param kIndex
   *          Index of NodeSortKey to use for sort
   * @param support
   *          XPath context to use
   * 
  ...
  int compare(NodeCompareElem n1, NodeCompareElem n2, int kIndex, XPathContext support) throws TransformerException {
    ...
    if (k.m_treatAsNumbers) {
      ...
      /*
       * Leave this in case we decide to use an array later if (kIndex < maxkey)
       * { double n1Num = (double)n1.m_keyValue[kIndex]; double n2Num =
       * (double)n2.m_keyValue[kIndex]; }
       */
      else {

        // Get values dynamically
        final XObject r1 = k.m_selectPat.execute(m_execContext, n1.m_node, k.m_namespaceContext);
        final XObject r2 = k.m_selectPat.execute(m_execContext, n2.m_node, k.m_namespaceContext);
        n1Num = r1.num();
      ...
      /*
       * Leave this in case we decide to use an array later if (kIndex < maxkey)
       * { String n1String = (String)n1.m_keyValue[kIndex]; String n2String =
       * (String)n2.m_keyValue[kIndex]; }
       */
      else {
    ...
    if (0 == result) {
      ...
      // {
      final DTM dtm = support.getDTM(n1.m_node); // %OPT%
  ...
  /**
   * This implements a standard Mergesort, as described in Robert Sedgewick's
   * Algorithms book. This is a better sort for our purpose than the Quicksort
   * because it maintains the original document order of the input if the order
   * isn't changed by the sort.
   * 
   * @param a
   *          First vector of nodes to compare
   * @param b
   ...
   * java.io.IOException { int lo = lo0; int hi = hi0;
   * 
   * if ( hi0 > lo0) { // Arbitrarily establishing partition element as the
   * midpoint of // the array. Node midNode = (Node)v.elementAt( ( lo0 + hi0 ) /
   * 2 );
   * 
   * // loop through the array until indices cross while( lo <= hi ) { // find
   * the first element that is greater than or equal to // the partition element
   * starting from the left Index. while( (lo < hi0) &&
   * (compare((Node)v.elementAt(lo), midNode, 0, support) < 0) ) { ++lo; } //
   * end while
   * 
   * // find an element that is smaller than or equal to // the partition
   * element starting from the right Index. while( (hi > lo0) &&
   * (compare((Node)v.elementAt(hi), midNode, 0, support) > 0) ) { --hi; }
   * 
   * // if the indexes have not crossed, swap if( lo <= hi ) { swap(v, lo, hi);
   * ++lo; --hi; } }
   * 
   * // If the right index has not reached the left side of array // must now
   * sort the left partition. if( lo0 < hi ) { QuickSort2( v, lo0, hi, support
   * ); }
   * 
   * // If the left index has not reached the right side of array // must now
   * sort the right partition. if( lo < hi0 ) { QuickSort2( v, lo, hi0, support
   * ); } } } // end QuickSort2
   */
  ...
  /**
   * This class holds the value(s) from executing the given node against the
   * sort key(s).
   * 
   * @xsl.usage internal
   */
  class NodeCompareElem {
     ...
     * maxkey + 1 keys will only hit fairly rarely and therefore, we // will get
     * the node values for those keys dynamically.
     */
    int maxkey = 2;
    ...
    NodeCompareElem(int node) throws TransformerException {
      ...
      if (!m_keys.isEmpty()) {
        ...
        /*
         * Leave this in case we decide to use an array later while (kIndex <=
         * m_keys.size() && kIndex < maxkey) { NodeSortKey k =
         * (NodeSortKey)m_keys.elementAt(kIndex); XObject r =
         * k.m_selectPat.execute(m_execContext, node, k.m_namespaceContext);
         * if(k.m_treatAsNumbers) m_KeyValue[kIndex] = r.num(); else
