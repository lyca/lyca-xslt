...
public class MsgMgr {
  ...
  public void message(SourceLocator srcLctr, String msg, boolean terminate) throws TransformerException {
    ...
    final ErrorListener errHandler = m_transformer.getErrorListener();
   ...
   * XPathContext decide the error condition is severe enough to halt
   * processing.
   * 
   * @throws TransformerException
   * 
   ...
   * 
   * if (null != errHandler) errHandler.fatalError(new TransformerException(msg,
   * srcLctr)); else throw new TransformerException(msg, srcLctr); }
   * 
   * @xsl.usage internal
   */
