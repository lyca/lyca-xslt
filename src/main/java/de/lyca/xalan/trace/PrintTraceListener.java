...
/**
 * Implementation of the TraceListener interface that prints each event to
 * standard out as it occurs.
 * 
 * @see de.lyca.xalan.trace.TracerEvent
 * @xsl.usage advanced
 */
public class PrintTraceListener implements TraceListenerEx3 {
  ...
  /**
   * This needs to be set to true if the listener is to print an event whenever
   * a template is invoked.
   */
  public boolean m_traceTemplates = false;

  /**
   * Set to true if the listener is to print events that occur as each node is
   * 'executed' in the stylesheet.
   */
  public boolean m_traceElements = false;

  /**
   * Set to true if the listener is to print information after each result-tree
   * generation event.
   */
  ...
  /**
   * Set to true if the listener is to print information after each extension
   * event.
   */
  ...
  public void _trace(TracerEvent ev) {
    ...
    switch (ev.m_styleNode.getXSLToken()) {
      case Constants.ELEMNAME_TEXTLITERALRESULT:
        if (m_traceElements) {
          m_pw.print(ev.m_styleNode.getSystemId() + " Line #" + ev.m_styleNode.getLineNumber() + ", " + "Column #"
                  + ev.m_styleNode.getColumnNumber() + " -- " + ev.m_styleNode.getNodeName() + ": ");
      ...
      case Constants.ELEMNAME_TEMPLATE:
        if (m_traceTemplates || m_traceElements) {
          ...
          m_pw.print(et.getSystemId() + " Line #" + et.getLineNumber() + ", " + "Column #" + et.getColumnNumber()
                  + ": " + et.getNodeName() + " ");
      ...
      default:
        if (m_traceElements) {
          m_pw.println(ev.m_styleNode.getSystemId() + " Line #" + ev.m_styleNode.getLineNumber() + ", " + "Column #"
                  + ev.m_styleNode.getColumnNumber() + ": " + ev.m_styleNode.getNodeName());
        }
    }
  }
  ...
  public void selected(SelectionEvent ev) throws javax.xml.transform.TransformerException {
    ...
    if (m_traceSelection) {
      ...
      if (ev.m_styleNode.getLineNumber() == 0) {
        ...
        m_pw.print(ev.m_styleNode.getSystemId() + " Line #" + ev.m_styleNode.getLineNumber() + ", " + "Column #"
                + ev.m_styleNode.getColumnNumber() + ": " + ete.getNodeName() + ", " + ev.m_attributeName + "='"
      ...
      if (ev.m_selection.getType() == XObject.CLASS_NODESET) {
        ...
        final de.lyca.xml.dtm.DTMIterator nl = ev.m_selection.iter();
        ...
        // with the transformer. The fix is that we record the initial state
        // before looping, then
        // restore the state when we finish it, which is done in the following
        // lines added.
        int currentPos = DTM.NULL;
        currentPos = nl.getCurrentPos();
        nl.setShouldCacheNodes(true); // This MUST be done before we clone the
                                      // iterator!
        de.lyca.xml.dtm.DTMIterator clone = null;
        // End of block
