...
public class ExsltSets extends ExsltBase {
   ...
   * set passed as the second argument. If the first node in the second node set
   * is not contained in the first node set, then an empty node set is returned.
   ...
   *         order the first node in nl2; an empty node-set if the first node in
   *         nl2 is not in nl1; all of nl1 if nl2 is empty.
   * 
   * @see <a href="http://www.exslt.org/">EXSLT</a>
   */
   ...
   * node-set NS passed as the first argument. Specifically, it selects a node N
   * if there is no node in NS that has the same string value as N, and that
