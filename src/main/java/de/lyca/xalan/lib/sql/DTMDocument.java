...
public class DTMDocument extends DTMDefaultBaseIterators {
   ...
   * arrays are not meeant to hold indexes to attribute information but as long
   * as there is not direct connection back into the main DTM tree we should be
   * OK.
   */
  protected SuballocatedIntVector m_attribute;

  /**
   * The Document Index will most likely be 0, but we will reference it by
   * variable in case that paradigm falls through.
   */
  ...
  /**
   * A common routine that allocates an Object from the Object Array. One of the
   * common bugs in this code was to allocate an Object and not incerment
   * m_size, using this method will assure that function.
   * 
   * @param o
   * 
   */
  ...
  protected int addElementWithData(Object o, int level, int extendedType, int parent, int prevsib) {
    final int elementIdx = addElement(level, extendedType, parent, prevsib);

    final int data = allocateNodeObject(o);
  ...
  /**
   * Link an attribute to a node, if the node already has one or more attributes
   * assigned, then just link this one to the attribute list. The first
   * attribute is attached to the Parent Node (pnode) through the m_attribute
   * array, subsequent attributes are linked through the m_prevsib, m_nextsib
   * arrays.
   * 
   * @param o
   * @param extendedType
   ...
   * problems because the parent of any attribute will be the original node they
   * were assigned to. Need to see how the attribute walker works, then we
   * should be able to fake it out.
   * 
   * @param toNode
   * @param fromNode
   * 
  ...
  public int getFirstAttribute(int parm1) {
    ...
    final int nodeIdx = makeNodeIdentity(parm1);
    if (nodeIdx != DTM.NULL) {
      final int attribIdx = m_attribute.elementAt(nodeIdx);
  ...
  public String getNodeValue(int parm1) {
    ...
    try {
      final Object o = m_ObjectArray.getAt(makeNodeIdentity(parm1));
  ...
  public XMLString getStringValue(int nodeHandle) {
    final int nodeIdx = makeNodeIdentity(nodeHandle);
    if (DEBUG) {
      System.out.println("getStringValue(" + nodeIdx + ")");
    }

    final Object o = m_ObjectArray.getAt(nodeIdx);
    if (o == S_ELEMENT_NODE) {
   ...
   * 
   * @param nodeIdx
   *          Index of node whose subtree is to be walked, gathering the
  ...
  protected void getNodeData(int nodeIdx, FastStringBuffer buf) {
    for (int child = _firstch(nodeIdx); child != DTM.NULL; child = _nextsib(child)) {
      final Object o = m_ObjectArray.getAt(child);
      if (o == S_ELEMENT_NODE) {
  ...
  /**
   * The Expanded Name table holds all of our Node names. The Base class will
   * add the common element types, need to call this function from the derived
   * class.
   * 
   */
  ...
  public void dumpDTM() {
    try {
      ...
      for (int i = 0; i < nRecords; i++) {
        ...
        final int exTypeID = getExpandedTypeID(makeNodeHandle(i));

        ps.println("Expanded Type ID: " + Integer.toHexString(exTypeID));

        final int type = getNodeType(makeNodeHandle(i));
        String typestring;
        ...
        final int firstChild = _firstch(i);
        ...
        final int prevSibling = _prevsib(i);
        ...
        final int nextSibling = _nextsib(i);
  ...
  public String getLocalName(int parm1) {
    ...
    final int exID = getExpandedTypeID(parm1);
  ...
  public short getNodeType(int parm1) {
    if (DEBUG) {
      ...
      final int exID = getExpandedTypeID(parm1);
      final String name = getLocalNameFromExpandedNameID(exID);
