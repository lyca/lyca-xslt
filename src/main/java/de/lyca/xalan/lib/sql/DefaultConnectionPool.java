...
public class DefaultConnectionPool implements ConnectionPool {
  ...
  /**
   * The mimimum size of the connection pool, if the number of available
   * connections falls below this mark, min connections will be allocated. The
   * Connection Pool will always be somewhere between MinSize and MinSize*2
   */
  private int m_PoolMinSize = 1;

  /**
   * Always implement the properties mechinism, if the Password or Username is
   * set seperatly then we will add them to the property manually.
   */
  private final Properties m_ConnectionProtocol = new Properties();
  ...
  /**
   * Go through the connection pool and release any connections that are not
   * InUse;
   * 
   */
  @Override
  ...
  /**
   * The Protocol string is used to pass in other connection properties. A
   * properties file is a general purpose container
   * 
   * @param p
   * 
   */
   ...
   * setting will only have effect on a new pool or when a new connection is
   * requested and there is less connections that this setting.
   * 
   * @param n
   * 
   */
  ...
  /**
   * Try to aquire a new connection, if it succeeds then return true, else
   * return false. Note: This method will cause the connection pool to be built.
   * 
   */
  @Override
  public boolean testConnection() {
    try {
      ...
      if (DEBUG) {
        final DatabaseMetaData dma = conn.getMetaData();
  ...
  public synchronized void initializePool() throws IllegalArgumentException, SQLException {
    ...
    if (m_driver == null)
      throw new IllegalArgumentException(
              XSLMessages.createMessage(XSLTErrorResources.ER_NO_DRIVER_NAME_SPECIFIED, null));
    ...
    do {
      ...
      if (con != null) {
        ...
        if (DEBUG) {
          System.out.println("Adding DB Connection to the Pool");
        }
      }
