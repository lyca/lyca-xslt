...
public class SQLQueryParser {
  /**
   * If the parser used inline parser to pull out variables then this will be
   * true. The default is not to use the Inline Parser.
   */
   ...
   * XConnection. Since we are only caching the Query and its parsed results we
   * may be able to use this as a real SINGLETON. It all depends on how
  ...
  /**
   * On a per Xconnection basis, we will create a SQLStatemenetParser, from this
   * parser, individual parsers will be created. The Init method is defined to
   * initialize all the internal structures that maintains the pool of parsers.
   */
  private void init() {
    // Do nothing for now.
  }
   ...
   * String.
   * 
   * If the Inline Parser is not enabled in the Options, no action will be taken
   * on the parser. This option can be set by the Stylesheet. If the option is
   * not set or cleared, a default value will be set determined by the way
   * variables were passed into the system.
   */
  ...
  /**
   * If the Inline Parser is used, the parser will note if this stastement is a
   * plain SQL Statement or a Called Procedure. Called Procudures generally have
   * output parameters and require special handling.
   * 
   * Called Procudures that are not processed with the Inline Parser will still
   * be executed but under the context of a PreparedStatement and not a
   * CallableStatement. Called Procudures that have output parameters MUST be
   * handled with the Inline Parser.
   */
  ...
  /**
   * Return a copy of the parsed SQL query that will be set to the Database
   * system to execute. If the inline parser was not used, then the original
   ...
   * variables used to populate a statement. The data use to popoulate a can
   * also be provided. If the data is provided, it will overide the populastion
   * using XSL variables. When the Inline PArser is not used, then the Data will
   * always be provided.
   * 
   */
    ...
    for (int indx = 0; indx < m_Parameters.size(); indx++) {
      ...
      try {
        ...
        if (m_InlineVariables) {
          ...
          if (value != null) {
            stmt.setObject(indx + 1, value.object(), parm.getType(), 4); // Currently
                                                                         // defaulting
                                                                         // scale
                                                                         // to 4
                                                                         // -
          ...
          final String value = parm.getValue();

          if (value != null) {
            stmt.setObject(indx + 1, value, parm.getType(), 4); // Currently
                                                                // defaulting
                                                                // scale to 4 -
                                                                // should read
                                                                // this!
  ...
  protected void inlineParser() {
    ...
    if (m_ParsedQuery == null) {
      m_ParsedQuery = new StringBuffer();
    }

    for (int idx = 0; idx < m_OrigQuery.length(); idx++) {
      final char ch = m_OrigQuery.charAt(idx);
      switch (state) {
