...
public class SQLErrorDocument extends DTMDocument {
  ...
  public SQLErrorDocument(DTMManager mgr, int ident, SQLException error) {
    ...
    final int sqlError = addElement(2, m_SQLError_TypeID, m_extErrorID, m_MainMessageID);
  ...
  public SQLErrorDocument(DTMManager mgr, int ident, Exception error, SQLWarning warning, boolean full) {
    ...
    while (se != null) {
      final int sqlError = addElement(2, inWarnings ? m_SQLWarning_TypeID : m_SQLError_TypeID, m_extErrorID, prev);
      prev = sqlError;
      int element = DTM.NULL;
  ...
  private void buildBasicStructure(Exception e) {
    ...
    m_MainMessageID = addElementWithData(e != null ? e.getLocalizedMessage() : "SQLWarning", 2, m_Message_TypeID,
            m_extErrorID, DTM.NULL);
  }
