...
import de.lyca.xml.dtm.ref.DTMNodeIterator;
import de.lyca.xml.dtm.ref.DTMNodeProxy;
import de.lyca.xpath.XPathContext;
import de.lyca.xpath.objects.XBooleanStatic;
 ...
 * </pre>
 * 
 * From the stylesheet perspective, XConnection provides 3 extension functions:
 * new(), query(), and close(). Use new() to call one of XConnection
 * constructors, which establishes a JDBC driver connection to a data source and
 * returns an XConnection object. Then use the XConnection object query() method
 * to return a result set in the form of a row-set element. When you have
 * finished working with the row-set, call the XConnection object close() method
...
public class XConnection {
  ...
  /**
   * The Current Connection Pool in Use. An XConnection can only represent one
   * query at a time, prior to doing some type of query.
   */
  private ConnectionPool m_ConnectionPool = null;

  /**
   * The DBMS Connection used to produce this SQL Document. Will be used to
   * clear free up the database resources on close.
   */
  private Connection m_Connection = null;

  /**
   * If a default Connection Pool is used. i.e. A connection Pool that is
   * created internally, then do we actually allow pools to be created. Due to
   * the archititure of the Xalan Extensions, there is no notification of when
   * the Extension is being unloaded and as such, there is a good chance that
   * JDBC COnnections are not closed. A finalized is provided to try and catch
   * this situation but since support of finalizers is inconsistant across JVM's
   * this may cause a problem. The robustness of the JDBC Driver is also at
   * issue here. if a controlled shutdown is provided by the driver then default
   * conntectiom pools are OK.
   */
  private boolean m_DefaultPoolingEnabled = false;

  /**
   * As we do queries, we will produce SQL Documents. Any ony may produce one or
   * more SQL Documents so that the current connection information may be
   * easilly reused. This collection will hold a collection of all the documents
   ...
   * the static pool alive We will also use this Pool Manager to register our
   * default pools.
   */
  private final ConnectionPoolManager m_PoolMgr = new ConnectionPoolManager();
   ...
   * will be stored in a seperate Error Document that can easily be retrived
   * using the getError() method. %REVIEW% This functionality will probably be
   * buried inside the SQLDocument.
   */
  private Exception m_Error = null;

  /**
   * When the Stylesheet wants to review the errors from a paticular document,
   * it asks the XConnection. We need to track what document in the list of
   * managed documents caused the last error. As SetError is called, it will
   * record the document that had the error.
   */
   ...
   * in getError(). This includes chained errors and warnings. If false (the
   * default) then getError() returns just the first SQLException.
   */
  private boolean m_FullErrors = false;

  /**
   * One a per XConnection basis there is a master QueryParser that is
   * responsible for generating Query Parsers. This will allow us to cache
   * previous instances so the inline parser execution time is minimized.
   */
  private final SQLQueryParser m_QueryParser = new SQLQueryParser();
  ...
  /**
   * This flag will be used to indicate to the SQLDocument to use Streaming
   * mode. Streeaming Mode will reduce the memory footprint to a fixed amount
   * but will not let you traverse the tree more than once since the Row data
   * will be reused for every Row in the Query.
   */
  ...
  /**
   * This flag will be used to indicate if multiple result sets are supported
   * from the database. If they are, then the metadata element is moved to
   * insude the row-set element and multiple row-set elements may be included
   * under the sql root element.
   */
  private boolean m_IsMultipleResultsEnabled = false;

  /**
   * This flag will be used to indicate if database preparedstatements should be
   * cached. This also controls if the Java statement object should be cached.
   */
   ...
   * method. <code>de.lyca.xalan.lib.sql.ConnectionPool</code> or a JNDI
   * datasource.
   * 
   * @param exprContext
   *          Context automatically passed from the XSLT sheet.
   * @param name
   ...
   * <code>de.lyca.xalan.lib.sql.ConnectionPool</code> or a JNDI datasource.
   * 
   * 
   * This method first tries to resolve the passed name against
   * <code>ConnectionPool</code>s registered with
   * <code>ConnectionPoolManager</code>. If that fails, it attempts to find the
   * name as a JNDI DataSource path.
   * 
   * @param exprContext
   *          Context automatically passed from the XSLT sheet.
   * @param name
  ...
  public XBooleanStatic connect(ExpressionContext exprContext, String name) {
    try {
      ...
      if (m_ConnectionPool == null) {
        ...
        final ConnectionPool pool = new JNDIConnectionPool(name);

        if (pool.testConnection()) {

          // JNDIConnectionPool seems good, so register it with the pool
          // manager.
   ...
   * @param exprContext
   * @param driver
   *          JDBC driver of the form foo.bar.Driver.
   * @param dbURL
   ...
   *          database URL of the form jdbc:subprotocol:subname.
   * @param protocolElem
   *          list of string tag/value connection arguments, normally including
   *          at least "user" and "password".
   * 
   */
   ...
   * The connection information could also be externally originated and passed
   * in as an XSL Parameter. The required XML Format is as follows. A document
   * fragment is needed to specify the connection information the top tag name
   * is not specific for this code, we are only interested in the tags inside.
   * <DBINFO-TAG> Specify the driver name for this connection pool
   * <dbdriver>drivername</dbdriver> Specify the URL for the driver in this
   * connection pool <dburl>url</dburl> Specify the password for this connection
   * pool <password>password</password> Specify the username for this connection
   * pool <user>username</user> You can add extra protocol items including the
   * User Name & Password with the protocol tag. For each extra protocol item,
   * add a new element where the name of the item is specified as the name
   * attribute and and its value as the elements value. <protocol
   * name="name of value">value</protocol> </DBINFO-TAG>
   * 
   * @param e
   * 
   * @throws SQLException
  ...
  private void initFromElement(Element e) throws SQLException {
    ...
    do {
      ...
      if (nName.equalsIgnoreCase("protocol")) {
        ...
        final Node n1 = attrs.getNamedItem("name");
        if (null != n1) {
          String s = "";
  ...
  /**
   * Initilize is being called because we did not have an existing Connection
   * Pool, so let's see if we created one already or lets create one ourselves.
   * 
   * @param driver
   * @param dbURL
   * @param prop
  ...
  private void init(String driver, String dbURL, Properties prop) throws SQLException {
    ...
    final ConnectionPool cpool = m_PoolMgr.getPool(poolName);
  ...
  /**
   * Allow the SQL Document to retrive a connection to be used to build the SQL
   * Statement.
   */
   ...
   * @throws SQLException
   * @link de.lyca.xalan.lib.sql.XStatement XStatement} object. The XStatement
   *       executes the query, and uses the result set to create a
   * @link de.lyca.xalan.lib.sql.RowSet RowSet}, a row-set element.
   */
  public DTM query(ExpressionContext exprContext, String queryString) {
    ...
    try {
      ...
      final SQLQueryParser query = m_QueryParser.parse(this, queryString, SQLQueryParser.NO_INLINE_PARSER);
   ...
   * @link de.lyca.xalan.lib.sql.XStatement XStatement} object. The XStatement
   *       executes the query, and uses the result set to create a
   * @link de.lyca.xalan.lib.sql.RowSet RowSet}, a row-set element. This method
   *       allows for the user to pass in a comma seperated String that
   *       represents a list of parameter types. If supplied the parameter types
   *       will be used to overload the current types in the current parameter
   *       list.
   */
  public DTM pquery(ExpressionContext exprContext, String queryString, String typeInfo) {
    ...
    try {
      ...
      final SQLQueryParser query = m_QueryParser.parse(this, queryString, SQLQueryParser.NO_OVERRIDE);
  ...
  /**
   * The purpose of this routine is to force the DB cursor to skip forward N
   * records. You should call this function after [p]query to help with
  ...
  /**
   * Add a section of parameters to the Parameter List Do each element from the
   * list
   * 
   * @param nl
   * 
  ...
  /**
   * There is a problem with some JDBC drivers when a Connection is open and the
   * JVM shutsdown. If there is a problem, there is no way to control the
   * currently open connections in the pool. So for the default connection pool,
   * the actuall pooling mechinsm is disabled by default. The Stylesheet
   * designer can re-enabled pooling to take advantage of connection pools. The
   * connection pool can even be disabled which will close all outstanding
   * connections.
   * 
  ...
  /**
   * Control how the SQL Document uses memory. In Streaming Mode, memory
   * consumption is greatly reduces so you can have queries of unlimited size
   * but it will not let you traverse the data more than once.
   * 
  ...
  /**
   * Provide access to the last error that occued. This error may be over
   * written when the next operation occurs.
   * 
   */
  public DTM getError() {
    if (m_FullErrors) {
  ...
  /**
   * Close the connection to the data source. Only close the connections for a
   * single document.
   * 
   * @throws SQLException
   */
  ...
  public void close(ExpressionContext exprContext, Object doc) throws SQLException {
    ...
    final SQLDocument sqlDoc = locateSQLDocument(exprContext, doc);
    if (sqlDoc != null) {
      // If we are using the Default Connection Pool, then
      // force the connection pool to flush unused connections.
  ...
  /**
   * When an SQL Document is returned as a DTM object, the XSL variable is
   * actually assigned as a DTMIterator. This is a helper function that will
   * allow you to get a reference to the original SQLDocument from the iterator.
   * 
   * Original code submitted by Moraine Didier
   * mailto://didier.moraine@winterthur.be
   * 
   * @param doc
   * @return
   */
  private SQLDocument locateSQLDocument(ExpressionContext exprContext, Object doc) {
    try {
       ...
       * = (OneStepIterator)xNS.getContainedIter(); DTMManager aDTMManager =
       * (DTMManager)iter.getDTMManager(); return
       * (SQLDocument)aDTMManager.getDTM(xNS.nextNode());
       */
  ...
  private SQLErrorDocument buildErrorDocument() {
    ...
    if (m_LastSQLDocumentWithError != null) {
      ...
      final ExpressionContext ctx = m_LastSQLDocumentWithError.getExpressionContext();
      final SQLWarning warn = m_LastSQLDocumentWithError.checkWarnings();
  ...
  /**
   * This is an internal version of Set Error that is called withen XConnection
   * where there is no SQLDocument created yet. As in the Connect statement or
   * creation of the ConnectionPool.
   */
   ...
   * 
   * @param feature
   *          The name of the feature being set, currently supports (streaming,
   *          inline-variables, multiple-results, cache-statements,
   *          default-pool-enabled).
   * @param setting
   *          The new setting for the specified feature, currently "true" is
  ...
  protected void finalize() {
    if (DEBUG) {
      System.out.println("In XConnection, finalize");
    }
    try {
      close();
    } catch (final Exception e) {
