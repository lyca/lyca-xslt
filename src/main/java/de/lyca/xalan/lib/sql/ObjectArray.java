 ...
 * the basic storage mechinism but the Arrays will be stored as blocks. The size
 * of the Array blocks is determine during object construction. This is intended
 * to be a simple storage mechinsim where the storage only can grow. Array
 * elements can not be removed, only added to.
 */
public class ObjectArray {
  ...
  /**
   * @param minArraySize
   *          The size of the Arrays stored in the Vector
   */
  ...
  public Object getAt(final int idx) {
    final int arrayIndx = idx / m_minArraySize;
    final int arrayOffset = idx - arrayIndx * m_minArraySize;
  ...
  public int append(Object o) {
    ...
    final int pos = m_Arrays.size() * m_minArraySize + m_nextSlot;
  ...
  public static void main(String[] args) {
    ...
    for (int x = 0; x < word.length; x++) {
      final String s = (String) m_ObjectArray.getAt(x);
