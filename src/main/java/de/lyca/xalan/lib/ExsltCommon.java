...
public class ExsltCommon {
   ...
   * 
   * Most XSLT object types can be coerced to each other without error. However,
   * there are certain coercions that raise errors, most importantly treating
   * anything other than a node set as a node set. Authors of utilities such as
   * named templates or user-defined extension functions may wish to give some
   * flexibility in the parameter and argument values that are accepted by the
   * utility; the exsl:object-type function enables them to do so.
   * 
   ...
   * 
   * You can also use this function to turn a string into a text node, which is
   * helpful if you want to pass a string to a function that only accepts a node
   * set.
   * 
