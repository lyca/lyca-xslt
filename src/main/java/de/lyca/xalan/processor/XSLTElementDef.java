 ...
 * and the mapping between Xalan classes and the markup elements in the XSLT
 * instance. This actually represents both text nodes and elements.
 */
public class XSLTElementDef {

  /**
   * Construct an instance of XSLTElementDef. This must be followed by a call to
   * build().
   */
  XSLTElementDef() {
  }
   ...
   * @param name
   *          The local name (without prefix), "*", or null.
   * @param nameAlias
  ...
  XSLTElementDef(XSLTSchema schema, String namespace, String name, String nameAlias, XSLTElementDef[] elements,
    ...
    if (null != namespace
            && (namespace.equals(de.lyca.xml.utils.Constants.S_XSLNAMESPACEURL)
                    || namespace.equals(de.lyca.xml.utils.Constants.S_BUILTIN_EXTENSIONS_URL) || namespace
   ...
   *          The class of the object that this element def should produce.
   * @param has_required
   *          true if this element has required elements by the XSLT
   *          specification.
   */
  XSLTElementDef(XSLTSchema schema, String namespace, String name, String nameAlias, XSLTElementDef[] elements,
   ...
   *          true if this element is required by the XSLT specification.
   * @param order
   *          the order this element should appear according to the XSLT
   *          specification.
   * @param multiAllowed
   ...
   *          true if this element is required by the XSLT specification.
   * @param has_order
   *          whether this element has ordered child elements
   * @param order
   *          the order this element should appear according to the XSLT
   *          specification.
   * @param multiAllowed
  ...
  void build(String namespace, String name, String nameAlias, XSLTElementDef[] elements, XSLTAttributeDef[] attributes,
    ...
    if (hasRequired() && m_elements != null) {
      final int n = m_elements.length;
      for (int i = 0; i < n; i++) {
        final XSLTElementDef def = m_elements[i];
  ...
  /**
   * Tell if two objects are equal, when either one may be null. If both are
   * null, they are considered equal.
   * 
   * @param obj1
   *          A reference to the first object, or null.
   * @param obj2
   *          A reference to the second object, or null.
   * 
   * @return true if the to objects are equal by both being null or because
   *         obj2.equals(obj1) returns true.
   */
  ...
  /**
   * Tell if the two string refs are equal, equality being defined as: 1) Both
   * strings are null. 2) One string is null and the other is empty. 3) Both
   * strings are non-null, and equal.
   * 
   * @param s1
   *          A reference to the first string, or null.
   * @param s2
   *          A reference to the second string, or null.
   * 
   * @return true if Both strings are null, or if one string is null and the
   *         other is empty, or if both strings are non-null, and equal because
   ...
   * 
   * @return true if the uri and local name arguments are considered to match
   *         the uri and local name of this element def.
   */
  private boolean QNameEquals(String uri, String localName) {

    return equalsMayBeNullOrZeroLen(m_namespace, uri)
            && (equalsMayBeNullOrZeroLen(m_name, localName) || equalsMayBeNullOrZeroLen(m_nameAlias, localName));
  }

  /**
   * Given a namespace URI, and a local name, get the processor for the element,
   * or return null if not allowed.
   * 
   * @param uri
  ...
  XSLTElementProcessor getProcessorFor(String uri, String localName) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (def.m_name.equals("*")) {
        ...
        if (!equalsMayBeNullOrZeroLen(uri, de.lyca.xml.utils.Constants.S_XSLNAMESPACEURL)) {
    ...
    if (elemDef != null && this.isOrdered()) {
      final int lastOrder = getLastOrder();
      if (order > lastOrder) {
        setLastOrder(order);
  ...
  /**
   * Given a namespace URI, and a local name, return the element's attribute
  ...
  XSLTAttributeDef getAttributeDef(String uri, String localName) {
    ...
    final XSLTAttributeDef[] attrDefs = getAttributes();
    final int nAttrDefs = attrDefs.length;

    for (int k = 0; k < nAttrDefs; k++) {
      final XSLTAttributeDef attrDef = attrDefs[k];
      final String uriDef = attrDef.getNamespace();
      final String nameDef = attrDef.getName();

      if (nameDef.equals("*")
              && (equalsMayBeNullOrZeroLen(uri, uriDef) || uriDef != null && uriDef.equals("*") && uri != null
                      && uri.length() > 0))
        return attrDef;
    ...
    if (null == defaultDef) {
      if (uri.length() > 0 && !equalsMayBeNullOrZeroLen(uri, de.lyca.xml.utils.Constants.S_XSLNAMESPACEURL))
  ...
  boolean isOrdered() {
    /*
     * if (!m_CheckedOrdered) { m_CheckedOrdered = true; m_isOrdered = false; if
     * (null == m_elements) return false;
     * 
     * int n = m_elements.length;
     * 
     * for (int i = 0; i < n; i++) { if (m_elements[i].getOrder() > 0) {
     * m_isOrdered = true; return true; } } return false; } else
     */
    return m_isOrdered;
  }
  ...
  /**
   * the highest order of child elements have appeared so far, or -1 if not
   * ordered
   */
