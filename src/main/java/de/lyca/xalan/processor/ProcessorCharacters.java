...
public class ProcessorCharacters extends XSLTElementProcessor {
  ...
  public void startNonText(StylesheetHandler handler) throws org.xml.sax.SAXException {
    ...
    final int nChars = m_accumulator.length();

    if (nChars > 0 && (null != m_xslTextElement || !XMLCharacterRecognizer.isWhiteSpace(m_accumulator))
      ...
      final boolean doe = null != m_xslTextElement ? m_xslTextElement.getDisableOutputEscaping() : false;
   ...
   * @see de.lyca.xalan.processor.StylesheetHandler#endElement
  ...
  /**
   * Set the current setXslTextElement. The xsl:text processor will call this to
   * set a preserve space state.
   * 
