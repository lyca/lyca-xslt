...
import de.lyca.xalan.transformer.TrAXFilter;
import de.lyca.xalan.transformer.TransformerIdentityImpl;
...
import de.lyca.xml.utils.TreeWalker;
...
public class TransformerFactoryImpl extends SAXTransformerFactory {
   ...
   */
  public static final String XSLT_PROPERTIES = "de/lyca/xalan/res/XSLTInfo.properties";
  ...
  public javax.xml.transform.Templates processFromNode(Node node) throws TransformerConfigurationException {
    ...
    try {
      final TemplatesHandler builder = newTemplatesHandler();
      final TreeWalker walker = new TreeWalker(builder, new de.lyca.xml.utils.DOM2Helper(), builder.getSystemId());
      ...
      if (m_errorListener != null) {
        ...
        throw new TransformerConfigurationException(XSLMessages.createMessage(
                XSLTErrorResources.ER_PROCESSFROMNODE_FAILED, null), se);
    ...
    /*
     * catch (TransformerException tce) { // Assume it's already been reported
     * to the error listener. throw new
     ...
     */
      ...
      if (m_errorListener != null) {
        ...
        // se.printStackTrace();
        throw new TransformerConfigurationException(XSLMessages.createMessage(
                XSLTErrorResources.ER_PROCESSFROMNODE_FAILED, null), e); // "processFromNode failed",
      // e);
    }
  }
   ...
   * 
   * @param node
   *          A DOM tree which must contain valid transform instructions that
   ...
   * that match the criteria, in which case they are applied as if they were a
   * list of imports or cascades.
   * 
   * <p>
  ...
  public Source getAssociatedStylesheet(Source source, String media, String title, String charset)
    ...
    try {
      if (null != node) {
        final TreeWalker walker = new TreeWalker(handler, new de.lyca.xml.utils.DOM2Helper(), baseID);
    ...
    } catch (final StopParseException spe) {
  ...
  public void setFeature(String name, boolean value) throws TransformerConfigurationException {
    ...
    // the secure processing feature.
      ...
      throw new TransformerConfigurationException(XSLMessages.createMessage(XSLTErrorResources.ER_UNSUPPORTED_FEATURE,
              new Object[] { name }));
  }
  ...
  public boolean getFeature(String name) {
    ...
    // will be faster.
    if (DOMResult.FEATURE == name || DOMSource.FEATURE == name || SAXResult.FEATURE == name
            || SAXSource.FEATURE == name || StreamResult.FEATURE == name || StreamSource.FEATURE == name
            || SAXTransformerFactory.FEATURE == name || SAXTransformerFactory.FEATURE_XMLFILTER == name)
      return true;
    else if (DOMResult.FEATURE.equals(name) || DOMSource.FEATURE.equals(name) || SAXResult.FEATURE.equals(name)
            || SAXSource.FEATURE.equals(name) || StreamResult.FEATURE.equals(name) || StreamSource.FEATURE.equals(name)
            || SAXTransformerFactory.FEATURE.equals(name) || SAXTransformerFactory.FEATURE_XMLFILTER.equals(name))
      return true;
    // secure processing?
   ...
   * available from the source -- in other words, if you pass in a DOM, there's
   * little we can do for you.
   * 
  ...
  /**
   * Flag set by FEATURE_INCREMENTAL. This feature specifies whether to produce
   * output incrementally, rather than waiting to finish parsing the input
   * before generating any output. By default this attribute is set to false.
   */
  ...
  public void setAttribute(String name, Object value) throws IllegalArgumentException {
    if (name.equals(FEATURE_INCREMENTAL)) {
      if (value instanceof Boolean) {
        ...
        throw new IllegalArgumentException(XSLMessages.createMessage(XSLTErrorResources.ER_BAD_VALUE, new Object[] {
                name, value })); // name + " bad value " + value);
    ...
    // yet.
      ...
      throw new IllegalArgumentException(XSLMessages.createMessage(XSLTErrorResources.ER_NOT_SUPPORTED,
  ...
  public Object getAttribute(String name) throws IllegalArgumentException {
    if (name.equals(FEATURE_INCREMENTAL))
      ...
      throw new IllegalArgumentException(XSLMessages.createMessage(XSLTErrorResources.ER_ATTRIB_VALUE_NOT_RECOGNIZED,
   ...
   * into a Result, based on the transformation instructions specified by the
   * argument.
   * 
   * @param src
   *          The source of the transformation instructions.
  ...
  /**
   * Get a TransformerHandler object that can process SAX ContentHandler events
   * into a Result, based on the Templates argument.
   * 
   * @param templates
   *          The source of the transformation instructions.
   * 
  ...
  public Transformer newTransformer(Source source) throws TransformerConfigurationException {
    try {
      final Templates tmpl = newTemplates(source);
      /*
       * this can happen if an ErrorListener is present and it doesn't throw any
       * exception in fatalError. The spec says: "a Transformer must use this
       * interface instead of throwing an exception" - the newTemplates() does
