...
import de.lyca.xml.utils.StringToIntTable;
import de.lyca.xml.utils.StringVector;
import de.lyca.xml.utils.XML11Char;
import de.lyca.xpath.XPath;

/**
 * This class defines an attribute for an element in a XSLT stylesheet, is meant
 * to reflect the structure defined in http://www.w3.org/TR/xslt#dtd, and the
...
public class XSLTAttributeDef {
   ...
   *          processing.
   * @param type
   *          One of T_CDATA, T_URL, T_AVT, T_PATTERN, T_EXPR, T_CHAR, T_NUMBER,
   *          T_YESNO, T_QNAME, T_QNAMES, T_ENUM, T_SIMPLEPATTERNLIST,
   *          T_NMTOKEN, T_STRINGLIST, T_PREFIX_URLLIST, T_ENUM_OR_PQNAME,
   *          T_NCNAME.
   * @param required
   *          true if this is attribute is required by the XSLT specification.
   * @param supportsAVT
   *          true if this attribute supports AVT's.
   * @param errorType
   *          the type of error to issue if validation fails. One of FATAL,
   *          ERROR, WARNING.
   */
   ...
   *          processing.
   * @param required
   *          true if this attribute is required by the XSLT specification.
   * @param supportsAVT
   *          true if this attribute supports AVT's.
   * @param prefixedQNameValAllowed
   *          If true, the type is T_ENUM_OR_PQNAME
   * @param errorType
   *          the type of error to issue if validation fails. One of FATAL,
   *          ERROR, WARNING.
   * @param k1
   ...
   * @param v1
   *          An integer representation of k1.
   * @param k2
   *          The XSLT name of the enumerated value.
   * @param v2
   *          An integer representation of k2.
   */
  XSLTAttributeDef(String namespace, String name, boolean required, boolean supportsAVT,
    ...
    m_namespace = namespace;
    m_name = name;
    m_type = prefixedQNameValAllowed ? T_ENUM_OR_PQNAME : T_ENUM;
    m_required = required;
    m_supportsAVT = supportsAVT;
    m_errorType = errorType;
  ...
  /**
   * Construct an instance of XSLTAttributeDef that uses three enumerated
   * values.
   * 
   * @param namespace
   *          The Namespace URI, or an empty string.
   ...
   *          The XSLT name of the enumerated value.
   * @param v3
   *          An integer representation of k3.
   */
  XSLTAttributeDef(String namespace, String name, boolean required, boolean supportsAVT,
   ...
   * @param errorType
   *          the type of error to issue if validation fails. One of FATAL,
   *          ERROR, WARNING. * @param k1 The XSLT name of the enumerated value.
   * @param v1
   *          An integer representation of k1.
   * @param k2
   *          The XSLT name of the enumerated value.
   ...
   *          The XSLT name of the enumerated value.
   * @param v4
   *          An integer representation of k4.
   */
  XSLTAttributeDef(String namespace, String name, boolean required, boolean supportsAVT,
          boolean prefixedQNameValAllowed, int errorType, String k1, int v1, String k2, int v2, String k3, int v3,
          String k4, int v4) {
  ...
  // <!-- Used for the type of an attribute value that is a URI reference.-->
          ...
          // Used for enumerated values, one of which could be a
          ...
          // Used for QName attributes that are always AVT. Prefix isn't
          // resolved.
          T_AVT_QNAME = 18,

          // Used for a list of QNames where non-prefixed items are to be
          // resolved
  ...
  static final XSLTAttributeDef m_foreignAttr = new XSLTAttributeDef("*", "*", XSLTAttributeDef.T_CDATA, false, false,
          WARNING);
   ...
   * 
   * @return The allowed namespace for this attribute, which may be null, or may
   *         be "*".
   */
  String getNamespace() {
    return m_namespace;
   ...
   * 
   * @return One of T_CDATA, T_URL, T_AVT, T_PATTERN, T_EXPR, T_CHAR, T_NUMBER,
   *         T_YESNO, T_QNAME, T_QNAMES, T_ENUM, T_SIMPLEPATTERNLIST, T_NMTOKEN,
   *         T_STRINGLIST, T_PREFIX_URLLIST, T_ENUM_OR_PQNAME.
   */
  int getType() {
    return m_type;
  }
  ...
  /**
   * If this element is of type T_ENUM, this will contain a map from the
   * attribute string to the Xalan integer value.
   */
  private StringToIntTable m_enums;

  /**
   * If this element is of type T_ENUM, this will return a map from the
   * attribute string to the Xalan integer value.
   * 
   * @param key
   ...
   *         attribute.
   * @throws Throws
   *           NullPointerException if m_enums is null.
   */
  ...
  /**
   * If this element is of type T_ENUM, this will return an array of strings -
   ...
   * 
   * @param def
   *          String representation of the default value for this attribute.
   */
   ...
   */
  private final boolean m_supportsAVT;
  ...
  /**
   * String that should represent the setter method which which may be used on
   * objects to set a value that represents this attribute
   */
  String m_setterString = null;

  /**
   * Return a string that should represent the setter method. The setter method
   * name will be created algorithmically the first time this method is
   * accessed, and then cached for return by subsequent invocations of this
   * method.
   * 
   * @return String that should represent the setter method which which may be
   *         used on objects to set a value that represents this attribute, of
   *         null if no setter method should be called.
   */
  public String getSetterMethodName() {
    ...
    if (null == m_setterString) {
      ...
      if (m_namespace != null && m_namespace.equals(de.lyca.xml.utils.Constants.S_XMLNAMESPACEURI)) {
        outBuf.append("Xml");
      }

      final int n = m_name.length();
   ...
   *           which will wrap a
   *           {@link javax.xml.transform.TransformerException}, if there is a
   *           syntax error in the attribute value template string.
   */
  AVT processAVT(StylesheetHandler handler, String uri, String name, String rawName, String value,
   ...
   * 
   * @throws org.xml.sax.SAXException
   *           if the string is not a length of 1.
   */
  Object processCHAR(StylesheetHandler handler, String uri, String name, String rawName, String value,
          ElemTemplateElement owner) throws org.xml.sax.SAXException {
    if (getSupportsAVT()) {
      try {
        final AVT avt = new AVT(handler, uri, name, rawName, value, owner);
   ...
   *          The qualified name (with prefix).
   * @param value
   *          non-null string that represents an enumerated value that is valid
   *          for this element.
   * @param owner
   * 
   * @return An Integer representation of the enumerated value if this attribute
   *         does not support AVT. Otherwise, and AVT is returned.
   */
  Object processENUM(StylesheetHandler handler, String uri, String name, String rawName, String value,
    ...
    if (retVal == StringToIntTable.INVALID_KEY) {
      final StringBuffer enumNamesList = getListOfEnums();
      handleError(handler, XSLTErrorResources.INVALID_ENUM, new Object[] { name, value, enumNamesList.toString() },
              null);
      return null;
    }
   ...
   * qname-but-not-ncname. Returns an AVT, if this attribute support AVT;
   * otherwise returns int or qname.
   * 
   * @param handler
   *          non-null reference to current StylesheetHandler that is
   *          constructing the Templates.
   ...
   *         enumerated value if attribute does not support AVT and an
   *         enumerated value was used. Otherwise a qname is returned.
   */
  Object processENUM_OR_PQNAME(StylesheetHandler handler, String uri, String name, String rawName, String value,
          ElemTemplateElement owner) throws org.xml.sax.SAXException {
   ...
   *           the expression string contains a syntax error.
   */
  Object processEXPR(StylesheetHandler handler, String uri, String name, String rawName, String value,
          ElemTemplateElement owner) throws org.xml.sax.SAXException {
   ...
   * 
   * @throws org.xml.sax.SAXException
   *           if the value is not a valid nmtoken
   */
  ...
  /**
   * Process an attribute string of type T_PATTERN into an XPath match pattern
   * value.
   * 
   * @param handler
   *          non-null reference to current StylesheetHandler that is
   ...
   * @throws org.xml.sax.SAXException
   *           that wraps a {@link javax.xml.transform.TransformerException} if
   *           the match pattern string contains a syntax error.
   */
  Object processPATTERN(StylesheetHandler handler, String uri, String name, String rawName, String value,
          ElemTemplateElement owner) throws org.xml.sax.SAXException {
   ...
   *          The qualified name (with prefix).
   * @param value
   *          A string that can be parsed into a double value.
   * @param number
   * 
   ...
   * @throws org.xml.sax.SAXException
   *           that wraps a {@link javax.xml.transform.TransformerException} if
   *           the string does not contain a parsable number.
   */
  Object processNUMBER(StylesheetHandler handler, String uri, String name, String rawName, String value,
          ElemTemplateElement owner) throws org.xml.sax.SAXException {
   ...
   *          The qualified name (with prefix).
   * @param value
   *          A string that represents a potentially prefix qualified name.
   * @param owner
   * 
   * @return A QName object if this attribute does not support AVT's. Otherwise,
   *         an AVT is returned.
   * 
   * @throws org.xml.sax.SAXException
   *           if the string contains a prefix that can not be resolved, or the
   *           string contains syntax that is invalid for a qualified name.
   */
  Object processQNAME(StylesheetHandler handler, String uri, String name, String rawName, String value,
          ElemTemplateElement owner) throws org.xml.sax.SAXException {
   ...
   *           string contains syntax that is invalid for a qualified name.
   */
  Object processAVT_QNAME(StylesheetHandler handler, String uri, String name, String rawName, String value,
          ElemTemplateElement owner) throws org.xml.sax.SAXException {
    ...
    try {
      ...
      // If an AVT wasn't used, validate the value
      if (avt.isSimple()) {
        final int indexOfNSSep = value.indexOf(':');
   ...
   *         Otherwise, an AVT is returned.
   * 
   * @throws org.xml.sax.SAXException
   *           if the string contains a prefix that can not be resolved, or the
   *           string contains syntax that is invalid for a NCName.
   */
  Object processNCNAME(StylesheetHandler handler, String uri, String name, String rawName, String value,
          ElemTemplateElement owner) throws org.xml.sax.SAXException {
   ...
   * 
   * @throws org.xml.sax.SAXException
   *           if the one of the qualified name strings contains a prefix that
  ...
  /**
   * Process an attribute string of type T_QNAMES_RESOLVE_NULL into a vector of
   * QNames where the specification requires non-prefixed elements to be placed
   * in the default namespace. (See section 16 of XSLT 1.0; the <em>only</em>
   * time that this will get called is for the
  ...
  /**
   * Process an attribute string of type T_SIMPLEPATTERNLIST into a vector of
  ...
  /**
   * Process an attribute string of type T_STRINGLIST into a vector of XPath
   * match patterns.
   * 
   * @param handler
   *          non-null reference to current StylesheetHandler that is
  ...
  /**
   * Process an attribute string of type T_URLLIST into a vector of prefixes
   * that may be resolved to URLs.
   * 
   * @param handler
   *          non-null reference to current StylesheetHandler that is
   *          constructing the Templates.
   ...
   * 
   * @throws org.xml.sax.SAXException
   *           if one of the prefixes can not be resolved.
   */
  StringVector processPREFIX_URLLIST(StylesheetHandler handler, String uri, String name, String rawName, String value)
    ...
    for (int i = 0; i < nStrings; i++) {
      ...
      if (url != null) {
        ...
        throw new org.xml.sax.SAXException(XSLMessages.createMessage(XSLTErrorResources.ER_CANT_RESOLVE_NSPREFIX,
                new Object[] { prefix }));
  ...
  /**
   * Process an attribute string of type T_PREFIXLIST into a vector of prefixes
   * that may be resolved to URLs.
   * 
   * @param handler
   *          non-null reference to current StylesheetHandler that is
   ...
   *         argument. If this attribute supports AVT, an AVT is returned.
   * 
   * @throws org.xml.sax.SAXException
   *           if the URL does not conform to the URL syntax.
   */
  Object processURL(StylesheetHandler handler, String uri, String name, String rawName, String value,
          ElemTemplateElement owner) throws org.xml.sax.SAXException {
   ...
   * 
   * @throws org.xml.sax.SAXException
   *           if the attribute value can not be processed.
   */
  Object processValue(StylesheetHandler handler, String uri, String name, String rawName, String value,
          ElemTemplateElement owner) throws org.xml.sax.SAXException {
   ...
   * 
   * @throws org.xml.sax.SAXException
   *           wraps an invocation exception if the setter method can not be
   *           invoked on the object.
   */
  void setDefAttrValue(StylesheetHandler handler, ElemTemplateElement elem) throws org.xml.sax.SAXException {
    setAttrValue(handler, this.getNamespace(), this.getName(), this.getName(), this.getDefault(), elem);
  }

  /**
   * Get the primative type for the class, if there is one. If the class is a
   * Double, for instance, this will return double.class. If the class is not
   * one of the 9 primative types, it will return the same class that was passed
   * in.
   * 
   * @param obj
   *          The object which will be resolved to a primative class object if
   *          possible.
   * 
   ...
   *          constructing the Templates.
   * @param attrUri
   *          The Namespace URI of the attribute, or an empty string.
   * @param attrLocalName
   *          The local name (without prefix), or empty string if not namespace
   *          processing.
   * @param attrRawName
   *          The raw name of the attribute, including possible prefix.
   * @param attrValue
   *          The attribute's value.
   * @param elem
   *          The object that should contain a property that represents the
   *          attribute.
   * 
   * @throws org.xml.sax.SAXException
   */
  boolean setAttrValue(StylesheetHandler handler, String attrUri, String attrLocalName, String attrRawName,
          String attrValue, ElemTemplateElement elem) throws org.xml.sax.SAXException {
    if (attrRawName.equals("xmlns") || attrRawName.startsWith("xmlns:"))
      return true;

    final String setterString = getSetterMethodName();
    ...
    if (null != setterString) {
      try {
        ...
        if (setterString.equals(S_FOREIGNATTR_SETTER)) {
          ...
          final Object value = processValue(handler, attrUri, attrLocalName, attrRawName, attrValue, elem);
        ...
        if (!setterString.equals(S_FOREIGNATTR_SETTER)) {
          handler.error(XSLTErrorResources.ER_FAILED_CALLING_METHOD, new Object[] { setterString }, nsme);// "Failed calling "
                                                                                                          // +
                                                                                                          // setterString
                                                                                                          // +
                                                                                                          // " method!",
        ...
        handler.error(XSLTErrorResources.ER_FAILED_CALLING_METHOD, new Object[] { setterString }, iae);// "Failed calling "
                                                                                                       // +
                                                                                                       // setterString
                                                                                                       // +
                                                                                                       // " method!",
                                                                                                       // iae);
        return false;
      } catch (final InvocationTargetException nsme) {
        handleError(handler, XSLTErrorResources.WG_ILLEGAL_ATTRIBUTE_VALUE, new Object[] { Constants.ATTRNAME_NAME,
                getName() }, nsme);
        return false;
      }
    }
