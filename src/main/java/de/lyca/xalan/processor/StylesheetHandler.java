...
import de.lyca.xalan.extensions.ExpressionVisitor;
...
import de.lyca.xml.utils.NamespaceSupport2;
...
public class StylesheetHandler extends DefaultHandler implements TemplatesHandler, PrefixResolver, NodeConsumer {
   ...
   */
  private final FunctionTable m_funcTable = new FunctionTable();
  ...
  public StylesheetHandler(TransformerFactoryImpl processor) throws TransformerConfigurationException {
    ...
    m_source_location = ((Boolean) processor.getAttribute(TransformerFactoryImpl.FEATURE_SOURCE_LOCATION))
            .booleanValue();
   ...
   *          string.
   * 
   * @return A non-null reference to an XPath object that represents the string
   *         argument.
   * 
  ...
  public XPath createXPath(String str, ElemTemplateElement owningTemplate)
          throws javax.xml.transform.TransformerException {
    final ErrorListener handler = m_stylesheetProcessor.getErrorListener();
    final XPath xpath = new XPath(str, owningTemplate, this, XPath.SELECT, handler, m_funcTable);
    // Visit the expression, registering namespaces for any extension functions
    // it includes.
   ...
   * 
   * @param str
   *          A non-null reference to a valid or invalid match pattern string.
   * 
   * @return A non-null reference to an XPath object that represents the string
   *         argument.
   * 
  ...
  XPath createMatchPatternXPath(String str, ElemTemplateElement owningTemplate)
          throws javax.xml.transform.TransformerException {
    final ErrorListener handler = m_stylesheetProcessor.getErrorListener();
    final XPath xpath = new XPath(str, owningTemplate, this, XPath.MATCH, handler, m_funcTable);
    // Visit the expression, registering namespaces for any extension functions
    // it includes.
    xpath.callVisitors(xpath, new ExpressionVisitor(getStylesheetRoot()));
    return xpath;
  ...
  /**
   * Given a namespace, get the corrisponding prefix from the current namespace
   * support context.
   * 
   * @param prefix
   *          The prefix to look up, which may be an empty string ("") for the
   *          default Namespace.
   * 
   ...
   * support the {@link de.lyca.xml.utils.PrefixResolver} interface, and will
   * throw an error if invoked on this object.
   * 
   * @param prefix
   *          The prefix to look up, which may be an empty string ("") for the
   *          default Namespace.
  ...
  XSLTElementProcessor getProcessorFor(String uri, String localName, String rawName) throws org.xml.sax.SAXException {
    ...
    final XSLTElementProcessor currentProcessor = getCurrentProcessor();
    ...
    if (null == elemProcessor
            ...
            && (null == getStylesheet()
                    || Double.valueOf(getStylesheet().getVersion()).doubleValue() > de.lyca.xml.utils.Constants.XSLTVERSUPPORTED
                    || !uri.equals(de.lyca.xml.utils.Constants.S_XSLNAMESPACEURL)
                    && currentProcessor instanceof ProcessorStylesheetElement || getElemVersion() > de.lyca.xml.utils.Constants.XSLTVERSUPPORTED)) {
      elemProcessor = def.getProcessorForUnknown(uri, localName);
    }

    if (null == elemProcessor) {
      error(XSLMessages.createMessage(XSLTErrorResources.ER_NOT_ALLOWED_IN_POSITION, new Object[] { rawName }), null);// rawName
                                                                                                                      // +
  ...
  /**
   * Receive a Locator object for document events. This is called by the parser
   * to push a locator for the stylesheet being parsed. The stack needs to be
   * popped after the stylesheet has been parsed. We pop in popStylesheet.
   * 
   * @param locator
   ...
   * included/imported stylesheets have been been fully parsed, as an indication
   * that composition/optimization/compilation can begin.
   * 
  ...
  /**
   * Test whether the _last_ endDocument() has been processed. This is needed as
   * guidance for stylesheet optimization and compilation engines, which
  ...
  public void endDocument() throws org.xml.sax.SAXException {
    ...
    try {
      ...
      final XSLTElementProcessor elemProcessor = getCurrentProcessor();
  ...
  public void startElement(String uri, String localName, String rawName, Attributes attributes)
    ...
    final XSLTElementProcessor elemProcessor = getProcessorFor(uri, localName, rawName);
   ...
   */
  @Override
  public void endElement(String uri, String localName, String rawName) throws org.xml.sax.SAXException {
    ...
    final XSLTElementProcessor p = getCurrentProcessor();
  ...
  public void processingInstruction(String target, String data) throws org.xml.sax.SAXException {
    ...
    try {
      ...
      // testing purposes.
      if ("xalan-doc-cache-off".equals(target) || "xalan:doc-cache-off".equals(target)
              || "doc-cache-off".equals(localName) && ns.equals("de.lyca.xalan.xslt.extensions.Redirect")) {
        if (!(m_elems.peek() instanceof ElemForEach))
          throw new TransformerException("xalan:doc-cache-off not allowed here!", getLocator());
        final ElemForEach elem = (ElemForEach) m_elems.peek();
   ...
   * 
   * @param msg
   *          An key into the {@link de.lyca.xalan.res.XSLTErrorResources}
   ...
   *           the current {@link javax.xml.transform.ErrorListener#warning}
  ...
  public void warn(String msg, Object args[]) throws org.xml.sax.SAXException {
    ...
    final SAXSourceLocator locator = getLocator();
    final ErrorListener handler = m_stylesheetProcessor.getErrorListener();
   ...
   * @param msg
   *          Error message to be passed to the RuntimeException as an argument.
   ...
   * @throws org.xml.sax.SAXException
   *           that wraps a {@link javax.xml.transform.TransformerException} if
   *           the current {@link javax.xml.transform.ErrorListener#error}
   *           method chooses to flag this condition as an error.
   * @xsl.usage internal
   */
   ...
   * 
   * @param msg
   *          A key into the {@link de.lyca.xalan.res.XSLTErrorResources} table,
   *          that is one of the WG_ prefixed definitions.
   * @param args
   *          An array of arguments for the given warning.
   * @param e
   *          An error which the SAXException should wrap.
   ...
   * @throws org.xml.sax.SAXException
   *           that wraps a {@link javax.xml.transform.TransformerException} if
   *           the current {@link javax.xml.transform.ErrorListener#fatalError}
   *           method chooses to flag this condition as an error.
   */
  @Override
  ...
  /**
   * If we have a URL to a XML fragment, the value is stored in this string, and
   * the m_shouldProcess flag is set to false until we match an ID with this
   * string. (warning: I worry that this should be in a stack).
   */
  private String m_fragmentIDString;

  /**
   * Keep track of the elementID, so we can tell when is has completed. This
   * isn't a real ID, but rather a nesting level. However, it's good enough for
   * our purposes. (warning: I worry that this should be in a stack).
   */
  ...
  /**
   * Get the XSLT TransformerFactoryImpl for needed services. TODO: This method
   * should be renamed.
   * 
   ...
   * 
   * @return The current stylesheet that is on top of the stylesheets stack, or
   *         null if no stylesheet is on the stylesheets stack.
   */
  ...
  /**
   * Push the current stylesheet being constructed. If no other stylesheets have
   * been pushed onto the stack, assume the argument is a stylesheet root, and
   * also set the stylesheet root member.
   * 
   * @param s
  ...
  /**
   * Pop the last stylesheet pushed, and return the stylesheet that this handler
   * is constructing, and set the last popped stylesheet member. Also pop the
   ...
   */
  private final XSLTSchema m_schema = new XSLTSchema();
   ...
   * 
   * @param elem
   *          Should be a non-null reference to the intended current template
   *          element.
   */
   ...
   * 
   * @param baseID
   *          The current base identifier for this position in the stylesheet,
   *          which may be a fragment identifier, or which may be null.
  ...
  void pushBaseIndentifier(String baseID) {
    ...
    if (null != baseID) {
      final int posOfHash = baseID.indexOf('#');
  ...
  public String getBaseIdentifier() {
    ...
    if (null == base) {
      final SourceLocator locator = getLocator();
   ...
   * 
   * @param hrefUrl
   *          non-null reference to the URL for the current imported stylesheet.
   */
  ...
  /**
   * Push the Source of an import href onto the stylesheet stack, obtained from
   * a URIResolver, null if there is no URIResolver, or if that resolver
   * returned null.
   */
  ...
  /**
   * Pop a Source from a user provided URIResolver, corresponding to the URL
   ...
   * 
   * @return a non-null reference to the current NamespaceSupport object, which
   ...
   * 
   * @see de.lyca.xml.utils.NodeConsumer
   */
   ...
   */
  private final BoolStack m_spacePreserveStack = new BoolStack();

  /**
   * Return boolean value from the spacePreserve stack depending on the value of
  ...
  /**
   * Push boolean value on to the spacePreserve stack depending on the value of
   * xml:space=default/preserve.
   * 
   * @param attrs
  ...
  private double getElemVersion() {
    ...
    double version = -1;
    while ((version == -1 || version == de.lyca.xml.utils.Constants.XSLTVERSUPPORTED) && elem != null) {
      try {
    ...
    return version == -1 ? de.lyca.xml.utils.Constants.XSLTVERSUPPORTED : version;
  }
