...
public class ProcessorInclude extends XSLTElementProcessor {
   ...
   * 
   * @return non-null reference to the href attribute string, or null if setHref
   *         has not been called.
   */
  ...
  public void startElement(StylesheetHandler handler, String uri, String localName, String rawName,
    ...
    try {
      ...
      final Source sourceFromURIResolver = getSourceFromUriResolver(handler);
      // Get the system ID of the included/imported stylesheet module
      final String hrefUrl = getBaseURIOfIncludedStylesheet(handler, sourceFromURIResolver);

      if (handler.importStackContains(hrefUrl))
        throw new org.xml.sax.SAXException(XSLMessages.createMessage(getStylesheetInclErr(), new Object[] { hrefUrl })); // "(StylesheetHandler) "+hrefUrl+" is directly or indirectly importing itself!");
   ...
   * the {@link StylesheetHandler} to a new state, and recurse in with a new set
   * of parse events. Once this function returns, the state of the
    ...
    try {
      ...
      if (null != uriresolver) {
        ...
        if (null != source && source instanceof DOMSource) {
          ...
          final TreeWalker walker = new TreeWalker(handler, new de.lyca.xml.utils.DOM2Helper(), systemId);
      ...
      if (null == source) {
        final String absURL = SystemIDResolver.getAbsoluteURI(getHref(), handler.getBaseIdentifier());
  ...
  /**
   * Get the base URI of the included or imported stylesheet, if the user
   * provided a URIResolver, then get the Source object for the stylsheet from
   * it, and get the systemId from that Source object, otherwise try to recover
   * by using the SysteIDResolver to figure out the base URI.
   * 
   * @param handler
   *          The handler that processes the stylesheet as SAX events, and
   *          maintains state
   * @param s
   *          The Source object from a URIResolver, for the included stylesheet
   *          module, so this will be null if there is no URIResolver set.
   */
