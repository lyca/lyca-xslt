...
/**
 * Provides applet host for the XSLT processor. To perform transformations on an
 * HTML client:
 * <ol>
 * <li>Use an &lt;applet&gt; tag to embed this applet in the HTML client.</li>
 * <li>Use the DocumentURL and StyleURL PARAM tags or the
 * {@link #setDocumentURL} and {@link #setStyleURL} methods to specify the XML
 * source document and XSL stylesheet.</li>
 * <li>Call the {@link #getHtmlText} method (or one of the transformToHtml()
 * methods) to perform the transformation and return the result as a String.</li>
 * </ol>
 * 
 * This class extends Applet which ultimately causes this class to implement
 * Serializable. This is a serious restriction on this class. All fields that
 * are not transient and not static are written-out/read-in during
 * serialization. So even private fields essentially become part of the API.
...
public class XSLTProcessorApplet extends Applet {
  ...
  /**
   * The stylesheet processor. This field is now transient because a
   * javax.xml.transform.TransformerFactory from JAXP makes no claims to be
   * serializable.
   */
  ...
  public String[][] getParameterInfo() {
    ...
    final String[][] info = { { PARAM_styleURL, "String", "URL to an XSL stylesheet" },
  ...
  /**
   * Automatically called when the HTML client containing the applet loads. This
   * method starts execution of the applet thread.
   */
  @Override
  public void start() {
    ...
    try {
      ...
      final StringReader xslbuf = new StringReader(
      ...
      synchronized (m_tfactory) {
        final Templates templates = m_tfactory.newTemplates(new StreamSource(xslbuf));
  ...
  /**
   * Set the URL to the XSL stylesheet that will be used to transform the input
   * XML. No processing is done yet.
   * 
   * @param urlString
  ...
  /**
   * Set the URL to the XML document that will be transformed with the XSL
   * stylesheet. No processing is done yet.
   * 
   * @param urlString
  ...
  /**
   * The processor keeps a cache of the source and style trees, so call this
   * method if they have changed or you want to do garbage collection.
   */
  ...
  /**
   * Set an attribute in the stylesheet, which gives the ability to have some
   * dynamic selection control.
   * 
   * @param nameOfIDAttrOfElemToModify
   *          The name of an attribute to search for a unique id.
   * @param elemId
   *          The unique ID to look for.
   * @param attrName
   *          Once the element is found, the name of the attribute to set.
   * @param value
  ...
  /**
   * Given a String containing markup, escape the markup so it can be displayed
   * in the browser.
   * 
   * @param s
   *          String to escape
  ...
  public String escapeString(String s) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      if ('<' == ch) {
        ...
        if (i + 1 >= length)
          throw new RuntimeException(XSLMessages.createMessage(XSLTErrorResources.ER_INVALID_UTF16_SURROGATE,
          ...
          if (!(0xdc00 <= next && next < 0xe000))
            throw new RuntimeException(XSLMessages.createMessage(XSLTErrorResources.ER_INVALID_UTF16_SURROGATE,
                    new Object[] { Integer.toHexString(ch) + " " + Integer.toHexString(next) })); // "Invalid UTF-16 surrogate detected: "
  ...
  /**
   * Assuming the stylesheet URL and the input XML URL have been set, perform
   * the transformation and return the result as a String.
   * 
  ...
  private String getSource() throws TransformerException {
    ...
    String text = "";
    try {
      final URL docURL = new URL(m_documentBase, m_treeURL);
      synchronized (m_tfactory) {
        final Transformer transformer = m_tfactory.newTransformer();
    ...
    } catch (final Exception any_error) {
   ...
   * Note that this is for display of the XML itself, not for rendering of HTML
   * by the browser.
   * 
   * @return XML source document as a string.
   * @throws Exception
   *           thrown if tree can not be converted.
   */
  ...
  /**
   * Get the HTML result Tree as a text string suitable for display in a
   * browser. Note that this is for display of the XML itself, not for rendering
   * of HTML by the browser.
   * 
   ...
   * If one of these is null, the existing value (of a previous transformation)
   * is not affected.
   * 
   * @param doc
   *          URL string to XML document
   * @param style
  ...
  /**
   * Process a document and a stylesheet and return the transformation result.
   * Use the xsl:stylesheet PI to find the document, if one exists.
   * 
   * @param doc
  ...
  /**
   * This class maintains a worker thread that that is trusted and can do things
   * like access data. You need this because the thread that is called by the
   * browser is not trusted and can't access data from the URLs.
   */
  class TrustedAgent implements Runnable {
    ...
    /**
     * Specifies whether the worker thread should get an XML document or XSL
     * stylesheet.
     */
