...
/**
 * Class that contains only static methods that are used to "serialize", these
 * methods are used by Xalan and are not in de.lyca.xml.serializer because they
 * have dependancies on the packages de.lyca.xpath or org. apache.xml.dtm or
 * de.lyca.xalan.transformer. The package de.lyca.xml. serializer should not
 * depend on Xalan or XSLTC.
 * 
 * @xsl.usage internal
 */
public class SerializerUtils {
  ...
  /**
   * Copy an DOM attribute to the created output element, executing attribute
   * templates as need be, and processing the xsl:use attribute.
   * 
   * @param handler
   *          SerializationHandler to which the attributes are added.
   * @param attr
   ...
   * 
   * As a quick hack to support ClonerToResultTree, this can also be used to
   * copy an individual namespace node.
   * 
   * @param src
   *          Source Node NEEDSDOC @param type NEEDSDOC @param dtm
   * 
   * @throws TransformerException
   */
  public static void processNSDecls(SerializationHandler handler, int src, int type, DTM dtm)
    ...
    try {
      if (type == DTM.ELEMENT_NODE) {
        for (int namespace = dtm.getFirstNamespaceNode(src, true); DTM.NULL != namespace; namespace = dtm
                .getNextNamespaceNode(src, namespace, true)) {
        ...
        final String prefix = dtm.getNodeNameX(src);
        // Brian M. - some changes here to get desturi
        final String desturi = handler.getNamespaceURIFromPrefix(prefix);
        final String srcURI = dtm.getNodeValue(src);
  ...
  /**
   * This function checks to make sure a given prefix is really declared. It
   * might not be, because it may be an excluded prefix. If it's not, it still
   * needs to be declared at this point. TODO: This needs to be done at an
   * earlier stage in the game... -sb
   * 
   * NEEDSDOC @param dtm NEEDSDOC @param namespace
   * 
   * @throws org.xml.sax.SAXException
   */
