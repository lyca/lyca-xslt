...
final class SortSettings {
   ...
   * 
   * @param translet
   *          {@link de.lyca.xalan.xsltc.runtime.AbstractTranslet} object for
   *          the transformation
   * @param sortOrders
   ...
   * @param collators
   *          an array specifying the Collation in effect for each sort level
   * @param caseOrders
   *          an array specifying whether upper-case, lower-case or neither is
   *          to take precedence for each sort level. The value of each element
   *          is equal to one of
   *          <code>"upper-first", "lower-first", or ""</code>.
   */
  SortSettings(AbstractTranslet translet, int[] sortOrders, int[] types, Locale[] locales, Collator[] collators,
          String[] caseOrders) {
    _translet = translet;
