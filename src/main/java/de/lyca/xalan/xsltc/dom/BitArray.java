...
public class BitArray implements Externalizable {
  ...
  /**
   * Set the mask for this bit array. The upper 8 bits of this mask indicate the
   * DOM in which the nodes in this array belong.
   */
   ...
   * sequence of next() call in an index iterator. This method is a mess, but it
   * is fast and it works, so don't change it.
   */
  ...
  /**
   * Merge two bit arrays. This currently only works for nodes from a single DOM
   * (because there is only one _mask per array).
   */
  public final BitArray merge(BitArray other) {
    ...
    // Only merge if other array has any bits set
      ...
      final int start = _first < other._first ? _first : other._first;
