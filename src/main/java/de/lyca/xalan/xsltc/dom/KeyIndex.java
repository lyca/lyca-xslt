...
public class KeyIndex extends DTMAxisIteratorBase {
  ...
  /**
   * Adds a node to the node list for a given value. Nodes will always be added
   * in document order.
   */
  ...
  /**
   * This method must be called by the code generated by the id() function prior
   * to returning the node iterator. The lookup code for key() and id() differ
   * in the way the lookup value can be whitespace separated list of tokens for
   * the id() function, but a single string for the key() function.
   * 
   * @deprecated
   */
  @Deprecated
   ...
   * @return <code>1</code> if the context node is in the set of nodes returned
   *         by the reference to the <code>id</code> function; <code>0</code>,
   *         otherwise
   */
   ...
   * function, checks whether the context node is in the set of nodes that
   * results from that reference to the <code>key</code> function. This is used
   * in the implementation of key patterns.
   * </p>
   * <p>
   * This particular {@link KeyIndex} object is the result evaluating the first
   * argument to the <code>key</code> function, so it's not taken into any
   * further account.
   * </p>
   * 
   * @param node
  ...
  /**
   * Create a {@link KeyIndexIterator} that iterates over the nodes that result
   * from a reference to the XSLT <code>key</code> function or XPath
   * <code>id</code> function.
   * 
   * @param keyValue
   *          A string or iterator representing the key values or id references
   * @param isKeyCall
   *          A <code>boolean</code> indicating whether the iterator is being
   *          created for a reference <code>key</code> or <code>id</code>
   */
  ...
  public class KeyIndexIterator extends MultiValuedNodeHeapIterator {
    ...
    /**
     * <p>
     * A reference to the <code>key</code> function that only has one key value
     * or to the <code>id</code> function that has only one string argument can
     * be optimized to ignore the multi-valued heap. This field will be
     ...
     * argument to the <code>key</code> function or a node set argument to the
     * <code>id</code> function.
     * </p>
     * 
     * <p>
    ...
    /**
     * <p>
     * This field contains the iterator representing a non-node-set key value
     * argument to the <code>key</code> function or a non-node-set argument to
    ...
    /**
     * Indicates whether this object represents the result of a reference to the
     * <code>key</code> function (<code>true</code>) or the <code>id</code>
     * function (<code>false</code>).
     */
    private final boolean _isKeyIterator;
    ...
    protected class KeyIndexHeapNode extends MultiValuedNodeHeapIterator.HeapNode {
      /**
       * {@link IntegerArray} of DTM nodes retrieved for one key value. Must
       * contain no duplicates and be stored in document order.
       */
      private IntegerArray _nodes;

      /**
       * Position in {@link #_nodes} array of next node to return from this heap
       * node.
       */
      ...
      /**
       * Create a heap node representing DTM nodes retrieved for one key value
       * in a reference to the <code>key</code> function or string argument to
       * the <code>id</code> function.
       */
       ...
       * {@link KeyIndexIterator#setStartNode(int)} method should create new
       * instances of this class to represent the effect of changing the
       * context.
       * </p>
       */
      @Override
    ...
    /**
     * Constructor used when the argument to <code>key</code> or <code>id</code>
     * is not a node set.
     * 
     * @param keyValue
     *          the argument to <code>key</code> or <code>id</code> cast to a
     *          <code>String</code>
     * @param isKeyIterator
    ...
    /**
     * Set context node for the iterator. This will cause the iterator to reset
     * itself, reevaluate arguments to the function, look up nodes in the input
     * and reinitialize its internal heap.
     * 
     * @param node
     *          the context node
     ...
     * function with the context specified by {@link #setStartNode(int)} and set
     * up this iterator to iterate over the DTM nodes that are to be returned.
     */
    @Override
    protected void init() {
      super.init();
      ...
      final int rootHandle = _dom.getAxisIterator(Axis.ROOT).setStartNode(_startNode).next();
      ...
      if (_keyValueIterator == null) {
        ...
        for (int keyValueNode = keyValues.next(); keyValueNode != DTMAxisIterator.END; keyValueNode = keyValues.next()) {
          ...
          final String keyValue = BasisLibrary.stringF(keyValueNode, _dom);

          final IntegerArray nodes = lookupNodes(rootHandle, keyValue);
