...
/**
 * <p>
 * <code>MultiValuedNodeHeapIterator</code> takes a set of multi-valued heap nodes and produces a merged NodeSet in
 * document order with duplicates removed.
 * </p>
 * <p>
 * Each multi-valued heap node (which might be a {@link de.lyca.xml.dtm.DTMAxisIterator}, but that's not necessary)
 * generates DTM node handles in document order. The class maintains the multi-valued heap nodes in a heap, not
 * surprisingly, sorted by the next DTM node handle available form the heap node.
 * </p>
 * <p>
 * After a DTM node is pulled from the heap node that's at the top of the heap, the heap node is advanced to the next
 * DTM node handle it makes available, and the heap nature of the heap is restored to ensure the next DTM node handle
...
public abstract class MultiValuedNodeHeapIterator extends DTMAxisIteratorBase {
  ...
  public abstract class HeapNode implements Cloneable {
    ...
    /**
     * Creates a deep copy of this {@link HeapNode}. The clone is not reset from the current position of the original.
     * 
     ...
     * @return <code>true</code> if and only if the current node for this heap node is before the current node of the
  ...
  protected void addHeapNode(HeapNode node) {
    if (_free == _size) {
      final HeapNode[] newArray = new HeapNode[_size *= 2];
  ...
  /*
   * Build a heap in document order. put the smallest node on the top. "smallest node" means the node before other nodes
   * in document order
   */
