 ...
 * {@link de.lyca.xml.dtm.DTMAxisIterator}, but that's not necessary) generates
 * DTM node handles in document order. The class maintains the multi-valued heap
 * nodes in a heap, not surprisingly, sorted by the next DTM node handle
 * available form the heap node.
 * </p>
 * <p>
...
public abstract class MultiValuedNodeHeapIterator extends DTMAxisIteratorBase {
  ...
  public abstract class HeapNode implements Cloneable {
    ...
    /**
     * Creates a deep copy of this {@link HeapNode}. The clone is not reset from
     * the current position of the original.
     * 
     ...
     * @return <code>true</code> if and only if the current node for this heap
     *         node is before the current node of the argument heap node in
     *         document order.
     */
