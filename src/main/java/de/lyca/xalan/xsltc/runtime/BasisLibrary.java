...
public final class BasisLibrary {
  ...
  public static boolean compare(Object left, Object right, int op, DOM dom) {
    ...
    if (hasSimpleArgs) {
      ...
      if (right instanceof DTMAxisIterator) {
        ...
        final boolean temp = ((Boolean) right).booleanValue();
  ...
  public static org.w3c.dom.Node referenceToNode(Object obj, DOM dom) {
    if (obj instanceof Node || obj instanceof DTMAxisIterator) {
      ...
      dom = (DOM) obj;
      final DTMAxisIterator iter = dom.getChildren(DTMDefaultBase.ROOTNODE);
      return dom.makeNode(iter);
    } else {
      final String className = obj.getClass().getName();
  ...
  public static DTMAxisIterator node2Iterator(org.w3c.dom.Node node, Translet translet, DOM dom) {
    ...
    final org.w3c.dom.NodeList nodelist = new org.w3c.dom.NodeList() {
      @Override
  ...
  public static DTMAxisIterator nodeList2Iterator(org.w3c.dom.NodeList nodeList, Translet translet, DOM dom) {
    ...
    if (doc != null) {
      ...
      final DTMAxisIterator iter2 = idom.getAxisIterator(Axis.CHILD);
  ...
  public static void copy(Object obj, SerializationHandler handler, int node, DOM dom) {
    try {
      if (obj instanceof DTMAxisIterator) {
        ...
        // ((DOM)obj).copy(((de.lyca.xml.dtm.ref.DTMDefaultBase)((DOMAdapter)obj).getDOMImpl()).getDocument(),
        // handler);
        final DOM newDom = (DOM) obj;
        newDom.copy(newDom.getDocument(), handler);
      } else {
        final String string = obj.toString(); // or call stringF()
  ...
  public static String lookupStylesheetQNameNamespace(String lexicalQName, int stylesheetNodeID, int[] ancestorNodeIDs,
    ...
    if (prefix != null) {
      ...
      nodeLoop: for (int currentNodeID = stylesheetNodeID; currentNodeID >= 0; currentNodeID = ancestorNodeIDs[currentNodeID]) {
