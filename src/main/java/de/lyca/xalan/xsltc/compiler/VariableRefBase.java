...
class VariableRefBase extends Expression {
  ...
  /**
   * A reference to the enclosing expression/instruction for which a closure is
   * needed (Predicate, Number or Sort).
   */
  ...
  /**
   * If this variable reference is in a top-level element like another variable,
   * param or key, add a dependency between that top-level element and the
   * referenced variable. For example,
   * 
   * <xsl:variable name="x" .../> <xsl:variable name="y" select="$x + 1"/>
   * 
   * and assuming this class represents "$x", add a reference between variable y
   * and variable x.
   */
