...
public abstract class SyntaxTreeNode {
  ...
  protected JExpression compileResultTree(CompilerContext ctx) {
    ...
    JVar resultTreeFrag = body.decl(ctx.ref(DOM.class), nextResultTreeFrag, dom.invoke(GET_RESULT_TREE_FRAG)
        .arg(lit(RTF_INITIAL_SIZE)).arg(lit(rtfType)).arg(lit(stylesheet.callsNodeset())));
    ...
    if (stylesheet.callsNodeset()) {// && !DOM_CLASS.equals(DOM_IMPL_CLASS)) {
      ...
      // DOM_INTF_SIG + "[" + STRING_SIG + "[" + STRING_SIG
      // + "[I" + "[" + STRING_SIG + ")V");
      ...
      JExpression adapter = _new(ctx.ref(DOMAdapter.class)).arg(resultTreeFrag).arg(direct(NAMES_INDEX))
          .arg(direct(URIS_INDEX)).arg(direct(TYPES_INDEX)).arg(direct(NAMESPACE_INDEX));
  ...
  protected final int getNodeIDForStylesheetNSLookup() {
    if (_nodeIDForStylesheetNSLookup == UNKNOWN_STYLESHEET_NODE_ID) {
      ...
      if (prefixMapping == null) {
        ...
        _nodeIDForStylesheetNSLookup = getXSLTC().registerStylesheetPrefixMappingForRuntime(prefixMapping,
            parentNodeID);
      }
    }
   ...
   *          The error message.
   */
  protected void reportError(SyntaxTreeNode element, Parser parser, String message) {
    final ErrorMsg error = new ErrorMsg(element, message);
   ...
   *          The error message.
   */
  protected void reportWarning(SyntaxTreeNode element, Parser parser, String message) {
    final ErrorMsg error = new ErrorMsg(element, message);
