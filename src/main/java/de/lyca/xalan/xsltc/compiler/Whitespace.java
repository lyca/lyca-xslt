...
final class Whitespace extends TopLevelElement {
  ...
  public void parseContents(Parser parser) {
    ...
    final StringBuffer elements = new StringBuffer(Constants.EMPTYSTRING);
  ...
  /**
   * Scans through the rules vector and looks for a rule of higher priority that
  ...
  private static WhitespaceRule findContradictingRule(Vector rules, WhitespaceRule rule) {
    for (int i = 0; i < rules.size(); i++) {
      ...
      final WhitespaceRule currentRule = (WhitespaceRule) rules.elementAt(i);
  ...
  private static void compilePredicate(Vector rules, int defaultAction, ClassGenerator classGen) {
    ...
    for (int i = 0; i < rules.size(); i++) {
      ...
      final WhitespaceRule rule = (WhitespaceRule) rules.elementAt(i);
