...
import de.lyca.xalan.xsltc.compiler.Pattern;
 ...
 */
public class MethodGenerator extends MethodGen implements de.lyca.xalan.xsltc.compiler.Constants {
  ...
  private final SlotAllocator _slotAllocator;
  private boolean _allocatorInit = false;
  private LocalVariableRegistry _localVariableRegistry;

  /**
   * A mapping between patterns and instruction lists used by test sequences to
  ...
  public MethodGenerator(int access_flags, Type return_type, Type[] arg_types, String[] arg_names, String method_name,
    ...
    final int attribute = cpg.addInterfaceMethodref(TRANSLET_OUTPUT_INTERFACE, "addAttribute", "(" + STRING_SIG
            + STRING_SIG + ")V");
    ...
    final int namespace = cpg.addInterfaceMethodref(TRANSLET_OUTPUT_INTERFACE, "namespaceAfterStartElement", "("
  ...
  public LocalVariableGen addLocalVariable2(String name, Type type, InstructionHandle start) {
    final LocalVariableGen lvg = super.addLocalVariable(name, type, _slotAllocator.allocateSlot(type), start, null);
    getLocalVariableRegistry().registerLocalVariable(lvg);
    return lvg;
  }
  ...
  protected class LocalVariableRegistry {
     ...
     * variable. The JVM stack frame of local variables is divided into "slots".
     * A single slot can be used to store more than one variable in a method,
     * without regard to type, so long as the byte code keeps the ranges of the
     * two disjoint.
     * </p>
     * <p>
     * If only one registration of use of a particular slot occurs, the
     * corresponding entry of <code>_variables</code> contains the
     * <code>LocalVariableGen</code>; if more than one occurs, the corresponding
     ...
     * <b>Preconditions:</b>
     * <ul>
     * <li>The range of instructions for <code>lvg</code> does not overlap with
     * the range of instructions for any <code>LocalVariableGen</code> with the
     * same slot index previously registered for this method. <b>
    ...
    protected void registerLocalVariable(LocalVariableGen lvg) {
      ...
      final int registrySize = _variables.size();
     ...
     * {@link InstructionList} associated with this {@link MethodGenerator}.</li>
     * </ul>
     * </p>
     * 
     * @param slot
     ...
     * @return the <code>LocalVariableGen</code> for the local variable stored
     *         in the relevant slot at the relevant offset; <code>null</code> if
     *         there is none.
     */
    protected LocalVariableGen lookupRegisteredLocalVariable(int slot, int offset) {
      final Object localsInSlot = _variables != null ? _variables.get(slot) : null;
    ...
    /**
     * <p>
     * Set up a mapping of the name of the specified {@link LocalVariableGen}
     * object to the <code>LocalVariableGen</code> itself.
     * </p>
     * <p>
     * This is a bit of a hack. XSLTC is relying on the fact that the name that
     * is being looked up won't be duplicated, which isn't guaranteed. It
     * replaces code which used to call {@link MethodGen#getLocalVariables()}
     * and looped through the <code>LocalVariableGen</code> objects it contained
     * to find the one with the specified name. However,
     * <code>getLocalVariables()</code> has the side effect of setting the start
     * and end for any <code>LocalVariableGen</code> which did not already have
     * them set, which causes problems for outlining..
     * </p>
     * <p>
     ...
     * {@link MethodGen#getLocalVariables()} which has a side-effect of setting
     * the start and end range for any <code>LocalVariableGen</code> if either
     * was <code>null</code>. That side-effect causes problems for outlining of
     * code in XSLTC.
     * 
     * @param includeRemoved
     *          Specifies whether all local variables ever declared should be
     *          returned (<code>true</code>) or only those not removed (
     *          <code>false</code>)
  ...
  public Instruction loadCurrentNode() {
    if (_iloadCurrent == null) {
      final int idx = getLocalIndex("current");
      if (idx > 0) {
  ...
  /**
   * Get the instruction list for a pre-compiled pattern. Used by test sequences
  ...
  private static class Chunk implements Comparable<Chunk> {
     ...
     *          outlineable chunk.
     * @param end
     *          The {@link InstructionHandle} of the last instruction in the
     *          outlineable chunk.
     */
     ...
     * followed immediately by the argument <code>MethodGenerator.Chunk</code>,
     * with no other intervening instructions, including
     * {@link OutlineableChunkStart} or {@link OutlineableChunkEnd}
     * instructions.
     * 
     * @param neighbour
     *          an outlineable {@link MethodGenerator.Chunk}
     * @return <code>true</code> if and only if the argument chunk immediately
     *         follows <code>this</code> chunk
     */
  ...
  private List<Chunk> getCandidateChunks(ClassGenerator classGen, int totalMethodSize) {
    ...
    if (m_openChunks != 0) {
      final String msg = new ErrorMsg(ErrorMsg.OUTLINE_ERR_UNBALANCED_MARKERS).toString();
  ...
  private List<Chunk> mergeAdjacentChunks(Chunk[] chunks) {
    ...
    if (chunks.length - startOfCurrentRun > 1) {
      final int lengthOfRun = chunks.length - startOfCurrentRun;
   ...
   * methods so that no method exceeds the 64KB limit on the length of the byte
   * code associated with a method.
   * 
   * @param classGen
   *          The {@link ClassGen} with which the generated methods will be
   *          associated
  ...
  public Method[] outlineChunks(ClassGenerator classGen, int originalMethodSize) {
    ...
    do {
      ...
      for (int i = candidateChunks.size() - 1; i >= 0 && currentMethodSize > TARGET_METHOD_SIZE; i--) {
        ...
        final InstructionList il = getInstructionList();
        final InstructionHandle lastInst = il.getEnd();
        il.setPositions();
    ...
    if (currentMethodSize > MAX_METHOD_SIZE) {
      final String msg = new ErrorMsg(ErrorMsg.OUTLINE_ERR_METHOD_TOO_BIG).toString();
      throw new InternalError(msg);
    }
   ...
   *          chunk to outline
   * @param last
   *          The <code>InstructionHandle</code> of the last instruction in the
   *          chunk to outline
   * @param outlinedMethodName
   *          The name of the new method
   * @param classGen
   *          The {@link ClassGenerator} of which the original and new methods
   *          will be members
  ...
  private Method outline(InstructionHandle first, InstructionHandle last, String outlinedMethodName,
    ...
    if (getExceptionHandlers().length != 0) {
      final String msg = new ErrorMsg(ErrorMsg.OUTLINE_ERR_TRY_CATCH).toString();
      throw new InternalError(msg);
    }
    ...
    final MethodGenerator outlinedMethodGen = new MethodGenerator(methodAttributes, org.apache.bcel.generic.Type.VOID,
            argTypes, argNames, outlinedMethodName, getClassName(), newIL, cpg);
    ...
    final ClassGenerator copyAreaCG = new ClassGenerator(argTypeName, OBJECT_CLASS, argTypeName + ".java", ACC_FINAL
    ...
    if (isStaticMethod) {
      outlinedMethodRef = oldMethCopyOutIL.append(new INVOKESTATIC(cpg.addMethodref(classGen.getClassName(),
      ...
      outlinedMethodRef = oldMethCopyOutIL.append(new INVOKEVIRTUAL(cpg.addMethodref(classGen.getClassName(),
              outlinedMethodName, outlinedMethodGen.getSignature())));
    }
    ...
    for (InstructionHandle ih = first; ih != limit; ih = ih.getNext()) {
      ...
      if (inst instanceof MarkerInstruction) {
        ...
        // InstructionList.
        final Instruction c = inst.copy(); // Use clone for shallow copy
        ...
        if (c instanceof LocalVariableInstruction || c instanceof RET) {
          ...
          if (localVarMap.get(oldLVG) == null) {
            ...
            final boolean copyInLocalValue = offsetInLocalVariableGenRange(oldLVG,
                    outlineChunkStartOffset != 0 ? outlineChunkStartOffset - 1 : 0);
            final boolean copyOutLocalValue = offsetInLocalVariableGenRange(oldLVG, outlineChunkEndOffset + 1);
            ...
            if (copyInLocalValue || copyOutLocalValue) {
              ...
              copyAreaCG.addField(new Field(ACC_PUBLIC, copyAreaCPG.addUtf8(copyAreaFieldName), copyAreaCPG
                      .addUtf8(varSignature), null, copyAreaCPG.getConstantPool()));
      ...
      if (ih.hasTargeters()) {
        ...
        for (int idx = 0; idx < targeters.length; idx++) {
          ...
          if (targeter instanceof LocalVariableGen && ((LocalVariableGen) targeter).getEnd() == ih) {
            final Object newLVG = localVarMap.get(targeter);
            if (newLVG != null) {
    ...
    // new outlined method
    final InstructionList oldMethodIL = getInstructionList();
    ...
    try {
      ...
      for (int i = 0; i < targets.length; i++) {
        final InstructionHandle lostTarget = targets[i];
        final InstructionTargeter[] targeters = lostTarget.getTargeters();
        for (int j = 0; j < targeters.length; j++) {
          if (targeters[j] instanceof LocalVariableGen) {
            final LocalVariableGen lvgTargeter = (LocalVariableGen) targeters[j];
    ...
    final String[] exceptions = getExceptions();
   ...
   * {@link LoadInstruction} based on the specified {@link Type} that will load
  ...
  Method[] getGeneratedMethods(ClassGenerator classGen) {
    ...
    if (instructionListSize > MAX_BRANCH_TARGET_OFFSET) {
      final boolean ilChanged = widenConditionalBranchTargetOffsets();
  ...
  /**
   * <p>
   * Rewrites branches to avoid the JVM limits of relative branch offsets. There
   * is no need to invoke this method if the bytecode for the
   * {@link MethodGenerator} does not exceed 32KB.
   * </p>
   * <p>
   * The Java Virtual Machine Specification permits the code portion of a method
   * to be up to 64KB in length. However, some control transfer instructions
   * specify relative offsets as a signed 16-bit quantity, limiting the range to
   * a subset of the instructions that might be in a method.
   * </p>
   * <p>
   ...
   * </p>
   * <p>
   * This leaves the various conditional branch instructions, <code>IFEQ</code>,
   * <code>IFNULL</code>, <code>IF_ICMPEQ</code>, <em>et al.</em>, all of which
   * use 16-bit signed relative offsets, with no 32-bit wide form available.
   * </p>
   * <p>
   * This method scans the {@link InstructionList} associated with this
   * {@link MethodGenerator} and finds all conditional branch instructions that
   * might exceed the 16-bit limitation for relative branch offsets. The logic
   * of each such instruction is inverted, and made to target the instruction
   * which follows it. An unconditional branch to the original target of the
   * instruction is then inserted between the conditional branch and the
   * instruction which previously followed it. The unconditional branch is
   * permitted to have a 16-bit or a 32-bit relative offset, as described above.
   * For example, <code>
   * 1234:   NOP
   *          ...
   * 55278:  IFEQ -54044
   ...
   * </p>
   * 
   * @return <code>true</code> if the <code>InstructionList</code> was modified;
      ...
      if (inst instanceof IfInstruction) {
        ...
        if (relativeTargetOffset - maxOffsetChange < MIN_BRANCH_TARGET_OFFSET
          ...
          try {
            ...
            final String msg = new ErrorMsg(ErrorMsg.OUTLINE_ERR_DELETED_TARGET, tle.getMessage()).toString();
            throw new InternalError(msg);
          }
