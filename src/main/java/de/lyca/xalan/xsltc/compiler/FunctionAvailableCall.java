...
final class FunctionAvailableCall extends FunctionCall {
  ...
  public FunctionAvailableCall(QName fname, List<Expression> arguments) {
    ...
    if (_arg instanceof LiteralExpr) {
      final LiteralExpr arg = (LiteralExpr) _arg;
  ...
  /**
   * Argument of function-available call must be literal, typecheck returns the
   * type of function-available to be boolean.
   */
  @Override
  public Type typeCheck(SymbolTable stable) throws TypeCheckError {
    ...
    final ErrorMsg err = new ErrorMsg(this, Messages.get().needLiteralErr("function-available"));
    throw new TypeCheckError(err);
  }
  ...
  private boolean hasMethods() {
    ...
    final int colonIndex = _nameOfFunct.indexOf(":");
    if (colonIndex > 0) {
      final String functionName = _nameOfFunct.substring(colonIndex + 1);
      final int lastDotIndex = functionName.lastIndexOf('.');
      if (lastDotIndex > 0) {
