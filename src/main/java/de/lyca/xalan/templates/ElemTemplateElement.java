...
import de.lyca.xml.utils.UnImplNode;
import de.lyca.xpath.ExpressionNode;
import de.lyca.xpath.WhitespaceStrippingElementMatcher;
...
public class ElemTemplateElement extends UnImplNode implements PrefixResolver, Serializable, ExpressionNode,
   ...
   * until it calls getStylesheetComposed on a Stylesheet object, which will Get
   * the owning aggregated stylesheet, or that stylesheet if it is aggregated.
   * 
  ...
  /**
   * This function is called during recomposition to control how this element is
   * composed.
   */
  ...
  public void compose(StylesheetRoot sroot) throws TransformerException {
    resolvePrefixTables();
    final ElemTemplateElement t = getFirstChildElem();
    m_hasTextLitOnly = t != null && t.getXSLToken() == Constants.ELEMNAME_TEXTLITERALRESULT
  ...
  public void error(String msg, Object[] args) {
    ...
    throw new RuntimeException(XSLMessages.createMessage(XSLTErrorResources.ER_ELEMTEMPLATEELEM_ERR,
            new Object[] { themsg }));
  }
  ...
  public Node appendChild(Node newChild) throws DOMException {
    ...
    if (null == m_firstChild) {
      ...
      final ElemTemplateElement last = (ElemTemplateElement) getLastChild();
  ...
  public ElemTemplateElement appendChild(ElemTemplateElement elem) {
    ...
    if (null == m_firstChild) {
      ...
      final ElemTemplateElement last = getLastChildElem();
   ...
   * 
   * @param childETE
   *          The child to remove. This operation is a no-op if oldChild is not
   *          a child of this node.
   * 
   * @return the removed child, or null if the specified node was not a child of
   *         this element.
   */
  ...
  /**
   * From the SAX2 helper class, set the namespace table for this element. Take
   * care to call resolveInheritedNamespaceDecls. after all namespace
   * declarations have been added.
   * 
   * @param nsSupport
   *          non-null reference to NamespaceSupport from the ContentHandler.
   * 
   * @throws TransformerException
   */
  ...
  /**
   * Copy the namespace declarations from the NamespaceSupport object. Take care
   * to call resolveInheritedNamespaceDecls. after all namespace declarations
   * have been added.
   * 
   * @param nsSupport
   *          non-null reference to NamespaceSupport from the ContentHandler.
  ...
  public void setPrefixes(NamespaceSupport nsSupport, boolean excludeXSLDecl) throws TransformerException {
    ...
    while (decls.hasMoreElements()) {
      ...
      if (excludeXSLDecl && uri.equals(de.lyca.xml.utils.Constants.S_XSLNAMESPACEURL)) {
        continue;
      }
  ...
  /**
   * The table of {@link XMLNSDecl}s for this element and all parent elements,
   * screened for excluded prefixes.
   * 
   * @serial
   */
   ...
   * @param uri
   *          reference to namespace that prefix maps to, which is protected for
   *          null, but should really never be passed as null.
   * 
  ...
  private boolean excludeResultNSDecl(String prefix, String uri) throws TransformerException {
    ...
    if (uri != null) {
      if (uri.equals(de.lyca.xml.utils.Constants.S_XSLNAMESPACEURL) || getStylesheet().containsExtensionElementURI(uri))
        return true;
  ...
  public void resolvePrefixTables() throws TransformerException {
    ...
    if (null != m_declaredPrefixes) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        final boolean shouldExclude = excludeResultNSDecl(prefix, uri);
  ...
  /**
   * Send startPrefixMapping events to the result tree handler for all declared
  ...
  /**
   * Get the previous sibling (as a Node) or return null. Note that this may be
   * expensive if the parent has many kids; we accept that price in exchange for
   * avoiding the prev pointer TODO: If we were sure parents and sibs are always
   * ElemTemplateElements, we could hit the fields directly rather than thru
   * accessors.
   * 
  ...
  public ElemTemplateElement getPreviousSiblingElem() {
    ...
    if (walker != null) {
      for (walker = walker.getFirstChildElem(); walker != null; prev = walker, walker = walker.getNextSiblingElem()) {
        if (walker == this)
          return prev;
      }
    }
   ...
   */
  @Override
  public boolean shouldStripWhiteSpace(de.lyca.xpath.XPathContext support, org.w3c.dom.Element targetElement)
          throws TransformerException {
