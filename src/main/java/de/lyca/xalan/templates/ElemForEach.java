...
public class ElemForEach extends ElemTemplateElement implements ExpressionOwner {
   ...
   * "xalan:doc-cache-off" pi. The old form of the PI only works for XML parsers
   * that are not namespace aware. It tells the engine that documents created in
   * the location paths executed by this element will not be reparsed. It's set
   * by StylesheetHandler during construction. Note that this feature applies
   * _only_ to xsl:for-each elements in its current incarnation; a more general
   * cache management solution is desperately needed.
   */
  ...
  public void compose(StylesheetRoot sroot) throws TransformerException {
    ...
    final int length = getSortElemCount();
   ...
   * 
   * @param xctxt
   *          The XPath runtime state for the sort.
   * @param keys
   *          Vector of sort keyx
   * @param sourceNodes
  ...
  public void transformSelectedNodes(TransformerImpl transformer) throws TransformerException {
    ...
    try {
      ...
      if (transformer.getDebug()) {
         ...
         * "select", new XPath(m_selectExpression), new
         * de.lyca.xpath.objects.XNodeSet(sourceNodes));
         */
        ...
        final Expression expr = m_xpath.getExpression();
        final de.lyca.xpath.objects.XObject xObject = expr.execute(xctxt);
      ...
      final IntStack currentNodes = xctxt.getCurrentNodeStack();

      xctxt.pushCurrentExpressionNode(DTM.NULL);

      final IntStack currentExpressionNodes = xctxt.getCurrentExpressionNodeStack();
      ...
      while (DTM.NULL != (child = sourceNodes.nextNode())) {
        ...
        // each of them.
        for (ElemTemplateElement t = m_firstChild; t != null; t = t.m_nextSibling) {
        ...
        if (m_doc_cache_off) {
          ...
          // NOTE: This will work because this is _NOT_ a shared DTM, and thus
          // has
      ...
      if (transformer.getDebug()) {
        transformer.getTraceManager().fireSelectedEndEvent(sourceNode, this, "select", new XPath(m_selectExpression),
                new de.lyca.xpath.objects.XNodeSet(sourceNodes));
      }
  ...
  /**
   * Add a child to the child list. <!ELEMENT xsl:apply-templates
   * (xsl:sort|xsl:with-param)*> <!ATTLIST xsl:apply-templates select %expr;
   * "node()" mode %qname; #IMPLIED >
   * 
   * @param newChild
