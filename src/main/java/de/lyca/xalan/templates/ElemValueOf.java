...
public class ElemValueOf extends ElemTemplateElement {
  ...
  /**
   * Set the "select" attribute. The required select attribute is an expression;
  ...
  /**
   * Get the "select" attribute. The required select attribute is an expression;
   * this expression is evaluated and the resulting object is converted to a
   * string as if by a call to the string function.
   * 
   ...
   * 
   * @param v
   *          The value to set for the "disable-output-escaping" attribute.
   */
  ...
  /**
   * Execute the string expression and copy the text to the result tree. The
   * required select attribute is an expression; this expression is evaluated
   * and the resulting object is converted to a string as if by a call to the
   * string function. The string specifies the string-value of the created text
   * node. If the string is empty, no text node will be created. The created
  ...
  public void execute(TransformerImpl transformer) throws TransformerException {
    ...
    try {
      ...
      if (false && m_isDot && !transformer.getDebug()) {
        final int child = xctxt.getCurrentNode();
        final DTM dtm = xctxt.getDTM(child);
        ...
        try {
          final Expression expr = m_selectExpression.getExpression();
