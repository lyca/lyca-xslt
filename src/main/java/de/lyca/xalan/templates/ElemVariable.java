...
import de.lyca.xpath.objects.XRTreeFragSelectWrapper;
...
public class ElemVariable extends ElemTemplateElement {
   ...
   * declares an RTF, which is equal to the maximum number of variables that can
   * be declared in the variable at one time.
   */
  int m_frameSize = -1;

  /**
   * Sets the relative position of this variable within the stack frame (if
   * local) or the global area (if global). Note that this should be called only
   * for global variables since the local position is computed in the compose()
   * method.
   */
   ...
   * name attribute, which specifies the name of the variable. The value of the
   * name attribute is a QName, which is expanded as described in [2.4 Qualified
   * Names].
   * 
   ...
   * 
   * @param v
   *          Boolean indicating whether this is a top-level variable or param,
   *          or not.
   */
  ...
  /**
   * If the children of a variable is a single xsl:value-of or text literal, it
   * is cheaper to evaluate this as an expression, so try and adapt the child an
   * an expression.
   * 
   * @param varElem
  ...
  /**
   * Add a child to the child list. If the select attribute is present, an error
   * will be raised.
   * 
   * @param elem
   *          New element to append to this element's children list
   * 
   * @return null if the select attribute was present, otherwise the child just
