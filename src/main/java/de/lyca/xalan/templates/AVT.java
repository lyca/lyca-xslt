...
import de.lyca.xalan.processor.StylesheetHandler;
...
public class AVT implements java.io.Serializable, XSLTVisitable {
  ...
  /**
   * INIT_BUFFER_CHUNK_BITS is used to set initial size of of the char m_array
   * in FastStringBuffer if USE_OBJECT_POOL == false. size = 2^
   * INIT_BUFFER_CHUNK_BITS, INIT_BUFFER_CHUNK_BITS = 7 corresponds size = 256.
   */
  ...
  /**
   * Construct an AVT by parsing the string, and either constructing a vector of
   * AVTParts, or simply hold on to the string if the AVT is simple.
   * 
   * @param handler
   *          non-null reference to StylesheetHandler that is constructing.
   * @param uri
   *          non-null reference to URI, "" if null namespace.
   * @param name
   *          non-null reference to name string.
  ...
  public AVT(StylesheetHandler handler, String uri, String name, String rawName, String stringedValue,
          ElemTemplateElement owner) throws javax.xml.transform.TransformerException {
            ...
            switch (t.charAt(0)) {
              ...
              case '{': {
                ...
                try {
                   ...
                   * // Error. Expressions can not begin with quotes. error =
                   * "Expressions can not begin with quotes."; break; // from
                   * switch }
                   */
                  else {
                    if (buffer.length() > 0) {
                  ...
                  error = XSLMessages.createMessage(XSLTErrorResources.ER_ILLEGAL_ATTRIBUTE_VALUE, new Object[] { name,
                          stringedValue });
                  break;
                }
              }
   ...
   * 
   * @param xctxt
   *          Te XPathContext to use to evaluate this.
   * @param context
   ...
   * @throws javax.xml.transform.TransformerException
   */
  public String evaluate(XPathContext xctxt, int context, de.lyca.xml.utils.PrefixResolver nsNode)
          throws javax.xml.transform.TransformerException {
    if (null != m_simpleString)
      return m_simpleString;
    else if (null != m_parts) {
  ...
  /**
   * Test whether the AVT is insensitive to the context in which it is being
   * evaluated. This is intended to facilitate compilation of templates, by
   * allowing simple AVTs to be converted back into strings.
   * 
   * Currently the only case we recognize is simple strings. ADDED 9/5/2000 to
   * support compilation experiment
   * 
