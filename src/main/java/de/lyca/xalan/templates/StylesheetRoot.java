...
public class StylesheetRoot extends StylesheetComposed implements java.io.Serializable, Templates {
  ...
  public StylesheetRoot(ErrorListener errorListener) throws TransformerConfigurationException {
    ...
    try {
      ...
      throw new TransformerConfigurationException(XSLMessages.createMessage(
              XSLTErrorResources.ER_CANNOT_INIT_DEFAULT_TEMPLATES, null), se); // "Can't init default templates!",
                                                                               // se);
    }
  }
   ...
   * this StylesheetRoot. The schema may be needed later for an
  ...
  /**
   * Only instantiate an ExtensionNamespacesManager if one is called for (i.e.,
   * if the stylesheet contains extension functions and/or elements).
   */
   ...
   * TransformerException { System.out.println("StylesheetRoot.runtimeInit()");
   * 
  ...
  /**
   * Recompose the values of all "composed" properties, meaning properties that
   * need to be combined or calculated from the combination of imported and
   * included stylesheets. This method determines the proper import precedence
   * of all imported stylesheets. It then iterates through all of the elements
   * and properties in the proper order and triggers the individual recompose
   * methods.
   * 
  ...
  public void recompose() throws TransformerException {
    ...
    for (int i = 0; i < n; i++) {
      final StylesheetComposed imported = getGlobalImport(i);
    ...
    /*
     * Backing out REE again, as it seems to cause some new failures which need
     * to be investigated. -is
     */
    // This has to be done before the initialization of the compose state,
    // because
    // eleminateRedundentGlobals will add variables to the m_variables vector,
    // which
    // it then copied in the ComposeState constructor.

    // if(true && de.lyca.xalan.processor.TransformerFactoryImpl.m_optimize)
    // {
    ...
    // Need final composition of TemplateList. This adds the wild cards onto the
    // chains.
   ...
   * 
   * @param templ
   *          non-null reference to template element that will have the composed
   *          method called on it, and will have it's children's composed
   *          methods called.
   */
   ...
   * precedence will be at element 0. The one with the lowest import precedence
   * will be at element length - 1.
   * 
   * @serial
   */
   ...
   * @param addToList
   *          <code>true</code> if this template should be added to the import
   *          list
   * @param importList
   ...
   * 
   * @param sheet
   *          The stylesheet which will be located in the global import list.
  ...
  public int getImportNumber(StylesheetComposed sheet) {
    ...
    final int n = getGlobalImportCount();
   ...
   * stylesheet or an included stylesheet, look in the imports. Please note that
   * this returns a reference to the OutputProperties object, not a cloned
   * object, like getOutputProperties does.
   * 
   ...
   * @serial
   */
  private final boolean m_outputMethodSet = false;
  ...
  void recomposeDecimalFormats(DecimalFormatProperties dfp) {
    ...
    if (null == oldDfs) {
      ...
      if (dfp.getName().equals(new QName(""))) {
        ...
        themsg = XSLMessages.createWarning(XSLTErrorResources.WG_XSLDECIMALFORMAT_NAMES_MUST_BE_UNIQUE,
  ...
  /**
   * Given a valid element decimal-format name, return the decimalFormatSymbols
   * with that name.
   * <p>
   * It is an error to declare either the default decimal-format or a
   * decimal-format with a given name more than once (even with different import
   * precedence), unless it is declared every time with the same value for all
   ...
   * @return DecimalFormatSymbols object matching the given name or null if name
   *         is not found.
   */
  ...
  /**
   * A list of all key declarations visible from this stylesheet and all lesser
   * stylesheets.
   * 
   * @serial
   */
   ...
   * @throws TransformerException
   */
  public ElemTemplate getTemplateComposed(XPathContext xctxt, int targetNode, QName mode,
   ...
   * @throws TransformerException
   */
  public ElemTemplate getTemplateComposed(XPathContext xctxt, int targetNode, QName mode, int maxImportLevel,
          int endImportLevel, boolean quietConflictWarnings, DTM dtm) throws TransformerException {
    return m_templateList.getTemplate(xctxt, targetNode, mode, maxImportLevel, endImportLevel, quietConflictWarnings,
            dtm);
  }
  ...
  /**
   * A list of properties that specify how to do space stripping. This uses the
   * same exact mechanism as Templates.
   * 
   * @serial
   */
  ...
  public boolean shouldStripWhiteSpace(XPathContext support, int targetElement) throws TransformerException {
    if (null != m_whiteSpaceInfoList) {
      while (DTM.NULL != targetElement) {
        ...
        final WhiteSpaceInfo info = (WhiteSpaceInfo) m_whiteSpaceInfoList.getTemplate(support, targetElement, null,
                false, dtm);
        if (null != info)
   ...
   * This is initialized in initDefaultRule().
   * 
   * @serial
   * @xsl.usage advanced
   */
   ...
   * This is initialized in initDefaultRule(). We kind of need this because the
   * defaultRule isn't good enough because it doesn't supply a document context.
   * For now, I default the root document element to "HTML". Don't know if this
   * is really a good idea or not. I suspect it is not.
   * 
   * @serial
   * @xsl.usage advanced
   */
  ...
  private void initDefaultRule(ErrorListener errorListener) throws TransformerException {
    ...
    final XPath selectPattern = new XPath(".", this, this, XPath.SELECT, errorListener);
   ...
   * handle arrays that are already sorted, and arrays with duplicate keys. It
   * was lifted from the NodeSorter class but should probably be eliminated and
   * replaced with a call to Collections.sort when we migrate to Java2.<BR>
   * 
   ...
   * time you call this function it will be with the parameters 0, a.length - 1.
   * 
   * @param v
  ...
  private String m_extensionHandlerClass = "de.lyca.xalan.extensions.ExtensionHandlerExsltFunction";
  ...
  public String setExtensionHandlerClass(String handlerClassName) {
    final String oldvalue = m_extensionHandlerClass;
  ...
  class ComposeState {
    ...
    private final ExpandedNameTable m_ent = new ExpandedNameTable();
    ...
    public int getQNameID(QName qname) {
      ...
      return m_ent.getExpandedTypeID(qname.getNamespace(), qname.getLocalName(),
      // The type doesn't matter for our
      // purposes.
              de.lyca.xml.dtm.DTM.ELEMENT_NODE);
    }
     ...
     * in a template element at startElement, so that it can be popped at
     * endElement.
     */
    ...
    void popStackMark() {
      final int mark = m_marks.pop();
      setCurrentStackFrameSize(mark);
    }

    /**
     * Get the Vector of the current params and QNames to be collected within
