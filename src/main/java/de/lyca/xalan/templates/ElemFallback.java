...
public class ElemFallback extends ElemTemplateElement {
  ...
  /**
   * This is the normal call when xsl:fallback is instantiated. In accordance
   * with the XSLT 1.0 Recommendation, chapter 15,
   ...
   * instantiate the content of an xsl:fallback element. When an XSLT
   * transformer performs fallback for an instruction element, if the
   * instruction element has one or more xsl:fallback children, then the content
   * of each of the xsl:fallback children must be instantiated in sequence;
   * otherwise, an error must be signaled. The content of an xsl:fallback
  ...
  public void executeFallback(TransformerImpl transformer) throws TransformerException {
    ...
    final int parentElemType = m_parentNode.getXSLToken();
