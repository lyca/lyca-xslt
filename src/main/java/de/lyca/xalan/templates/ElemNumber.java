...
import de.lyca.xalan.transformer.CountersTable;
import de.lyca.xalan.transformer.DecimalToRoman;
...
import de.lyca.xml.utils.res.LongArrayWrapper;
import de.lyca.xml.utils.res.StringArrayWrapper;
import de.lyca.xml.utils.res.XResourceBundle;
import de.lyca.xpath.NodeSetDTM;
import de.lyca.xpath.XPath;
import de.lyca.xpath.XPathContext;
import de.lyca.xpath.objects.XObject;

// import de.lyca.xalan.dtm.*;
...
public class ElemNumber extends ElemTemplateElement {
  ...
  private class MyPrefixResolver implements PrefixResolver {
    ...
    /**
     * @see PrefixResolver#getNamespaceForPrefix(String, Node) this shouldn't
     *      get called.
     */
    @Override
  ...
  /**
   * Set the "count" attribute. The count attribute is a pattern that specifies
   * what nodes should be counted at those levels. If count attribute is not
   * specified, then it defaults to the pattern that matches any node with the
  ...
  /**
   * Get the "count" attribute. The count attribute is a pattern that specifies
   * what nodes should be counted at those levels. If count attribute is not
   * specified, then it defaults to the pattern that matches any node with the
   * same node type as the current node and, if the current node has an
   * expanded-name, with the same expanded-name as the current node.
  ...
  /**
   * Specifies where to count from. For level="single" or level="multiple": Only
   * ancestors that are searched are those that are descendants of the nearest
   * ancestor that matches the from pattern. For level="any: Only nodes after
  ...
  /**
   * Set the "from" attribute. Specifies where to count from. For level="single"
   * or level="multiple": Only ancestors that are searched are those that are
   * descendants of the nearest ancestor that matches the from pattern. For
   * level="any: Only nodes after the first node before the current node that
   * match the from pattern are considered.
   * 
   * @param v
  ...
  /**
   * Get the "from" attribute. For level="single" or level="multiple": Only
   * ancestors that are searched are those that are descendants of the nearest
   * ancestor that matches the from pattern. For level="any: Only nodes after
   * the first node before the current node that match the from pattern are
   * considered.
   ...
   * empty list. If the from attribute is specified, then the only ancestors
   * that are searched are those that are descendants of the nearest ancestor
   * that matches the from pattern. Preceding siblings has the same meaning here
   * as with the preceding-sibling axis.
   * 
   * When level="multiple", it constructs a list of all ancestors of the current
   * node in document order followed by the element itself; it then selects from
   * the list those nodes that match the count pattern; it then maps each node
   * in the list to one plus the number of preceding siblings of that node that
   * match the count pattern. If the from attribute is specified, then the only
   * ancestors that are searched are those that are descendants of the nearest
   * ancestor that matches the from pattern. Preceding siblings has the same
   ...
   * current node in document order, excluding any namespace and attribute nodes
   * (in other words the union of the members of the preceding and
   * ancestor-or-self axes). If the from attribute is specified, then only nodes
   * after the first node before the current node that match the from pattern
   * are considered.
   * 
   * @serial
   */
  ...
  /**
   * Set the "level" attribute. The level attribute specifies what levels of the
   * source tree should be considered; it has the values single, multiple or
  ...
  /**
   * Get the "level" attribute. The level attribute specifies what levels of the
   * source tree should be considered; it has the values single, multiple or
   * any. The default is single.
   * 
  ...
  /**
   * The value attribute contains an expression. The expression is evaluated and
   * the resulting object is converted to a number as if by a call to the number
   * function.
   * 
   * @serial
   */
  ...
  /**
   * Set the "value" attribute. The value attribute contains an expression. The
   * expression is evaluated and the resulting object is converted to a number
   * as if by a call to the number function.
   * 
   * @param v
  ...
  /**
   * Get the "value" attribute. The value attribute contains an expression. The
   * expression is evaluated and the resulting object is converted to a number
   * as if by a call to the number function.
   * 
  ...
  /**
   * The "format" attribute is used to control conversion of a list of numbers
   * into a string.
   * 
  ...
  /**
   * Set the "format" attribute. The "format" attribute is used to control
   * conversion of a list of numbers into a string.
   * 
   * @see <a href="http://www.w3.org/TR/xslt#convert">convert in XSLT
   *      Specification</a>
   * 
  ...
  /**
   * Get the "format" attribute. The "format" attribute is used to control
   * conversion of a list of numbers into a string.
   * 
   * @see <a href="http://www.w3.org/TR/xslt#convert">convert in XSLT
   *      Specification</a>
   ...
   * which language's alphabet is to be used.
   * 
   * @serial
   */
  ...
  /**
   * Set the "lang" attribute. When numbering with an alphabetic sequence, the
   * lang attribute specifies which language's alphabet is to be used; it has
   * the same range of values as xml:lang [XML]; if no lang value is specified,
   * the language should be determined from the system environment. Implementers
   * should document for which languages they support numbering.
   * 
   * @see <a href="http://www.w3.org/TR/xslt#convert">convert in XSLT
   *      Specification</a>
   * 
  ...
  /**
   * Get the "lang" attribute. When numbering with an alphabetic sequence, the
   * lang attribute specifies which language's alphabet is to be used; it has
   * the same range of values as xml:lang [XML]; if no lang value is specified,
   * the language should be determined from the system environment. Implementers
   * should document for which languages they support numbering.
  ...
  /**
   * Set the "letter-value" attribute. The letter-value attribute disambiguates
   * between numbering sequences that use letters.
   * 
   * @see <a href="http://www.w3.org/TR/xslt#convert">convert in XSLT
   *      Specification</a>
   * 
  ...
  /**
   * Get the "letter-value" attribute. The letter-value attribute disambiguates
   * between numbering sequences that use letters.
   * 
   * @see <a href="http://www.w3.org/TR/xslt#convert">convert in XSLT
   *      Specification</a>
   ...
   * (e.g. thousands) separator in decimal numbering sequences.
   * 
   * @serial
   */
  ...
  /**
   * Set the "grouping-separator" attribute. The grouping-separator attribute
   * gives the separator used as a grouping (e.g. thousands) separator in
   * decimal numbering sequences.
   * 
   * @see <a href="http://www.w3.org/TR/xslt#convert">convert in XSLT
   *      Specification</a>
   * 
  ...
  /**
   * Get the "grouping-separator" attribute. The grouping-separator attribute
   * gives the separator used as a grouping (e.g. thousands) separator in
   * decimal numbering sequences.
   * 
   * @see <a href="http://www.w3.org/TR/xslt#convert">convert in XSLT
  ...
  /**
   * Set the "grouping-size" attribute. The optional grouping-size specifies the
   * size (normally 3) of the grouping.
   * 
   * @see <a href="http://www.w3.org/TR/xslt#convert">convert in XSLT
   *      Specification</a>
   * 
  ...
  /**
   * Get the "grouping-size" attribute. The optional grouping-size specifies the
   * size (normally 3) of the grouping.
   * 
   * @see <a href="http://www.w3.org/TR/xslt#convert">convert in XSLT
   *      Specification</a>
   ...
   * 
   * @see de.lyca.xalan.transformer.DecimalToRoman
   */
  private final static DecimalToRoman m_romanConvertTable[] = { new DecimalToRoman(1000, "M", 900, "CM"),
          new DecimalToRoman(500, "D", 400, "CD"), new DecimalToRoman(100L, "C", 90L, "XC"),
          new DecimalToRoman(50L, "L", 40L, "XL"), new DecimalToRoman(10L, "X", 9L, "IX"),
          new DecimalToRoman(5L, "V", 4L, "IV"), new DecimalToRoman(1L, "I", 1L, "I") };
  ...
  /**
   * Execute an xsl:number instruction. The xsl:number element is used to insert
   * a formatted number into the result tree.
   * 
   * @param transformer
  ...
  public void execute(TransformerImpl transformer) throws TransformerException {
    ...
    final String countString = getCountString(transformer, sourceNode);
  ...
  /**
   * Given a 'from' pattern (ala xsl:number), a match pattern and a context,
   * find the first ancestor that matches the pattern (including the context
   * handed in).
   * 
   * @param xctxt
   *          The XPath runtime state for this.
   * @param fromMatchPattern
   *          The ancestor must match this pattern.
   * @param countMatchPattern
   *          The ancestor must also match this pattern.
   * @param context
   *          The node that "." expresses.
   * @param namespaceContext
   *          The context in which namespaces in the queries are supposed to be
   *          expanded.
   * 
   ...
   */
  int findAncestor(XPathContext xctxt, XPath fromMatchPattern, XPath countMatchPattern, int context,
          ElemNumber namespaceContext) throws javax.xml.transform.TransformerException {
   ...
   *          expanded.
   * 
   * @return the first preceding, ancestor or self node that matches the given
   *         pattern
   * 
   * @throws javax.xml.transform.TransformerException
   */
  private int findPrecedingOrAncestorOrSelf(XPathContext xctxt, XPath fromMatchPattern, XPath countMatchPattern,
          int context, ElemNumber namespaceContext) throws javax.xml.transform.TransformerException {
    final DTM dtm = xctxt.getDTM(context);
    while (DTM.NULL != context) {
      if (null != fromMatchPattern) {
        if (fromMatchPattern.getMatchScore(xctxt, context) != XPath.MATCH_SCORE_NONE) {
    ...
    if (null == countMatchPattern) {
      switch (dtm.getNodeType(contextNode)) {
        case DTM.ELEMENT_NODE:
          ...
          countMatchPattern = new XPath(dtm.getNodeName(contextNode), this, resolver, XPath.MATCH,
        ...
        case DTM.ATTRIBUTE_NODE:
          ...
          // countMatchPattern =
          // m_stylesheet.createMatchPattern("@"+contextNode.getNodeName(),
          // this);
          countMatchPattern = new XPath("@" + dtm.getNodeName(contextNode), this, this, XPath.MATCH,
                  support.getErrorListener());
          break;
        ...
        case DTM.PROCESSING_INSTRUCTION_NODE:
          ...
          // countMatchPattern =
          // m_stylesheet.createMatchPattern("pi("+contextNode.getNodeName()+")",
          // this);
          countMatchPattern = new XPath("pi(" + dtm.getNodeName(contextNode) + ")", this, this, XPath.MATCH,
                  support.getErrorListener());
          break;
        default:
  ...
  /**
   * Given an XML source node, get the count according to the parameters set up
   * by the xsl:number attributes.
   * 
   * @param transformer
   *          non-null reference to the the current transform-time state.
   * @param sourceNode
  ...
  String getCountString(TransformerImpl transformer, int sourceNode) throws TransformerException {
    ...
    if (null != m_valueExpr) {
      final XObject countObj = m_valueExpr.execute(xctxt, sourceNode, this);
      // According to Errata E24
      final double d_count = java.lang.Math.floor(countObj.num() + 0.5);
      ...
      if (Constants.NUMBERLEVEL_ANY == m_level) {
        ...
        final NodeVector ancestors = getMatchingAncestors(xctxt, sourceNode, Constants.NUMBERLEVEL_SINGLE == m_level);
  ...
  public int getPreviousNode(XPathContext xctxt, int pos) throws TransformerException {
    ...
    final XPath countMatchPattern = getCountMatchPattern(xctxt, pos);
    final DTM dtm = xctxt.getDTM(pos);

    if (Constants.NUMBERLEVEL_ANY == m_level) {
      final XPath fromMatchPattern = m_fromMatchPattern;
      ...
      while (DTM.NULL != pos) {
        ...
        if (DTM.NULL == next) {
          ...
          if (DTM.NULL != next
                  && (null != fromMatchPattern && fromMatchPattern.getMatchScore(xctxt, next) != XPath.MATCH_SCORE_NONE || dtm
                          .getNodeType(next) == DTM.DOCUMENT_NODE)) {
        ...
        if (DTM.NULL != pos
                && (null == countMatchPattern || countMatchPattern.getMatchScore(xctxt, pos) != XPath.MATCH_SCORE_NONE)) {
          break;
        }
      }
   ...
   *          href="http://www.w3.org/TR/xslt#dt-current-node">current source
   *          node</a>.
   * 
  ...
  public int getTargetNode(XPathContext xctxt, int sourceNode) throws TransformerException {
    ...
    int target = DTM.NULL;
    final XPath countMatchPattern = getCountMatchPattern(xctxt, sourceNode);
   ...
   * @param stopAtFirstFound
   *          Flag indicating to stop after the first node is found (difference
   *          between level = single or multiple)
  ...
  NodeVector getMatchingAncestors(XPathContext xctxt, int node, boolean stopAtFirstFound)
    ...
    final XPath countMatchPattern = getCountMatchPattern(xctxt, node);
   ...
   *          The node that "." expresses.
   * 
   * @return The locale to use. May be specified by "lang" attribute, but if
   *         not, use default locale on the system.
   * 
   * @throws TransformerException
   */
  Locale getLocale(TransformerImpl transformer, int contextNode) throws TransformerException {
    ...
    if (null != m_lang_avt) {
      final XPathContext xctxt = transformer.getXPathContext();
      final String langValue = m_lang_avt.evaluate(xctxt, contextNode, this);
  ...
  private DecimalFormat getNumberFormatter(TransformerImpl transformer, int contextNode) throws TransformerException {
    ...
    final Locale locale = (Locale) getLocale(transformer, contextNode).clone();
    ...
    final String digitGroupSepValue = null != m_groupingSeparator_avt ? m_groupingSeparator_avt.evaluate(
    ...
    final String nDigitsPerGroupValue = null != m_groupingSize_avt ? m_groupingSize_avt.evaluate(
            transformer.getXPathContext(), contextNode, this) : null;
   ...
   *          non-null reference to the the current transform-time state.
   * @param list
   *          Array of one or more long integer numbers.
   * @param contextNode
   *          The node that "." expresses.
   * @return String that represents list according to %conversion-atts;
   *         attributes. TODO: Optimize formatNumberList so that it caches the
   *         last count and reuses that info for the next count.
   * 
   * @throws TransformerException
   */
  String formatNumberList(TransformerImpl transformer, long[] list, int contextNode) throws TransformerException {
    ...
    try {
      ...
      String formatValue = null != m_format_avt ? m_format_avt.evaluate(transformer.getXPathContext(), contextNode,
              this) : null;
   ...
   * @throws javax.xml.transform.TransformerException
   */
  private void getFormattedNumber(TransformerImpl transformer, int contextNode, char numberType, int numberWidth,
          long listElement, FastStringBuffer formattedNumber) throws javax.xml.transform.TransformerException {

    final String letterVal = m_lettervalue_avt != null ? m_lettervalue_avt.evaluate(transformer.getXPathContext(),
    ...
    switch (numberType) {
      case 'A':
        if (null == m_alphaCountTable) {
          thisBundle = XResourceBundle.loadResourceBundle(de.lyca.xml.utils.res.XResourceBundle.LANG_BUNDLE_NAME,
                  getLocale(transformer, contextNode));
          m_alphaCountTable = (CharArrayWrapper) thisBundle
                  .getObject(de.lyca.xml.utils.res.XResourceBundle.LANG_ALPHABET);
        }
      ...
      case 0x3042: {
        ...
        if (letterVal != null && letterVal.equals(Constants.ATTRVAL_TRADITIONAL)) {
          ...
          formattedNumber.append(int2singlealphaCount(listElement,
                  (CharArrayWrapper) thisBundle.getObject(de.lyca.xml.utils.res.XResourceBundle.LANG_ALPHABET)));
        }
      ...
      default: // "1"
        final DecimalFormat formatter = getNumberFormatter(transformer, contextNode);
        final String padString = formatter == null ? String.valueOf(0) : formatter.format(0);
        final String numString = formatter == null ? String.valueOf(listElement) : formatter.format(listElement);
        final int nPadding = numberWidth - numString.length();
  ...
  /**
   * Convert a long integer into alphabetic counting, in other words count using
   * the sequence A B C ... Z.
   * 
   * @param val
  ...
  protected String int2singlealphaCount(long val, CharArrayWrapper table) {
    ...
    if (val > radix)
      ...
      return new Character(table.getChar((int) val - 1)).toString(); // index
                                                                     // into
                                                                     // table is
                                                                     // off one,
                                                                     // starts
  ...
  /**
   * Convert a long integer into alphabetic counting, in other words count using
   * the sequence A B C ... Z AA AB AC.... etc.
   * 
   * @param val
   *          Value to convert -- must be greater than zero.
   * @param table
   ...
   * @param stringBuf
   *          Buffer where to save the string representing alpha count of
   *          number.
   * 
   * @see TransformerImpl#DecimalToRoman
   * 
  ...
  protected void int2alphaCount(long val, CharArrayWrapper aTable, FastStringBuffer stringBuf) {
    ...
    // String orientation =
    // thisBundle.getString(de.lyca.xml.utils.res.XResourceBundle.LANG_ORIENTATION);
    ...
    do {
      ...
      // most of the correction calculation is explained above, the reason for
      // the
  ...
  protected String tradAlphaCount(long val, XResourceBundle thisBundle) {
    ...
    final IntArrayWrapper groups = (IntArrayWrapper) thisBundle
            .getObject(de.lyca.xml.utils.res.XResourceBundle.LANG_NUMBERGROUPS);

    // array of tables of hundreds, tens, digits...
    final StringArrayWrapper tables = (StringArrayWrapper) thisBundle
            .getObject(de.lyca.xml.utils.res.XResourceBundle.LANG_NUM_TABLES);

    // some languages have additive alphabetical notation,
    // some multiplicative-additive, etc... Handle them differently.
    final String numbering = thisBundle.getString(de.lyca.xml.utils.res.XResourceBundle.LANG_NUMBERING);

    // do multiplicative part first
    if (numbering.equals(de.lyca.xml.utils.res.XResourceBundle.LANG_MULT_ADD)) {
      final String mult_order = thisBundle.getString(de.lyca.xml.utils.res.XResourceBundle.MULT_ORDER);
      final LongArrayWrapper multiplier = (LongArrayWrapper) thisBundle
              .getObject(de.lyca.xml.utils.res.XResourceBundle.LANG_MULTIPLIER);
        ...
        if (val < multiplier.getLong(i)) {
          ...
          while (k < groups.getLength()) {
            ...
            if (mult / groups.getInt(k) <= 0) {
              ...
              final char multiplierChar = ((CharArrayWrapper) thisBundle
                      .getObject(de.lyca.xml.utils.res.XResourceBundle.LANG_MULTIPLIER_CHAR)).getChar(i);

              // put out the next character of output
              if (lookupIndex < table.length) {
                if (mult_order.equals(de.lyca.xml.utils.res.XResourceBundle.MULT_PRECEDES)) {
   ...
   *          Value to convert.
   * @param prefixesAreOK
   *          true_ to enable prefix notation (e.g. 4 = "IV"), false_ to disable
   *          prefix notation (e.g. 4 = "IIII").
  ...
  protected String long2roman(long val, boolean prefixesAreOK) {
    ...
    if (val <= 3999L) {
      final StringBuffer romanBuffer = new StringBuffer();
      do {
