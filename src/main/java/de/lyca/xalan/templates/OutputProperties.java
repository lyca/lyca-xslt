...
import de.lyca.xml.serializer.OutputPropertyUtils;
import de.lyca.xml.utils.FastStringBuffer;
import de.lyca.xml.utils.QName;

/**
 * This class provides information from xsl:output elements. It is mainly a
 * wrapper for {@link java.util.Properties}, but can not extend that class
 * because it must be part of the
 * {@link de.lyca.xalan.templates.ElemTemplateElement} heararchy.
 * <p>
 * An OutputProperties list can contain another OutputProperties list as its
 * "defaults"; this second property list is searched if the property key is not
 * found in the original property list.
 * </p>
 * 
 * @see <a href="http://www.w3.org/TR/xslt#dtd">XSLT DTD</a>
 * @see <a href="http://www.w3.org/TR/xslt#output">xsl:output in XSLT
 *      Specification</a>
 * 
 */
public class OutputProperties extends ElemTemplateElement implements Cloneable {
  ...
  public OutputProperties() {
    this(de.lyca.xml.serializer.Method.XML);
  }
  ...
  /**
   * Creates an empty OutputProperties with the defaults specified by a property
   * file. The method argument is used to construct a string of the form
   * output_[method].properties (for instance, output_html.properties). The
   * output_xml.properties file is always used as the base.
   * <p>
   * At the moment, anything other than 'text', 'xml', and 'html', will use the
  ...
  /**
   * Searches for the property with the specified key in the property list. If
   ...
   * @param key
   *          the property key.
   * @return the value in this property list as a int value, or false if null or
   *         not a number.
   */
  ...
  /**
   * Set an output property with a QName value. The QName will be turned into a
   * string with the namespace in curly brackets.
   * 
   * @param key
   *          the key to be placed into the property list.
   * @param value
  ...
  public void setMethodDefaults(String method) {
    final String defaultMethod = m_properties.getProperty(OutputKeys.METHOD);
    ...
    //
      ...
      final Properties savedProps = m_properties;
   ...
   * @param key
   *          the property key.
   * @return the value in this property list as a QName value, or false if null
   *         or not "yes".
   */
  ...
  /**
   * Copy the keys and values from the source to this object. This will not copy
   * the default values. This is meant to be used by going from a higher
   * precedence object to a lower precedence object, so that if a key already
   * exists, this method will not reset it.
   * 
   * @param src
   ...
   * @param src
   *          non-null reference to the source properties.
   * @param shouldResetDefaults
   *          true if the defaults should be reset based on the method property.
   */
    ...
    while (keys.hasMoreElements()) {
      ...
      if (!isLegalPropertyKey(key))
        throw new IllegalArgumentException(XSLMessages.createMessage(
                XSLTErrorResources.ER_OUTPUT_PROPERTY_NOT_RECOGNIZED, new Object[] { key })); // "output property not recognized: "

      final Object oldValue = m_properties.get(key);
      if (null == oldValue) {
        final String val = (String) src.get(key);
  ...
  public static boolean isLegalPropertyKey(String key) {
    ...
    return key.equals(OutputKeys.CDATA_SECTION_ELEMENTS) || key.equals(OutputKeys.DOCTYPE_PUBLIC)
            || key.equals(OutputKeys.DOCTYPE_SYSTEM) || key.equals(OutputKeys.ENCODING)
            || key.equals(OutputKeys.INDENT) || key.equals(OutputKeys.MEDIA_TYPE) || key.equals(OutputKeys.METHOD)
            || key.equals(OutputKeys.OMIT_XML_DECLARATION) || key.equals(OutputKeys.STANDALONE)
            || key.equals(OutputKeys.VERSION) || key.length() > 0 && key.charAt(0) == '{' && key.lastIndexOf('{') == 0
            && key.indexOf('}') > 0 && key.lastIndexOf('}') == key.indexOf('}');
  }
   ...
   * 
   * @deprecated Use de.lyca.xml.serializer.OuputPropertiesFactory.
   *             getDefaultMethodProperties directly.
   */
  @Deprecated
  static public Properties getDefaultMethodProperties(String method) {
    return de.lyca.xml.serializer.OutputPropertiesFactory.getDefaultMethodProperties(method);
  }
}
