...
import de.lyca.xalan.extensions.ExtensionHandler;
...
public class ElemExtensionCall extends ElemLiteralResult {
   ...
   * 
   * @param stylesheet
   *          Stylesheet root associated with this extension element
   * @param namespace
  ...
  public void execute(TransformerImpl transformer) throws TransformerException {
    if (transformer.getStylesheet().isSecureProcessing())
      throw new TransformerException(XSLMessages.createMessage(
              XSLTErrorResources.ER_EXTENSION_ELEMENT_NOT_ALLOWED_IN_SECURE_PROCESSING, new Object[] { getRawName() }));
    ...
    try {
      ...
      final ExtensionHandler nsh = etable.get(m_extns);

      if (null == nsh) {
        if (hasFallbackChildren()) {
          executeFallbacks(transformer);
        } else {
          final TransformerException te = new TransformerException(XSLMessages.createMessage(
      ...
      try {
        ...
        if (hasFallbackChildren()) {
          ...
          if (e instanceof TransformerException) {
            final TransformerException te = (TransformerException) e;
  ...
  /**
   * Return the value of the attribute interpreted as an Attribute Value
   * Template (in other words, you can use curly expressions such as
   * href="http://{website}".
   * 
   * @param rawName
   ...
   * @throws TransformerException
   */
  public String getAttribute(String rawName, org.w3c.dom.Node sourceNode, TransformerImpl transformer)
          throws TransformerException {
