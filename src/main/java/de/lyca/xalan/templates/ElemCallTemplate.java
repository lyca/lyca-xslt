...
public class ElemCallTemplate extends ElemForEach {
  ...
  /**
   * An xsl:call-template element invokes a template by name; it has a required
   * name attribute that identifies the template to be invoked.
   * 
   * @serial
   */
  ...
  /**
   * Set the "name" attribute. An xsl:call-template element invokes a template
   * by name; it has a required name attribute that identifies the template to
   * be invoked.
   * 
   * @param name
  ...
  /**
   * Get the "name" attribute. An xsl:call-template element invokes a template
   * by name; it has a required name attribute that identifies the template to
   * be invoked.
   * 
  ...
  public void compose(StylesheetRoot sroot) throws TransformerException {
    ...
    for (int i = 0; i < length; i++) {
      final ElemWithParam ewp = getParamElem(i);
      ewp.compose(sroot);
    }
    ...
    if (null != m_templateName && null == m_template) {
      ...
      if (null == m_template) {
        final String themsg = XSLMessages.createMessage(XSLTErrorResources.ER_ELEMTEMPLATEELEM_ERR,
  ...
  public void endCompose(StylesheetRoot sroot) throws TransformerException {
    final int length = getParamElemCount();
    for (int i = 0; i < length; i++) {
      final ElemWithParam ewp = getParamElem(i);
  ...
  public void execute(TransformerImpl transformer) throws TransformerException {
    ...
    if (null != m_template) {
      ...
      final int nextFrame = vars.link(m_template.m_frameSize);
      ...
      if (m_template.m_inArgsSize > 0) {
        ...
        if (null != m_paramElems) {
          ...
          final int size = m_paramElems.length;

          for (int i = 0; i < size; i++) {
            final ElemWithParam ewp = m_paramElems[i];
            if (ewp.m_index >= 0) {
              if (transformer.getDebug()) {
  ...
  public void setParamElem(ElemWithParam ParamElem) {
    if (null == m_paramElems) {
      ...
      // I think it's OK.
      final int length = m_paramElems.length;
