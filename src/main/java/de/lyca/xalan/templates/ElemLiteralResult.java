...
public class ElemLiteralResult extends ElemUse {
  ...
  /**
   * Tells if this element represents a root element that is also the stylesheet
   * element. TODO: This should be a derived class.
   * 
   * @serial
   */
  ...
  /**
   * The created element node will have the attribute nodes that were present on
   * the element node in the stylesheet tree, other than attributes with names
  ...
  public String getAttributeNS(String namespaceURI, String localName) {
    ...
    final AVT avt = getLiteralResultAttributeNS(namespaceURI, localName);
    ...
    if (null != m_avts) {
      ...
      for (int i = nAttrs - 1; i >= 0; i--) {
        ...
        if (namespace != null && namespace.length() != 0 && (namespace + ":" + avt.getName()).equals(name)
                || (namespace == null || namespace.length() == 0) && avt.getRawName().equals(name))
          return avt;
      } // end for
    }
   ...
   * 
   * @param namespaceURI
   *          :localName or localName if the namespaceURI is null of the
   *          attribute to get
   * 
   * @return The Attr value as a string, or the empty string if that attribute
   *         does not have a specified or default value
  ...
  public String getAttribute(String rawName) {
    ...
    final AVT avt = getLiteralResultAttribute(rawName);
   ...
   * 
   * @param prefix
   *          non-null reference to prefix that might be excluded.(not currently
   *          used)
   * @param uri
  ...
  public void resolvePrefixTables() throws TransformerException {
    ...
    final StylesheetRoot stylesheet = getStylesheetRoot();

    if (null != m_namespace && m_namespace.length() > 0) {
      final NamespaceAlias nsa = stylesheet.getNamespaceAliasComposed(m_namespace);

      if (null != nsa) {
        m_namespace = nsa.getResultNamespace();

        // String resultPrefix = nsa.getResultPrefix();
        final String resultPrefix = nsa.getStylesheetPrefix(); // As per xsl WG,
                                                               // Mike Kay
    ...
    if (null != m_avts) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (null != ns && ns.length() > 0) {
          final NamespaceAlias nsa = stylesheet.getNamespaceAliasComposed(m_namespace); // %REVIEW%
                                                                                        // ns?

          if (null != nsa) {
            final String namespace = nsa.getResultNamespace();

            // String resultPrefix = nsa.getResultPrefix();
            final String resultPrefix = nsa.getStylesheetPrefix(); // As per XSL
                                                                   // WG
  ...
  boolean needToCheckExclude() {
    if (null == m_excludeResultPrefixes && null == getPrefixTable() && m_ExtensionElementURIs == null // JJK
                                                                                                      // Bugzilla
                                                                                                      // 1133
    )
      return false;
  ...
  /**
   * Set the namespace URI of the result element to be created. Note that after
   * resolvePrefixTables has been called, this will return the aliased result
   * namespace, not the original stylesheet namespace.
   * 
   * @param ns
   ...
   * 
   * @return The qualified name (with prefix), or the empty string if qualified
   *         names are not available.
   */
  ...
  public String getPrefix() {
    final int len = m_rawName.length() - m_localName.length() - 1;
  ...
  public class LiteralElementAttributes implements NamedNodeMap {
     ...
     * 
     * @return The number of nodes in this map. The range of valid child node
     ...
     * </code> is greater than or equal to the number of nodes in this map, this
    ...
    public Node removeNamedItem(String name) throws DOMException {
      throwDOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR, XSLTErrorResources.NO_MODIFICATION_ALLOWED_ERR);
      return null;
    }
  ...
  public class Attribute implements Attr {
    ...
    public String getNamespaceURI() {
      final String uri = m_attribute.getURI();
    ...
    public String getPrefix() {
      final String uri = m_attribute.getURI();
      final String rawName = m_attribute.getRawName();
  ...
  public void execute(TransformerImpl transformer) throws TransformerException {
    ...
    /*
     * If we make it to here we have done a successful startElement() we will do
     * an endElement() call for balance, no matter what happens in the middle.
     */
    ...
    try {
      ...
      // thrown in finally to prevent original exception consumed by subsequent
      // exceptions
      tException = te;
    } catch (final SAXException se) {
    ...
    try {
      ...
      /*
       * we did call endElement(). If thee was an exception in the middle throw
       * that one, otherwise if there was an exception from endElement() throw
       * that one.
       */
      if (tException != null)
        throw tException;
