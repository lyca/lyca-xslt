...
public class ElemApplyTemplates extends ElemCallTemplate {
  ...
  public void transformSelectedNodes(TransformerImpl transformer) throws TransformerException {
    ...
    final int nParams = getParamElemCount();
    final int thisframe = vars.getStackFrame();
    final StackGuard guard = transformer.getStackGuard();
    final boolean check = guard.getRecursionLimit() > -1 ? true : false;
    ...
    try {
      ...
      if (transformer.getDebug()) {
        transformer.getTraceManager().fireSelectedEvent(sourceNode, this, "select", new XPath(m_selectExpression),
      ...
      if (nParams > 0) {
        ...
        for (int i = 0; i < nParams; i++) {
          ...
          final XObject obj = ewp.getValue(transformer, sourceNode);
          if (transformer.getDebug()) {
      ...
      while (DTM.NULL != (child = sourceNodes.nextNode())) {
        ...
        if (template.m_frameSize > 0) {
          ...
          currentFrameBottom = vars.getStackFrame(); // See comment with unlink,
                                                     // below
          ...
          if (/* nParams > 0 && */template.m_inArgsSize > 0) {
            int paramIndex = 0;
            for (ElemTemplateElement elem = template.getFirstChildElem(); null != elem; elem = elem
                    .getNextSiblingElem()) {
              if (Constants.ELEMNAME_PARAMVARIABLE == elem.getXSLToken()) {
                final ElemParam ep = (ElemParam) elem;

                int i;
                for (i = 0; i < nParams; i++) {
                  final ElemWithParam ewp = m_paramElems[i];
                  if (ewp.m_qnameID == ep.m_qnameID) {
                    final XObject obj = vars.getLocalVariable(i, argsFrame);
