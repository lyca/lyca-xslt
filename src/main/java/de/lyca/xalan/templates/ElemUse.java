...
/**
 * Implement xsl:use. This acts as a superclass for ElemCopy, ElemAttributeSet,
 * ElemElement, and ElemLiteralResult, on order to implement shared behavior the
...
public class ElemUse extends ElemTemplateElement {
  ...
  /**
   * Set the "use-attribute-sets" attribute. Attribute sets are used by
   * specifying a use-attribute-sets attribute on xsl:element, xsl:copy (see
   * [7.5 Copying]) or xsl:attribute-set elements. The value of the
   * use-attribute-sets attribute is a whitespace-separated list of names of
  ...
  /**
   * Get the "use-attribute-sets" attribute. Attribute sets are used by
   * specifying a use-attribute-sets attribute on xsl:element, xsl:copy (see
   * [7.5 Copying]) or xsl:attribute-set elements, or a xsl:use-attribute-sets
   * attribute on Literal Result Elements. The value of the use-attribute-sets
   * attribute is a whitespace-separated list of names of attribute sets. Each
   * name is specified as a QName, which is expanded as described in [2.4
   * Qualified Names].
   * 
  ...
  private void applyAttrSets(TransformerImpl transformer, StylesheetRoot stylesheet, QName attributeSetsNames[])
    ...
    if (null != attributeSetsNames) {
      final int nNames = attributeSetsNames.length;
      ...
      for (int i = 0; i < nNames; i++) {
        ...
        if (null != attrSets) {
          ...
          throw new TransformerException(XSLMessages.createMessage(XSLTErrorResources.ER_NO_ATTRIB_SET,
   ...
   * xsl:attribute elements for each of the attributes in each of the named
   * attribute sets to the beginning of the content of the element with the
   * use-attribute-sets attribute, in the same order in which the names of the
   * attribute sets are specified in the use-attribute-sets attribute. It is an
   * error if use of use-attribute-sets attributes on xsl:attribute-set elements
   * causes an attribute set to directly or indirectly use itself.
   * 
   * @param transformer
   *          non-null reference to the the current transform-time state.
   * 
