 ...
 * accessors for all properties that represent XSL attributes or elements. These
 * properties have setter method names accessed generically by the processor,
 * and so these names must be fixed according to the system defined in the <a
 * href="XSLTAttributeDef#getSetterMethodName">getSetterMethodName</a> function.
 * </p>
 * <p>
 * 
 * <pre>
                                                                                     ...
                                                                                     */
{
  ...
  /**
   * Tell if this can be cast to a StylesheetComposed, meaning, you can ask
   * questions from getXXXComposed functions.
   * 
   ...
   * 
   * @param v
   *          The value to be set for the "extension-element-prefixes" property:
   *          a vector of extension element URIs.
   */
  ...
  /**
   * Set the "exclude-result-prefixes" property. The designation of a namespace
   * as an excluded namespace is effective within the subtree of the stylesheet
   * rooted at the element bearing the exclude-result-prefixes or
  ...
  /**
   * Get an "exclude-result-prefix" property. The designation of a namespace as
   * an excluded namespace is effective within the subtree of the stylesheet
   * rooted at the element bearing the exclude-result-prefixes or
   * xsl:exclude-result-prefixes attribute; a subtree rooted at an
   * xsl:stylesheet element does not include any stylesheets imported or
   * included by children of that xsl:stylesheet element.
  ...
  /**
   * Get whether or not the passed prefix is contained flagged by the
   * "exclude-result-prefixes" property.
   * 
   * @see <a
   *      href="http://www.w3.org/TR/xslt#literal-result-element">literal-result-element
   *      in XSLT Specification</a>
  ...
  public void setVersion(String v) {
    m_Version = v;
    m_isCompatibleMode = Double.valueOf(v).doubleValue() > de.lyca.xml.utils.Constants.XSLTVERSUPPORTED;
  }
   ...
   * @return null if not found, otherwise a DecimalFormatProperties object, from
  ...
  /**
   * The "xsl:strip-space" properties, A lookup table of all space stripping
   * elements.
   * 
   * @serial
   */
  ...
  /**
   * The "xsl:preserve-space" property, A lookup table of all space preserving
   * elements.
   * 
   * @serial
   */
   ...
   * 
   * @param v
   *          non-null reference to the OutputProperties object to be added to
   *          the collection.
   */
   ...
   *      Specification</a>
   * 
   * @return The number of OutputProperties objects contained in this
   *         stylesheet.
   */
  ...
  public ElemParam getParam(QName qname) {
    ...
    if (null != m_topLevelVariables) {
      ...
      for (int i = 0; i < n; i++) {
        final ElemVariable var = getVariableOrParam(i);
        if (var.getXSLToken() == Constants.ELEMNAME_PARAMVARIABLE && var.getName().equals(qname))
  ...
  /**
   * The root of the stylesheet, where all the tables common to all stylesheets
   * are kept.
   * 
   * @serial
   */
  ...
  /**
   * Get the root of the stylesheet, where all the tables common to all
  ...
  /**
   * Set the root of the stylesheet, where all the tables common to all
   * stylesheets are kept.
   * 
   * @param v
  ...
  /**
   * The parent of the stylesheet. This will be null if this is the root
   * stylesheet.
   * 
   * @serial
   */
  ...
  /**
   * Get the parent of the stylesheet. This will be null if this is the root
   * stylesheet.
   * 
  ...
  /**
   * Set the parent of the stylesheet. This should be null if this is the root
   * stylesheet.
   * 
   * @param v
  ...
  /**
   * Get the owning aggregated stylesheet, or this stylesheet if it is
   * aggregated.
   * 
  ...
  /**
   * Replace an "xsl:template" property. This is a hook for
   * CompilingStylesheetHandler, to allow us to access a template, compile it,
   * instantiate it, and replace the original with the compiled instance. ADDED
   * 9/5/2000 to support compilation experiment
   * 
   * @param v
   *          Compiled template to replace with
   * @param i
  ...
  protected void callChildVisitors(XSLTVisitor visitor, boolean callAttrs) {
    ...
    for (int j = 0; j < s; j++) {
      final ElemAttributeSet attrSet = getAttributeSet(j);
    ...
    for (int j = 0; j < s; j++) {
      try {
        final ElemTemplate template = getTemplate(j);
    ...
    for (int j = 0; j < s; j++) {
      final ElemVariable var = getVariableOrParam(j);
