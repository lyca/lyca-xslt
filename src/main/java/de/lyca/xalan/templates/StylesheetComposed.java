 ...
 * It has methods on it that return "composed" properties, which mean that:
 * <ol>
 * <li>Properties that are aggregates, like OutputProperties, will be composed
 * of properties declared in this stylsheet and all included stylesheets.</li>
 * <li>Properties that aren't found, will be searched for first in the includes,
 * and, if none are located, will be searched for in the imports.</li>
 * <li>Properties in that are not atomic on a stylesheet will have the form
 * getXXXComposed. Some properties, like version and id, are not inherited, and
 * so won't have getXXXComposed methods.</li>
 * </ol>
 * <p>
 * In some cases getXXXComposed methods may calculate the composed values
 * dynamically, while in other cases they may store the composed values.
 * </p>
 */
public class StylesheetComposed extends Stylesheet {
   ...
   * recomposableElements Vector that was passed in as the first parameter. All
   * elements added to the recomposableElements vector should extend
   * ElemTemplateElement.
   * 
   * @param recomposableElements
  ...
  public void recompose(List<ElemTemplateElement> recomposableElements) throws TransformerException {
    ...
    final int n = getIncludeCountComposed();

    for (int i = -1; i < n; i++) {
      final Stylesheet included = getIncludeComposed(i);
  ...
  /**
   * The precedence of this stylesheet in the global import list. The lowest
   * precedence stylesheet is 0. A higher number has a higher precedence.
   * 
   * @serial
   */
   ...
   * The lowest precedence stylesheet is 0. A higher number has a higher
   * precedence.
   */
  void recomposeImports() {
    ...
    final StylesheetRoot root = getStylesheetRoot();
  ...
  /**
   * Get the precedence of this stylesheet in the global import list. The lowest
   * precedence is 0. A higher number has a higher precedence.
   * 
   * @see <a href="http://www.w3.org/TR/xslt#import">import in XSLT
   *      Specification</a>
   * 
   ...
   * of all stylesheets included by this stylesheet to any depth.
   * 
   * @param including
   *          Stylesheet to recompose
   */
  ...
  /**
   * For compilation support, we need the option of overwriting (rather than
   * appending to) previous composition. We could phase out the old API in favor
   * of this one, but I'm holding off until we've made up our minds about
   * compilation. ADDED 9/5/2000 to support compilation experiment. NOTE: GLP
   * 29-Nov-00 I've left this method in so that CompilingStylesheetHandler will
   * compile. However, I'm not sure why it's needed or what it does and I've
   * commented out the body.
   * 
   * @see <a
   *      href="http://www.w3.org/TR/xslt#section-Defining-Template-Rules">section-Defining-Template-Rules
   *      in XSLT Specification</a>
   * @param flushFirst
   *          Flag indicating the option of overwriting (rather than appending
   *          to) previous composition.
   * 
   * @throws TransformerException
   */
  public void recomposeTemplates(boolean flushFirst) throws TransformerException {
    /***************************************
     * KEEP METHOD IN FOR COMPILATION if (flushFirst) m_templateList = new
     * TemplateList(this);
     * 
     * recomposeTemplates();
     *****************************************/
