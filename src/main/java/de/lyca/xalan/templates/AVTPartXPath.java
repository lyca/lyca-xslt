...
public class AVTPartXPath extends AVTPart {
   ...
   * @param nsNode
   *          An object which can be used to determine the Namespace Name (URI)
   *          for any Namespace prefix used in the XPath. Usually this is based
   *          on the context where the XPath was specified, such as a node
   *          within a Stylesheet.
   ...
   *          invocation and this thread. Maintains SAX output state, variables,
   ...
   */
  public AVTPartXPath(String val, de.lyca.xml.utils.PrefixResolver nsNode, XPathParser xpathProcessor,
          XPathFactory factory, XPathContext liaison) throws javax.xml.transform.TransformerException {
   ...
   * @param xctxt
   *          An XPathContext object, providing infomation specific to this
   *          invocation and this thread. Maintains SAX state, variables, error
   *          handler and so on, so the transformation/XPath object itself can
   *          be simultaneously invoked from multiple threads.
   * @param buf
   ...
   */
  @Override
  public void evaluate(XPathContext xctxt, FastStringBuffer buf, int context, de.lyca.xml.utils.PrefixResolver nsNode)
          throws javax.xml.transform.TransformerException {

    final XObject xobj = m_xpath.execute(xctxt, context, nsNode);
