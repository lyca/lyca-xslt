...
/**
 * Holds the attribute declarations for the xsl:keys element. A stylesheet
 * declares a set of keys for each document using the xsl:key element. When this
 * set of keys contains a member with node x, name y and value z, we say that
 * node x has a key with name y and value z.
 * 
...
public class KeyDeclaration extends ElemTemplateElement {
  ...
  /**
   * Constructs a new element representing the xsl:key. The parameters are
   * needed to prioritize this key element as part of the recomposing process.
   * For this element, they are not automatically created because the element is
   * never added on to the stylesheet parent.
   */
  ...
  /**
   * Set the "name" attribute. The name attribute specifies the name of the key.
  ...
  /**
   * Get the "name" attribute. The name attribute specifies the name of the key.
   * The value of the name attribute is a QName, which is expanded as described
   * in [2.4 Qualified Names].
   * 
  ...
  /**
   * Set the "match" attribute. The match attribute is a Pattern; an xsl:key
   * element gives information about the keys of any node that matches the
  ...
  /**
   * Get the "match" attribute. The match attribute is a Pattern; an xsl:key
   * element gives information about the keys of any node that matches the
   * pattern specified in the match attribute.
   * 
   * @see <a href="http://www.w3.org/TR/xslt#patterns">patterns in XSLT
  ...
  /**
   * Set the "use" attribute. The use attribute is an expression specifying the
  ...
  /**
   * Get the "use" attribute. The use attribute is an expression specifying the
   * values of the key; the expression is evaluated once for each node that
   * matches the pattern.
   * 
