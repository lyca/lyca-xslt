...
public class ElemSort extends ElemTemplateElement {
  ...
  /**
   * Set the "select" attribute. xsl:sort has a select attribute whose value is
   * an expression. For each node to be processed, the expression is evaluated
   * with that node as the current node and with the complete list of nodes
   * being processed in unsorted order as the current node list. The resulting
   * object is converted to a string as if by a call to the string function;
   * this string is used as the sort key for that node. The default value of the
   * select attribute is ., which will cause the string-value of the current
  ...
  /**
   * Get the "select" attribute. xsl:sort has a select attribute whose value is
   * an expression. For each node to be processed, the expression is evaluated
   * with that node as the current node and with the complete list of nodes
   * being processed in unsorted order as the current node list. The resulting
   * object is converted to a string as if by a call to the string function;
  ...
  /**
   * Set the "lang" attribute. lang specifies the language of the sort keys; it
   * has the same range of values as xml:lang [XML]; if no lang value is
  ...
  /**
   * Get the "lang" attribute. lang specifies the language of the sort keys; it
   * has the same range of values as xml:lang [XML]; if no lang value is
   * specified, the language should be determined from the system environment.
   * 
  ...
  /**
   * Set the "data-type" attribute. <code>data-type</code> specifies the data
   * type of the strings; the following values are allowed:
   * <ul>
   * <li>
   * <code>text</code> specifies that the sort keys should be sorted
   * lexicographically in the culturally correct manner for the language
   * specified by <code>lang</code>.</li>
   * <li>
   * <code>number</code> specifies that the sort keys should be converted to
   ...
   * the <code>lang</code> attribute is ignored.</li>
   * <li>
   ...
   * expanded-name identifies the data-type; the behavior in this case is not
   * specified by this document.</li>
   * </ul>
   * <p>
   * The default value is <code>text</code>.
   * </p>
   * <blockquote> <b>NOTE: </b>The XSL Working Group plans that future versions
   * of XSLT will leverage XML Schemas to define further values for this
  ...
  /**
   * Get the "data-type" attribute. <code>data-type</code> specifies the data
   * type of the strings; the following values are allowed:
   * <ul>
   * <li>
   * <code>text</code> specifies that the sort keys should be sorted
  ...
  /**
   * Set the "order" attribute. order specifies whether the strings should be
   * sorted in ascending or descending order; ascending specifies ascending
  ...
  /**
   * Get the "order" attribute. order specifies whether the strings should be
   * sorted in ascending or descending order; ascending specifies ascending
   * order; descending specifies descending order; the default is ascending.
   * 
  ...
  /**
   * case-order has the value upper-first or lower-first. The default value is
   * language dependent.
   * 
   * @serial
   */
  ...
  /**
   * Set the "case-order" attribute. case-order has the value upper-first or
   * lower-first; this applies when data-type="text", and specifies that
   * upper-case letters should sort before lower-case letters or vice-versa
   * respectively. For example, if lang="en", then A a B b are sorted with
  ...
  /**
   * Get the "case-order" attribute. case-order has the value upper-first or
   * lower-first; this applies when data-type="text", and specifies that
   * upper-case letters should sort before lower-case letters or vice-versa
   * respectively. For example, if lang="en", then A a B b are sorted with
   * case-order="upper-first" and a A b B are sorted with
