 ...
 * 
 * @see <a
 *      href="http://www.w3.org/TR/xslt#creating-attributes">creating-attributes
...
public class ElemAttribute extends ElemElement {
  ...
  // return;
  //
  // // warn(templateChild, sourceNode,
  // "Trying to add attribute after element child has been added, ignoring...");
  // }
   ...
   * @param nodeNamespace
   *          The namespace of the node, which may be null.
   * @param transformer
   *          non-null reference to the the current transform-time state.
   * @param sourceNode
  ...
  /**
   * Add a child to the child list. <!ELEMENT xsl:attribute %char-template;>
   * <!ATTLIST xsl:attribute name %avt; #REQUIRED namespace %avt; #IMPLIED
   * %space-att; >
   * 
   * @param newChild
