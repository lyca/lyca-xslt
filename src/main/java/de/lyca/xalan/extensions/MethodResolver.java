...
import de.lyca.xpath.objects.XRTreeFrag;
...
public class MethodResolver {
   ...
   * static method, all arguments are used. Otherwise, it is an instance method
   * and only arguments beginning with the second argument are used.
   */
   ...
   * @param argsOut
   *          An array of the exact size as argsIn, which will be populated with
   *          converted arguments if a suitable method is found.
   ...
   * from the XSLT argument types, and perform the conversion of the arguments.
   * 
   * @param classObj
  ...
  public static Method getElementMethod(Class<?> classObj, String name) throws NoSuchMethodException, SecurityException,
    ...
    if (null == bestMethod)
      ...
      throw new TransformerException(XSLMessages.createMessage(XSLTErrorResources.ER_MORE_MATCH_ELEMENT,
              new Object[] { name })); // "More than one best match for element method "
                                       // + name);
   ...
   *          converted arguments.
   * @param paramTypes
   *          An array of class objects, of the exact same size as argsIn and
   *          argsOut.
   * @throws TransformerException
   *           may be thrown for Xalan conversion exceptions.
   */
   ...
   */
  private final static ConversionInfo[] m_javaObjConversions = { new ConversionInfo(Double.TYPE, 11),
          new ConversionInfo(Float.TYPE, 12), new ConversionInfo(Long.TYPE, 13), new ConversionInfo(Integer.TYPE, 14),
          new ConversionInfo(Short.TYPE, 15), new ConversionInfo(Character.TYPE, 16),
          new ConversionInfo(Byte.TYPE, 17), new ConversionInfo(java.lang.String.class, 18) };

  /**
   * Specification of conversions from XSLT type CLASS_BOOLEAN to allowed Java
   * types.
   */
  private final static ConversionInfo[] m_booleanConversions = { new ConversionInfo(Boolean.TYPE, 0),
          new ConversionInfo(java.lang.Boolean.class, 1), new ConversionInfo(java.lang.Object.class, 2),
   ...
   * types.
   */
  private final static ConversionInfo[] m_numberConversions = { new ConversionInfo(Double.TYPE, 0),
          new ConversionInfo(java.lang.Double.class, 1), new ConversionInfo(Float.TYPE, 3),
          new ConversionInfo(Long.TYPE, 4), new ConversionInfo(Integer.TYPE, 5), new ConversionInfo(Short.TYPE, 6),
          new ConversionInfo(Character.TYPE, 7), new ConversionInfo(Byte.TYPE, 8), new ConversionInfo(Boolean.TYPE, 9),
          new ConversionInfo(java.lang.String.class, 10), new ConversionInfo(java.lang.Object.class, 11) };

  /**
   * Specification of conversions from XSLT type CLASS_STRING to allowed Java
   * types.
   */
  private final static ConversionInfo[] m_stringConversions = { new ConversionInfo(java.lang.String.class, 0),
          new ConversionInfo(java.lang.Object.class, 1), new ConversionInfo(Character.TYPE, 2),
          new ConversionInfo(Double.TYPE, 3), new ConversionInfo(Float.TYPE, 3), new ConversionInfo(Long.TYPE, 3),
          new ConversionInfo(Integer.TYPE, 3), new ConversionInfo(Short.TYPE, 3), new ConversionInfo(Byte.TYPE, 3),
          new ConversionInfo(Boolean.TYPE, 4) };

  /**
   * Specification of conversions from XSLT type CLASS_RTREEFRAG to allowed Java
   * types.
   */
  private final static ConversionInfo[] m_rtfConversions = {
          new ConversionInfo(org.w3c.dom.traversal.NodeIterator.class, 0),
          new ConversionInfo(org.w3c.dom.NodeList.class, 1), new ConversionInfo(org.w3c.dom.Node.class, 2),
          new ConversionInfo(java.lang.String.class, 3), new ConversionInfo(java.lang.Object.class, 5),
          new ConversionInfo(Character.TYPE, 6), new ConversionInfo(Double.TYPE, 7), new ConversionInfo(Float.TYPE, 7),
          new ConversionInfo(Long.TYPE, 7), new ConversionInfo(Integer.TYPE, 7), new ConversionInfo(Short.TYPE, 7),
          new ConversionInfo(Byte.TYPE, 7), new ConversionInfo(Boolean.TYPE, 8) };
   ...
   */
  private final static ConversionInfo[][] m_conversions = { m_javaObjConversions, // CLASS_UNKNOWN
                                                                                  // =
                                                                                  // 0;
   ...
   * parameters. If any invocations of this function for a method with the same
   * name return the same positive value, then a conflict has occured, and an
   * error should be signaled.
   * 
   * @param javaParamTypes
   *          Must be filled with valid class names, and of the same length as
   *          xsltArgs.
   * @param xsltArgs
   *          Must be filled with valid object instances, and of the same length
   *          as javeParamTypes.
  ...
  public static int scoreMatch(Class<?>[] javaParamTypes, int javaParamsStart, Object[] xsltArgs, int score) {
    ...
    for (int i = nParams - javaParamTypes.length + javaParamsStart, javaParamTypesIndex = javaParamsStart; i < nParams; i++, javaParamTypesIndex++) {
      ...
      final ConversionInfo[] convInfo = m_conversions[xsltClassType];
      final int nConversions = convInfo.length;
      int k;
      for (k = 0; k < nConversions; k++) {
        final ConversionInfo cinfo = convInfo[k];
      ...
      if (k == nConversions) {
        ...
        // mapping which we can't handle through the array mechanism. To do
        // this,
        // we must determine the class of the argument passed from the
        // stylesheet.

        // If we were passed a subclass of XObject, representing one of the
        // actual
        // XSLT types, and we are here, we reject this extension method as a
        // candidate
        // because a match should have been made using the ConversionInfo array.
        // If we
        ...
        // checks to see if we were passed an XObject from the XSLT stylesheet.
        // If not,
        // we use the class of the object that was passed and make sure that it
        // will
        // map to the class type of the parameter in the extension function.
        // If we were passed an XObject, we attempt to get the class of the
        // actual
        // object encapsulated inside the XObject. If the encapsulated object is
        // null,
        ...
        // whether this java method is a valid match for this extension function
        // call.
        // This approach eliminates the NullPointerException in the earlier
        // implementation
        // that resulted from passing an XObject encapsulating the null java
        // object.
  ...
  private static String errString(String callType, // "function" or "element"
    ...
    switch (searchMethod) {
      case STATIC_ONLY:
        return resultString + "static " + classObj.getName() + "." + funcName + "([ExpressionContext,] "
                + errArgs(xsltArgs, 0) + ").";
      ...
      case DYNAMIC:
        return resultString + "static " + classObj.getName() + "." + funcName + "([ExpressionContext, ]"
                + errArgs(xsltArgs, 0) + ") nor\n" + classObj + "." + funcName + "([ExpressionContext,] "
                + errArgs(xsltArgs, 1) + ").";
      ...
      default:
        if (callType.equals("function"))
          ...
          return resultString + classObj.getName() + "." + funcName + "(de.lyca.xalan.extensions.XSLProcessorContext, "
                  + "de.lyca.xalan.templates.ElemExtensionCall).";
    }
