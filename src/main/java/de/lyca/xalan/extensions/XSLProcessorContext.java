...
import de.lyca.xpath.axes.DescendantIterator;
...
public class XSLProcessorContext {
  ...
  public XSLProcessorContext(TransformerImpl transformer, Stylesheet stylesheetTree) {
    ...
    // %TBD%
    final de.lyca.xpath.XPathContext xctxt = transformer.getXPathContext();
  ...
  private final Stylesheet stylesheetTree;
  ...
  private final de.lyca.xml.dtm.DTM sourceTree;
  ...
  /**
   * Output an object to the result tree by doing the right conversions. This is
   * public for access by extensions.
   * 
   * 
   * @param stylesheetTree
   *          The owning stylesheet
   * @param obj
   *          the Java object to output. If its of an X<something> type then
   *          that conversion is done first and then sent out.
   * 
   ...
   */
  public void outputToResultTree(Stylesheet stylesheetTree, Object obj) throws TransformerException,
          java.net.MalformedURLException, java.io.FileNotFoundException, java.io.IOException {

    try {
      final SerializationHandler rtreeHandler = transformer.getResultTreeHandler();
      ...
      if (obj instanceof XObject) {
        ...
        final int handle = xctxt.getDTMHandleFromNode((DocumentFragment) obj);

        value = new XRTreeFrag(handle, xctxt);
      } else if (obj instanceof DTM) {
        final DTM dtm = (DTM) obj;
        final DTMIterator iterator = new DescendantIterator();
        ...
        final DTMIterator iterator = new OneStepIterator(iter, -1);
        value = new XNodeSet(iterator);
      } else if (obj instanceof DTMIterator) {
        value = new XNodeSet((DTMIterator) obj);
      } else if (obj instanceof NodeIterator) {
        value = new XNodeSet(new de.lyca.xpath.NodeSetDTM((NodeIterator) obj, xctxt));
