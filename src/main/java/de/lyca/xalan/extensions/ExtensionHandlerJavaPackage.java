...
import de.lyca.xalan.trace.ExtensionEvent;
...
public class ExtensionHandlerJavaPackage extends ExtensionHandlerJava {
  ...
  /**
   * Tests whether a certain function name is known within this namespace. Since
   * this is for a package, we concatenate the package name used when this
   * handler was created and the function name specified in the argument. There
   * is no information regarding the arguments to the function call or whether
   * the method implementing the function is a static method or an instance
   * method.
   * 
   * @param function
  ...
  /**
   * Process a call to a function in the package java namespace. There are three
   ...
   * class whose name is formed by concatenating the value specified on the
   * namespace with the value specified in the function invocation before
   * ".new".</li>
   ...
   * of the same name will <i>not</i> be called in the current implementation.
   * This module does not verify that the obj argument is a member of the
  ...
  public Object callFunction(String funcName, List args, Object methodKey, ExpressionContext exprContext)
    ...
    try {
      ...
      if (funcName.endsWith(".new")) { // Handle constructor call
        ...
        m = MethodResolver.getMethod(classObj, methodName, methodArgs, convertedArgs, exprContext,
        ...
        if (args.size() < 1)
          throw new TransformerException(XSLMessages.createMessage(XSLTErrorResources.ER_INSTANCE_MTHD_CALL_REQUIRES,
                  new Object[] { funcName })); // "Instance method call to method "
                                               // + funcName
        ...
        m = MethodResolver.getMethod(classObj, funcName, methodArgs, convertedArgs, exprContext,
   ...
   * effect, the results are sent to the TransformerImpl's result tree. For this
   * namespace, only static element methods are currently supported. If instance
   * methods are needed, please let us know your requirements.
   * 
   * @param localPart
  ...
  public void processElement(String localPart, ElemTemplateElement element, TransformerImpl transformer,
    ...
    if (null == m) {
      try {
        final String fullName = m_className + localPart;
        final int lastDot = fullName.lastIndexOf('.');
        if (lastDot < 0)
          throw new TransformerException(XSLMessages.createMessage(XSLTErrorResources.ER_INVALID_ELEMENT_NAME,
                  new Object[] { fullName })); // "Invalid element name specified "
                                               // + fullName);
        try {
        ...
        if (!Modifier.isStatic(m.getModifiers()))
          throw new TransformerException(XSLMessages.createMessage(XSLTErrorResources.ER_ELEMENT_NAME_METHOD_STATIC,
                  new Object[] { fullName })); // "Element name method must be static "
                                               // + fullName);
      } catch (final Exception e) {
        // e.printStackTrace ();
        throw new TransformerException(e);
