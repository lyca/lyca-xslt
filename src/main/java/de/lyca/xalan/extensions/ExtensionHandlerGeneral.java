...
public class ExtensionHandlerGeneral extends ExtensionHandler {
  ...
  private static final String propName = "de.lyca.xalan.extensions.bsf.BSFManager";
   ...
   */
  public ExtensionHandlerGeneral(String namespaceUri, StringVector elemNames, StringVector funcNames,
          String scriptLang, String scriptSrcURL, String scriptSrc, String systemId) throws TransformerException {

    super(namespaceUri, scriptLang);

    if (elemNames != null) {
      final Object junk = new Object();
      final int n = elemNames.size();
    ...
    if (m_scriptSrcURL != null) {
      ...
      try {
        ...
        final int indexOfColon = m_scriptSrcURL.indexOf(':');
        final int indexOfSlash = m_scriptSrcURL.indexOf('/');

        if (indexOfColon != -1 && indexOfSlash != -1 && indexOfColon < indexOfSlash) {
          // The url is absolute.
          url = null;
          throw new TransformerException(XSLMessages.createMessage(XSLTErrorResources.ER_COULD_NOT_FIND_EXTERN_SCRIPT,
          ...
          } catch (final java.net.MalformedURLException mue2) {
            throw new TransformerException(XSLMessages.createMessage(
                    XSLTErrorResources.ER_COULD_NOT_FIND_EXTERN_SCRIPT, new Object[] { m_scriptSrcURL }), mue2); // "src attribute not yet supported for "
            // + scriptLang);
          }
        }
      }
    ...
    try {
      final Method loadScriptingEngine = manager.getClass().getMethod("loadScriptingEngine",
              new Class[] { String.class });

      m_engine = loadScriptingEngine.invoke(manager, new Object[] { scriptLang });

      final Method engineExec = m_engine.getClass().getMethod("exec",
      ...
      throw new TransformerException(XSLMessages.createMessage(XSLTErrorResources.ER_CANNOT_CMPL_EXTENSN, null), e); // "Could not compile extension",
                                                                                                                     // e);
    }
  }
  ...
  public Object callFunction(String funcName, Vector args, Object methodKey, ExpressionContext exprContext)
    ...
    try {
      ...
      if (m_engineCall == null) {
        m_engineCall = m_engine.getClass()
                .getMethod("call", new Class[] { Object.class, String.class, Object[].class });
      }
      ...
      if (null != msg) {
        ...
        throw new TransformerException(XSLMessages.createMessage(XSLTErrorResources.ER_CANNOT_CREATE_EXTENSN,
                new Object[] { funcName, e })); // "Could not create extension: "
                                                // + funcName
      // + " because of: " + e);
    }
  }
