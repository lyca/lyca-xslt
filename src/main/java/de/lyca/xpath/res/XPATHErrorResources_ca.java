  ...
  public Object[][] getContents() {
    return new Object[][] {
            ...
            { ER_CURRENT_NOT_ALLOWED_IN_MATCH,
                    "La funci\u00f3 current() no \u00e9s permesa en un patr\u00f3 de coincid\u00e8ncia." },

            { ER_CURRENT_TAKES_NO_ARGS, "La funci\u00f3 current() no accepta arguments." },

            { ER_DOCUMENT_REPLACED,
                    "La implementaci\u00f3 de la funci\u00f3 document() s'ha substitu\u00eft per de.lyca.xalan.xslt.FuncDocument." },

            { ER_CONTEXT_HAS_NO_OWNERDOC, "El context no t\u00e9 un document de propietari." },

            { ER_LOCALNAME_HAS_TOO_MANY_ARGS, "local-name() t\u00e9 massa arguments." },

            { ER_NAMESPACEURI_HAS_TOO_MANY_ARGS, "namespace-uri() t\u00e9 massa arguments." },

            { ER_NORMALIZESPACE_HAS_TOO_MANY_ARGS, "normalize-space() t\u00e9 massa arguments." },

            { ER_NUMBER_HAS_TOO_MANY_ARGS, "number() t\u00e9 massa arguments." },

            { ER_NAME_HAS_TOO_MANY_ARGS, "name() t\u00e9 massa arguments." },

            { ER_STRING_HAS_TOO_MANY_ARGS, "string() t\u00e9 massa arguments." },

            { ER_STRINGLENGTH_HAS_TOO_MANY_ARGS, "string-length() t\u00e9 massa arguments." },

            { ER_TRANSLATE_TAKES_3_ARGS, "La funci\u00f3 translate() t\u00e9 tres arguments." },

            { ER_UNPARSEDENTITYURI_TAKES_1_ARG, "La funci\u00f3 unparsed-entity-uri ha de tenir un argument." },

            { ER_NAMESPACEAXIS_NOT_IMPLEMENTED, "L'eix de l'espai de noms encara no s'ha implementat." },

            { ER_UNKNOWN_AXIS, "Eix desconegut: {0}" },

            { ER_UNKNOWN_MATCH_OPERATION, "Operaci\u00f3 de coincid\u00e8ncia desconeguda." },

            { ER_INCORRECT_ARG_LENGTH,
                    "La longitud de l'argument de la prova de node processing-instruction() no \u00e9s correcta." },

            { ER_CANT_CONVERT_TO_NUMBER, "No es pot convertir {0} en un n\u00famero." },

            { ER_CANT_CONVERT_TO_NODELIST, "No es pot convertir {0} en una NodeList." },

            { ER_CANT_CONVERT_TO_MUTABLENODELIST, "No es pot convertir {0} en un NodeSetDTM." },

            { ER_CANT_CONVERT_TO_TYPE, "No es pot convertir {0} en un tipus #{1}" },

            { ER_EXPECTED_MATCH_PATTERN, "El patr\u00f3 de coincid\u00e8ncia de getMatchScore \u00e9s l'esperat." },

            { ER_COULDNOT_GET_VAR_NAMED, "No s''ha pogut obtenir la variable {0}." },

            { ER_UNKNOWN_OPCODE, "ERROR. Codi op desconegut: {0}" },

            { ER_EXTRA_ILLEGAL_TOKENS, "Senyals addicionals no permesos: {0}" },

            { ER_EXPECTED_DOUBLE_QUOTE, "Les cometes del literal s\u00f3n incorrectes. Hi ha d'haver cometes dobles." },

            { ER_EXPECTED_SINGLE_QUOTE,
                    "Les cometes del literal s\u00f3n incorrectes. Hi ha d'haver una cometa simple." },

            { ER_EMPTY_EXPRESSION, "Expressi\u00f3 buida." },

            { ER_EXPECTED_BUT_FOUND, "S''esperava {0}, per\u00f2 s''ha detectat {1}" },

            { ER_INCORRECT_PROGRAMMER_ASSERTION, "L''afirmaci\u00f3 del programador \u00e9s incorrecta. - {0}" },

            { ER_BOOLEAN_ARG_NO_LONGER_OPTIONAL,
                    "L'argument boolean(...) ja no \u00e9s opcional amb l'esborrany d'XPath 19990709." },

            { ER_FOUND_COMMA_BUT_NO_PRECEDING_ARG, "S'ha trobat ',' per\u00f2 al davant no hi havia cap argument." },

            { ER_FOUND_COMMA_BUT_NO_FOLLOWING_ARG, "S'ha trobat ',' per\u00f2 al darrere no hi havia cap argument." },

            {
                    ER_PREDICATE_ILLEGAL_SYNTAX,
                    "'..[predicate]' o '.[predicate]' no \u00e9s una sintaxi permesa. En comptes d'aix\u00f2, utilitzeu 'self::node()[predicate]'." },

            { ER_ILLEGAL_AXIS_NAME, "Nom d''eix no perm\u00e8s: {0}" },

            { ER_UNKNOWN_NODETYPE, "Tipus de node desconegut: {0}" },

            { ER_PATTERN_LITERAL_NEEDS_BE_QUOTED, "El literal de patr\u00f3 ({0}) ha d''anar entre cometes." },

            { ER_COULDNOT_BE_FORMATTED_TO_NUMBER, "{0} no s''ha pogut formatar com a n\u00famero." },

            { ER_COULDNOT_CREATE_XMLPROCESSORLIAISON,
                    "No s''ha pogut crear la relaci\u00f3 XML TransformerFactory: {0}" },

            { ER_DIDNOT_FIND_XPATH_SELECT_EXP, "Error. No s'ha trobat l'expressi\u00f3 select d'xpath (-select)." },

            { ER_COULDNOT_FIND_ENDOP_AFTER_OPLOCATIONPATH,
                    "ERROR. No s'ha trobat ENDOP despr\u00e9s d'OP_LOCATIONPATH." },

            { ER_ERROR_OCCURED, "S'ha produ\u00eft un error." },
            ...
            { ER_KEY_HAS_TOO_MANY_ARGS, "key() t\u00e9 un nombre incorrecte d'arguments." },

            { ER_COUNT_TAKES_1_ARG, "La funci\u00f3 count ha de tenir un argument." },

            { ER_COULDNOT_FIND_FUNCTION, "No s''ha pogut trobar la funci\u00f3: {0}" },

            { ER_UNSUPPORTED_ENCODING, "Codificaci\u00f3 sense suport: {0}" },

            { ER_PROBLEM_IN_DTM_NEXTSIBLING,
                    "S'ha produ\u00eft un error en el DTM de getNextSibling. S'intentar\u00e0 solucionar." },

            { ER_CANNOT_WRITE_TO_EMPTYNODELISTIMPL, "Error del programador: no es pot escriure a EmptyNodeList." },

            { ER_SETDOMFACTORY_NOT_SUPPORTED, "XPathContext no d\u00f3na suport a setDOMFactory." },
            ...
            { ER_SAX_API_NOT_HANDLED, "Els car\u00e0cters de l'API SAX (char ch[]... no es poden gestionar pel DTM." },

            { ER_IGNORABLE_WHITESPACE_NOT_HANDLED, "ignorableWhitespace(char ch[]... no es poden gestionar pel DTM." },

            { ER_DTM_CANNOT_HANDLE_NODES, "DTMLiaison no pot gestionar nodes del tipus {0}." },

            { ER_XERCES_CANNOT_HANDLE_NODES, "DOM2Helper no pot gestionar nodes del tipus {0}." },

            { ER_XERCES_PARSE_ERROR_DETAILS, "Error de DOM2Helper.parse: ID del sistema - {0} l\u00ednia - {1}" },
            ...
            { ER_CANNOT_CREATE_URL, "No es pot crear la URL de: {0}" },

            { ER_XPATH_READOBJECT, "En XPath.readObject: {0}" },

            { ER_FUNCTION_TOKEN_NOT_FOUND, "No s'ha trobat el senyal de funci\u00f3." },

            { ER_CANNOT_DEAL_XPATH_TYPE, "No s''ha pogut tractar amb el tipus d''XPath: {0}" },

            { ER_NODESET_NOT_MUTABLE, "Aquest NodeSet no \u00e9s mutable." },

            { ER_NODESETDTM_NOT_MUTABLE, "Aquest NodeSetDTM no \u00e9s mutable." },

            { ER_VAR_NOT_RESOLVABLE, "No es pot resoldre la variable: {0}" },

            { ER_NULL_ERROR_HANDLER, "Manejador d'error nul" },

            { ER_PROG_ASSERT_UNKNOWN_OPCODE, "Afirmaci\u00f3 del programador: opcode desconegut: {0}" },

            { ER_ZERO_OR_ONE, "0 o 1" },

            { ER_RTF_NOT_SUPPORTED_XRTREEFRAGSELECTWRAPPER, "rtf() no t\u00e9 suport d'XRTreeFragSelectWrapper" },

            { ER_RTF_NOT_SUPPORTED_XRTREEFRAGSELECTWRAPPER,
                    "asNodeIterator() no t\u00e9 suport d'XRTreeFragSelectWrapper" },

            { ER_DETACH_NOT_SUPPORTED_XRTREEFRAGSELECTWRAPPER, "detach() no t\u00e9 suport d'XRTreeFragSelectWrapper" },

            { ER_NUM_NOT_SUPPORTED_XRTREEFRAGSELECTWRAPPER, "num() no t\u00e9 suport d'XRTreeFragSelectWrapper" },

            { ER_XSTR_NOT_SUPPORTED_XRTREEFRAGSELECTWRAPPER, "xstr() no t\u00e9 suport d'XRTreeFragSelectWrapper" },

            { ER_STR_NOT_SUPPORTED_XRTREEFRAGSELECTWRAPPER, "str() no t\u00e9 suport d'XRTreeFragSelectWrapper" },

            { ER_FSB_NOT_SUPPORTED_XSTRINGFORCHARS, "fsb() no t\u00e9 suport d'XStringForChars" },

            { ER_COULD_NOT_FIND_VAR, "No s''ha trobat la variable amb el nom de {0}" },

            { ER_XSTRINGFORCHARS_CANNOT_TAKE_STRING, "XStringForChars no pot agafar una cadena com a argument." },

            { ER_FASTSTRINGBUFFER_CANNOT_BE_NULL, "L'argument FastStringBuffer no pot ser nul." },

            { ER_TWO_OR_THREE, "2 o 3" },

            { ER_VARIABLE_ACCESSED_BEFORE_BIND, "S'ha accedit a la variable abans que estigu\u00e9s vinculada." },

            { ER_FSB_CANNOT_TAKE_STRING, "XStringForFSB no pot agafar una cadena com a argument." },

            { ER_SETTING_WALKER_ROOT_TO_NULL, "\n Error. S'est\u00e0 establint l'arrel d'un itinerant en nul." },

            { ER_NODESETDTM_CANNOT_ITERATE, "Aquest NodeSetDTM no es pot iterar en un node previ" },

            { ER_NODESET_CANNOT_ITERATE, "Aquest NodeSet no es pot iterar en un node previ" },

            { ER_NODESETDTM_CANNOT_INDEX, "Aquest NodeSetDTM no pot indexar ni efectuar funcions de recompte" },

            { ER_NODESET_CANNOT_INDEX, "Aquest NodeSet no pot indexar ni efectuar funcions de recompte" },

            { ER_CANNOT_CALL_SETSHOULDCACHENODE,
                    "No es pot cridar setShouldCacheNodes despr\u00e9s que s'hagi cridat nextNode" },

            { ER_ONLY_ALLOWS, "{0} nom\u00e9s permet {1} arguments" },

            { ER_UNKNOWN_STEP, "Afirmaci\u00f3 del programador a getNextStepPos: stepType desconegut: {0}" },
            ...
            { ER_EXPECTED_REL_LOC_PATH, "S'esperava una via d'acc\u00e9s relativa darrere del senyal '/' o '//'." },
            ...
            { ER_EXPECTED_LOC_STEP, "S'esperava un pas d'ubicaci\u00f3 despr\u00e9s del senyal '/' o '//'." },
            ...
            { ER_EXPECTED_NODE_TEST, "S'esperava una prova de node que coincid\u00eds amb NCName:* o QName." },
            ...
            { ER_EXPECTED_STEP_PATTERN, "S'esperava un patr\u00f3 de pas per\u00f2 s'ha trobat '/'." },
            ...
            // expected,
            // but was not found.
            { ER_EXPECTED_REL_PATH_PATTERN, "S'esperava un patr\u00f3 de via d'acc\u00e9s relativa." },
            ...
            { ER_INVALID_XPATH_TYPE, "L''argument de tipus XPath no \u00e9s v\u00e0lid: {0}" },

            { ER_EMPTY_XPATH_RESULT, "L'objecte de resultats XPath est\u00e0 buit." },
            ...
            { ER_NULL_RESOLVER, "No es pot resoldre el prefix amb un solucionador de prefix nul." },
            ...
            { ER_WRONG_DOCUMENT, "El node de context no pertany al document vinculat a aquest XPathEvaluator." },

            // Note to translators: The XPath expression cannot be evaluated
            // with respect
            // to this type of node.
            { ER_WRONG_NODETYPE, "El tipus de node de context no t\u00e9 suport." },

            { ER_XPATH_ERROR, "S'ha produ\u00eft un error desconegut a XPath." },
            ...
            { ER_RESOLVE_VARIABLE_RETURNS_NULL, "resolveVariable de la variable {0} torna el valor nul" },

            /** Field ER_UNSUPPORTED_RETURN_TYPE */

            { ER_UNSUPPORTED_RETURN_TYPE, "Tipus de retorn no suportat: {0}" },

            /** Field ER_SOURCE_RETURN_TYPE_CANNOT_BE_NULL */

            { ER_SOURCE_RETURN_TYPE_CANNOT_BE_NULL, "El tipus de retorn o d'origen no pot ser nul" },

            /** Field ER_ARG_CANNOT_BE_NULL */

            { ER_ARG_CANNOT_BE_NULL, "L''argument {0} no pot ser nul " },
            ...
            { ER_GETTING_NULL_FEATURE, "Intent d''obtenir una caracter\u00edstica amb un nom nul: {0}#getFeature(null)" },
            ...
            { WG_PROPERTY_NOT_SUPPORTED, "La propietat XSL no t\u00e9 suport: {0}" },

            { WG_DONT_DO_ANYTHING_WITH_NS, "No feu res ara mateix amb l''espai de noms {0} de la propietat: {1}" },

            { WG_SECURITY_EXCEPTION,
                    "S''ha produ\u00eft SecurityException en intentar accedir a la propietat de sistema XSL: {0}" },

            { WG_QUO_NO_LONGER_DEFINED, "Sintaxi antiga: quo(...) ja no est\u00e0 definit a XPath." },

            { WG_NEED_DERIVED_OBJECT_TO_IMPLEMENT_NODETEST,
                    "XPath necessita un objecte dedu\u00eft per implementar nodeTest." },

            { WG_FUNCTION_TOKEN_NOT_FOUND, "No s'ha trobat el senyal de funci\u00f3." },

            { WG_COULDNOT_FIND_FUNCTION, "No s''ha pogut trobar la funci\u00f3: {0}" },

            { WG_CANNOT_MAKE_URL_FROM, "No es pot crear la URL de: {0}" },

            { WG_EXPAND_ENTITIES_NOT_SUPPORTED, "L'opci\u00f3 -E no t\u00e9 suport a l'analitzador de DTM" },

            { WG_ILLEGAL_VARIABLE_REFERENCE,
                    "S''ha donat VariableReference per a una variable fora de context o sense definici\u00f3. Nom = {0}" },

            { WG_UNSUPPORTED_ENCODING, "Codificaci\u00f3 sense suport: {0}" },
            ...
            { "noParsermsg5", "AlphaWorks d'IBM: http://www.alphaworks.ibm.com/formula/xml" }, { "gtone", ">1" },
