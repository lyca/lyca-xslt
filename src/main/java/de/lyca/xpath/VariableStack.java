...
public class VariableStack implements Cloneable {
  ...
  public void reset() {
    ...
    // large stack size.
    final int linksSize = _links == null ? XPathContext.RECURSIONLIMIT : _links.length;
    final int varArraySize = _stackFrames == null ? XPathContext.RECURSIONLIMIT * 2 : _stackFrames.length;
  ...
  public int link(final int size) {
    ...
    if (_frameTop >= _stackFrames.length) {
      final XObject newsf[] = new XObject[_stackFrames.length + XPathContext.RECURSIONLIMIT + size];
    ...
    if (_linksTop + 1 >= _links.length) {
      final int newlinks[] = new int[_links.length + CLEARLIMITATION * 2];
