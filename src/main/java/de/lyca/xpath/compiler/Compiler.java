...
import de.lyca.xpath.operations.And;
import de.lyca.xpath.operations.Div;
import de.lyca.xpath.operations.Equals;
import de.lyca.xpath.operations.Gt;
import de.lyca.xpath.operations.Gte;
import de.lyca.xpath.operations.Lt;
import de.lyca.xpath.operations.Lte;
import de.lyca.xpath.operations.Minus;
import de.lyca.xpath.operations.Mod;
import de.lyca.xpath.operations.Mult;
import de.lyca.xpath.operations.Neg;
import de.lyca.xpath.operations.NotEquals;
import de.lyca.xpath.operations.Operation;
import de.lyca.xpath.operations.Or;
import de.lyca.xpath.operations.Plus;
import de.lyca.xpath.operations.UnaryOperation;
...
/**
   ...
   *
   * @return reference to {@link de.lyca.xpath.operations.Lt} instance.
   *
   ...
   * @param opPos The current position in the m_opMap array.
   * 
   * @return reference to {@link de.lyca.xpath.operations.Quo} instance.
   * 
   * @throws TransformerException if a error occurs creating the Expression.
   */
  ...
  protected StepPattern stepPattern(int opPos, int stepCount, StepPattern ancestorPattern) throws TransformerException {
    ...
    switch (stepType) {
      ...
      case OpCodes.MATCH_ANY_ANCESTOR:
        ...
        argLen = getArgLengthOfStep(opPos);
        opPos = getFirstChildPosOfStep(opPos);
        final int what = getWhatToShow(startOpPos);
  ...
  private Expression compileExtension(int opPos) throws TransformerException {
    ...
    final int endExtFunc = opPos + getOp(opPos + 1) - 1;

    opPos = getFirstChildPos(opPos);

    final java.lang.String ns = (java.lang.String) getTokenQueue().elementAt(getOp(opPos));

    opPos++;

    final java.lang.String funcName = (java.lang.String) getTokenQueue().elementAt(getOp(opPos));
    ...
    final Function extension = new FuncExtFunction(ns, funcName, String.valueOf(getNextMethodId()));

    try {
      int i = 0;

      while (opPos < endExtFunc) {
        final int nextOpPos = getNextOpPos(opPos);
  ...
  public void error(String msg, Object[] args) throws TransformerException {
    ...
    if (null != m_errorHandler) {
      ...
      throw new TransformerException(fmsg, m_locator);
  }
