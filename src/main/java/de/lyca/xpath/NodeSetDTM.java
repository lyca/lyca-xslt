...
public class NodeSetDTM extends NodeVector implements /* NodeList, NodeIterator, */DTMIterator, Cloneable {
   ...
   * a node (the context node... same value as "root" defined below) a pair of
   * non-zero positive integers (the context position and the context size) a
   * set of variable bindings a function library the set of namespace
   * declarations in scope for the expression.
   * 
   * <p>
   ...
   * 
   * @return a new NodeSetDTM of the same type, having the same state... though
   *         unless overridden in the subclasses, it may not copy all the state
   *         information.
   * 
   ...
   * iteration.
   * 
   * @return a new NodeSetDTM of the same type, having the same state... except
   ...
   * The available set of constants is defined in the <code>DTMFilter</code>
   * interface. For NodeSetDTMs, the mask has been hardcoded to show all nodes
   ...
   * in the set. After a DTMIterator is created, the first call to nextNode()
  ...
  public int previousNode() {
    ...
    if (!m_cacheNodes)
      throw new RuntimeException(XPATHMessages.createXPATHMessage(XPATHErrorResources.ER_NODESETDTM_CANNOT_ITERATE,
   ...
   * @throws RuntimeException
   *           thrown if this NodeSetDTM is not one of the types which supports
   *           indexing/counting.
   */
  @Override
  public void runTo(int index) {
    ...
    if (!m_cacheNodes)
      throw new RuntimeException(XPATHMessages.createXPATHMessage(XPATHErrorResources.ER_NODESETDTM_CANNOT_INDEX, null)); // "This NodeSetDTM can not do indexing or counting functions!");
  ...
  public void addNode(int n) {
    ...
    if (!m_mutable)
      throw new RuntimeException(XPATHMessages.createXPATHMessage(XPATHErrorResources.ER_NODESETDTM_NOT_MUTABLE, null)); // "This NodeSetDTM is not mutable!");
