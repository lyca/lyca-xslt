...
public class DescendantIterator extends LocPathIterator {
  ...
  DescendantIterator(Compiler compiler, int opPos, int analysis) throws TransformerException {
    ...
    if (0 == (whatToShow & (DTMFilter.SHOW_ATTRIBUTE | DTMFilter.SHOW_ELEMENT | DTMFilter.SHOW_PROCESSING_INSTRUCTION))
  ...
  public int nextNode() {
    ...
    de.lyca.xpath.VariableStack vars;
    int savedStart;
    if (-1 != m_stackFrame) {
    ...
    try {
      do {
        if (0 == m_extendedTypeID) {
          next = m_lastFetched = DTM.NULL == m_lastFetched ? m_traverser.first(m_context) : m_traverser.next(m_context,
                  m_lastFetched);
        } else {
          next = m_lastFetched = DTM.NULL == m_lastFetched ? m_traverser.first(m_context, m_extendedTypeID)
