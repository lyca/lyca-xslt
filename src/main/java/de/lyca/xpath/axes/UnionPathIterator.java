...
public class UnionPathIterator extends LocPathIterator implements Cloneable, DTMIterator, java.io.Serializable,
  ...
  public void addIterator(DTMIterator expr) {
    ...
    if (null == m_iterators) {
      ...
      final DTMIterator[] exprs = m_iterators;
      final int len = m_iterators.length;
  ...
  public static LocPathIterator createUnionIterator(Compiler compiler, int opPos)
    ...
    for (int i = 0; i < nPaths; i++) {
      ...
      if (lpi.getAxis() != Axis.CHILD) {
        ...
        // check for positional predicates or position function, which won't
        // work.
  ...
  public int getAnalysisBits() {
    ...
    if (m_exprs != null) {
      ...
      for (int i = 0; i < n; i++) {
        final int bit = m_exprs[i].getAnalysisBits();
        bits |= bit;
      }
    }
    ...
    if (steptype == OpCodes.OP_LOCATIONPATH) {
      ...
      switch (steptype) {
        ...
        case OpCodes.OP_GROUP:
          ...
          iter.m_firstWalker = new de.lyca.xpath.axes.FilterExprWalker(iter);
   ...
   * href="http://www.w3.org/TR/xpath#NT-LocationPath">location path</a>
  ...
  class iterOwner implements ExpressionOwner {
    ...
    public void setExpression(Expression exp) {
      ...
      if (!(exp instanceof LocPathIterator)) {
        ...
        // plain expressions?
        final WalkingIterator wi = new WalkingIterator(getPrefixResolver());
