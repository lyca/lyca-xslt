...
public class UnionChildIterator extends ChildTestIterator {
  ...
  /**
   * Even though these may hold full LocPathIterators, this array does not have
   * to be cloned, since only the node test and predicate portion are used, and
   * these only need static information. However, also note that index
   * predicates can not be used!
   */
   ...
   * 
   * @param test
   *          reference to a NodeTest, which will be added directly to the list
   *          of node tests (in other words, it will not be cloned). The parent
   *          of this test will be set to this object.
   */
  ...
  public short acceptNode(int n) {
    ...
    try {
      ...
      for (int i = 0; i < m_nodeTests.length; i++) {
        final PredicatedNodeTest pnt = m_nodeTests[i];
