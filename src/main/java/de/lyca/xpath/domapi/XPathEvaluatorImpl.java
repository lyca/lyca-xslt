 ...
 * 
 * <p>
 * The evaluation of XPath expressions is provided by <code>XPathEvaluator</code>, which will provide evaluation of
 * XPath 1.0 expressions with no specialized extension functions or variables. It is expected that the
 * <code>XPathEvaluator</code> interface will be implemented on the same object which implements the
 * <code>Document</code> interface in an implementation which supports the XPath DOM module. <code>XPathEvaluator</code>
 * implementations may be available from other sources that may provide support for special extension functions or
...
public final class XPathEvaluatorImpl implements XPathEvaluator {
  ...
  /**
   * This prefix resolver is created whenever null is passed to the evaluate method. Its purpose is to satisfy the DOM
   * L3 XPath API requirement that if a null prefix resolver is used, an exception should only be thrown when an attempt
   * is made to resolve a prefix.
   */
  private static class DummyPrefixResolver implements PrefixResolver {
     ...
     * 
     * @see de.lyca.xml.utils.PrefixResolver#getNamespaceForPrefix(String, Node)
     */
    @Override
     ...
     * @throws DOMException NAMESPACE_ERR: Always throws this exceptionn
     * 
     * @see de.lyca.xml.utils.PrefixResolver#getNamespaceForPrefix(String)
     */
    @Override
    ...
    /**
     * @see de.lyca.xml.utils.PrefixResolver#handlesNullPrefixes()
     */
    @Override
    ...
    /**
     * @see de.lyca.xml.utils.PrefixResolver#getBaseIdentifier()
     */
    @Override
  ...
  /**
   * Creates a parsed XPath expression with resolved namespaces. This is useful when an expression will be reused in an
   * application since it makes it possible to compile the expression string into a more efficient internal form and
   * preresolve all namespace prefixes which occur within the expression.
   * 
   * @param expression The XPath expression string to be parsed.
   * @param resolver The <code>resolver</code> permits translation of prefixes within the XPath expression into
   *        appropriate namespace URIs . If this is specified as <code>null</code>, any namespace prefix within the
   *        expression will result in <code>DOMException</code> being thrown with the code <code>NAMESPACE_ERR</code>.
   * @return The compiled form of the XPath expression.
   * @throws XPathException INVALID_EXPRESSION_ERR: Raised if the expression is not legal according to the rules of the
   *         <code>XPathEvaluator</code>i
   * @throws DOMException NAMESPACE_ERR: Raised if the expression contains namespace prefixes which cannot be resolved
   *         by the specified <code>XPathNSResolver</code>.
   * 
  ...
  /**
   * Adapts any DOM node to resolve namespaces so that an XPath expression can be easily evaluated relative to the
   * context of the node where it appeared within the document. This adapter works like the DOM Level 3 method
   * <code>lookupNamespaceURI</code> on nodes in resolving the namespaceURI from a given prefix using the current
   * information available in the node's hierarchy at the time lookupNamespaceURI is called, also correctly resolving
   * the implicit xml prefix.
   * 
   * @param nodeResolver The node to be used as a context for namespace resolution.
   * @return <code>XPathNSResolver</code> which resolves namespaces with respect to the definitions in scope for a
   *         specified node.
   * 
   ...
   *        type relying on XPath type conversions and fail if the desired coercion is not possible. This must be one of
   *        the type codes of <code>XPathResult</code>.
   ...
   * @throws DOMException NAMESPACE_ERR: Raised if the expression contains namespace prefixes which cannot be resolved
   *         by the specified <code>XPathNSResolver</code>. <br>
   *         WRONG_DOCUMENT_ERR: The Node is from a document that is not supported by this XPathEvaluator. <br>
