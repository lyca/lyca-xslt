...
public final class XPathEvaluatorImpl implements XPathEvaluator {
   ...
   * method. Its purpose is to satisfy the DOM L3 XPath API requirement that if
   * a null prefix resolver is used, an exception should only be thrown when an
   * attempt is made to resolve a prefix.
   */
  private static class DummyPrefixResolver implements PrefixResolver {
    ...
    public String getNamespaceForPrefix(String prefix, Node context) {
      ...
      throw new DOMException(DOMException.NAMESPACE_ERR, fmsg); // Unable to
                                                                // resolve
                                                                // prefix with
                                                                // null prefix
                                                                // resolver.
  ...
  /**
   * Constructor in the case that the XPath expression can be evaluated without
   * needing an XML document at all.
   * 
   */
   ...
   * form and preresolve all namespace prefixes which occur within the
   * expression.
   * 
   * @param expression
   *          The XPath expression string to be parsed.
   * @param resolver
   *          The <code>resolver</code> permits translation of prefixes within
  ...
  public XPathExpression createExpression(String expression, XPathNSResolver resolver) throws XPathException,
    ...
    try {
      ...
      final XPath xpath = new XPath(expression, null, null == resolver ? new DummyPrefixResolver()
   ...
   * <code>lookupNamespaceURI</code> on nodes in resolving the namespaceURI from
   * a given prefix using the current information available in the node's
   ...
   * @return <code>XPathNSResolver</code> which resolves namespaces with respect
   *         to the definitions in scope for a specified node.
   * 
  ...
  public XPathNSResolver createNSResolver(Node nodeResolver) {
    ...
    return new XPathNSResolverImpl(
            nodeResolver.getNodeType() == Node.DOCUMENT_NODE ? ((Document) nodeResolver).getDocumentElement()
                    : nodeResolver);
  }
   ...
   *          be coerced to return the specified type relying on XPath type
   ...
   *              supported by this XPathEvaluator. <br>
