...
public class NodeTest extends Expression {
  ...
  /**
   * The URL to pass to the Node#supports method, to see if the DOM has already
   * been stripped of whitespace nodes.
   */
  ...
  /**
   * Special bitmap for match patterns starting with a function. Make sure this
   ...
   * 
   * @see <a href="http://www.w3.org/TR/xslt#conflict">XSLT Specification - 5.5
  ...
  /**
   * The match score if the pattern has the form of a QName optionally preceded
   * by an @ character.
   * 
   * @see <a href="http://www.w3.org/TR/xslt#conflict">XSLT Specification - 5.5
   *      Conflict Resolution for Template Rules</a>
   ...
   * 
   * @param whatToShow
   *          Bit set defined mainly by {@link org.w3c.dom.traversal.NodeFilter}
   *          .
   * @param namespace
   ...
   * 
   * @param whatToShow
   *          Bit set defined mainly by {@link de.lyca.xml.dtm.DTMFilter}.
   * @return the node type for the whatToShow. Since whatToShow can specify
   *         multiple types, it will return the first bit tested that is on, so
   *         the caller of this function should take care that this is the
  ...
  /**
   * Two names are equal if they and either both are null or the name t is wild
   * and the name p is non-null, or the two strings are equal.
   * 
   * @param p
