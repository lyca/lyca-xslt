...
public class NodeSet implements NodeList, NodeIterator, Cloneable, ContextNodeList {
   ...
   * interface. For NodeSets, the mask has been hardcoded to show all nodes
   * except EntityReference nodes, which have no equivalent in the XPath data
   * model.
   * 
   * @return integer used as a bit-array, containing flags defined in the DOM's
   *         NodeFilter class. The value will be
   ...
   * support it.
   * 
   * @return null, which is slightly misleading. True, there is no user-written
   *         filter object, but in fact we are doing some very sophisticated
   *         custom filtering. A DOM purist might suggest returning a
   *         placeholder object just to indicate that this is not going to
   *         return all nodes selected by whatToShow.
   */
  @Override
   ...
   * when creating the iterator. To produce a view of the document that has
   * entity reference nodes but no entity expansion, use the whatToShow flags to
   * show the entity reference node and set expandEntityReferences to false.
   * 
   * @return true for all iterators based on NodeSet, meaning that the contents
   *         of EntityRefrence nodes may be returned (though whatToShow says
   *         that the EntityReferences themselves are not shown.)
   */
  @Override
   ...
   *           thrown if this NodeSet is not of a cached type, and hence doesn't
  ...
  public Node previousNode() throws DOMException {
    ...
    if (!m_cacheNodes)
      throw new RuntimeException(XPATHMessages.createXPATHMessage(XPATHErrorResources.ER_NODESET_CANNOT_ITERATE, null)); // "This NodeSet can not iterate to a previous node!");
   ...
   * <p>
   * This operation is a no-op in NodeSet, and will not cause INVALID_STATE_ERR
   * to be raised by later operations.
   * </p>
   */
  @Override
  public void detach() {
   ...
   * 
   * @return true if nextNode() would return the first node in the set, false if
   *         it would return a later one.
   */
  @Override
  ...
  /**
   * If an index is requested, NodeSet will call this method to run the iterator
   * to the index. By default this sets m_next to the index. If the index
   * argument is -1, this signals that the iterator should be run to the end.
   * 
   * @param index
   *          Position to advance (or retreat) to, with 0 requesting the reset
   *          ("fresh") position and -1 (or indeed any out-of-bounds value)
   *          requesting the final position.
   * @throws RuntimeException
  ...
  public void runTo(int index) {
    ...
    if (!m_cacheNodes)
      throw new RuntimeException(XPATHMessages.createXPATHMessage(XPATHErrorResources.ER_NODESET_CANNOT_INDEX, null)); // "This NodeSet can not do indexing or counting functions!");
   ...
   * to <code>length-1</code> inclusive. Note that this operation requires
   ...
   *          Node to be added
   * @param pos
   *          Offset at which the node is to be inserted, with 0 being the first
   *          position.
   * @throws RuntimeException
   *           thrown if this NodeSet is not of a mutable type.
   */
   ...
   * @throws RuntimeException
   *           thrown if this NodeSet is not of a cached type, and thus doesn't
  ...
  /**
   * If setShouldCacheNodes(true) is called, then nodes will be cached. They are
   * not cached by default. This switch must be set before the first call to
   * nextNode is made, to ensure that all nodes are cached.
   * 
   * @param b
  ...
  public void setShouldCacheNodes(boolean b) {
    ...
    if (!isFresh())
      throw new RuntimeException(XPATHMessages.createXPATHMessage(
              XPATHErrorResources.ER_CANNOT_CALL_SETSHOULDCACHENODE, null)); // "Can not call setShouldCacheNodes after nextNode has been called!");
  ...
  /**
   * Return the node at the top of the stack without popping the stack. Special
  ...
  /**
   * Pop a pair of nodes from the tail of the stack. Special purpose method for
   * TransformerImpl, pushElemTemplateElement. Performance critical.
   */
   ...
   * method for TransformerImpl, pushElemTemplateElement. Performance critical.
   * 
  ...
  /**
   * Return the node one position from the tail without popping. Special purpose
   * method for TransformerImpl, pushElemTemplateElement. Performance critical.
   * 
