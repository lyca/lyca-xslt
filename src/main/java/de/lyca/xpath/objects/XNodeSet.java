...
public class XNodeSet extends NodeSequence {
  ...
  public double getNumberFromNode(int n) {
    final XMLString xstr = m_dtmMgr.getDTM(n).getStringValue(n);
  ...
  // public Object object()
  // {
  // return new de.lyca.xml.dtm.ref.DTMNodeList(iter());
  // }
  ...
  public boolean compare(XObject obj2, Comparator comparator) throws javax.xml.transform.TransformerException {
    ...
    if (XObject.CLASS_NODESET == type) {
      ...
      while (DTM.NULL != (node1 = list1.nextNode())) {
        ...
        if (null == node2Strings) {
          ...
          while (DTM.NULL != (node2 = list2.nextNode())) {
            final XMLString s2 = getStringFromNode(node2);
      ...
      final XMLString s2 = obj2.xstr();
      final DTMIterator list1 = iterRaw();
      int node;

      while (DTM.NULL != (node = list1.nextNode())) {
        final XMLString s1 = getStringFromNode(node);
