...
public class XStringForFSB extends XString {
  ...
  private XStringForFSB(String val) {
    ...
    throw new IllegalArgumentException(XPATHMessages.createXPATHMessage(XPATHErrorResources.ER_FSB_CANNOT_TAKE_STRING,
  ...
  public int hashCode() {
     ...
     * int h = m_hash;
     * 
     * if (h == 0) { int off = m_start; int len = m_length; FastStringBuffer fsb
     * = fsb();
     * 
   ...
   * 
   * %OPT% CHECK PERFORMANCE against generating a Java String and converting it
   * to double. The advantage of running in native machine code -- perhaps even
   * microcode, on some systems -- may more than make up for the cost of
   * allocating and discarding the additional object. We need to benchmark this.
   * 
