...
public class XStringForFSB extends XString {
  ...
  private XStringForFSB(String val) {
    ...
    throw new IllegalArgumentException(Messages.get().fsbCannotTakeString());
  }
  ...
  public void getChars(int srcBegin, int srcEnd, char dst[], int dstBegin) {
    ...
    final int end = srcBegin + m_start + n;
    int d = dstBegin;
  ...
  public int hashCode() {
    ...
    /*
     * int h = m_hash; if (h == 0) { int off = m_start; int len = m_length; FastStringBuffer fsb = fsb(); for (int i =
     * 0; i < len; i++) { h = 31 * h + fsb.charAt(off); off++; } m_hash = h; }
     */
  ...
  public int indexOf(int ch, int fromIndex) {
    ...
    final int max = m_start + m_length;
    final FastStringBuffer fsb = fsb();
  ...
  /**
   * Returns whether the specified <var>ch</var> conforms to the XML 1.0 definition of whitespace. Refer to
  ...
  public XMLString fixWhiteSpace(boolean trimHead, boolean trimTail, boolean doublePunctuationSpaces) {
    ...
    final int end = m_length + m_start;
    final char[] buf = new char[m_length];
    final FastStringBuffer fsb = fsb();
    boolean edit = false;
  ...
  /**
   * Convert a string to a double -- Allowed input is in fixed notation ddd.fff. %OPT% CHECK PERFORMANCE against
   * generating a Java String and converting it to double. The advantage of running in native machine code -- perhaps
   * even microcode, on some systems -- may more than make up for the cost of allocating and discarding the additional
   * object. We need to benchmark this. %OPT% More importantly, we need to decide whether we _care_ about the
   * performance of this operation. Does XString.toDouble constitute any measurable percentage of our typical runtime? I
   * suspect not!
   * 
  ...
  public double toDouble() {
    ...
    int i;
    char c;
    final String valueString = fsb().getString(m_start, m_length);

    // The following are permitted in the Double.valueOf, but not by the XPath
    // spec:
    // - a plus sign
