...
import de.lyca.xml.utils.PrefixResolverDefault;
...
public class CachedXPathAPI {
   ...
   * its own {@link de.lyca.xml.dtm.DTMManager}. Good choice for simple uses.
   * </p>
   * <p>
   ...
   * {@link de.lyca.xml.dtm.DTM}) and previously established location state.
   * </p>
   * <p>
   * Note that the original {@link CachedXPathAPI} and the new one should not be
   * operated upon concurrently; we do not support multithreaded access to a
   * single {@link de.lyca.xml.dtm.DTM} at this time. Similarly, any particular
   * instance of {@link CachedXPathAPI} must not be operated upon by multiple
   ...
   * @see de.lyca.xpath.objects.XNull
   * @see de.lyca.xpath.objects.XBoolean
   * @see de.lyca.xpath.objects.XNumber
   * @see de.lyca.xpath.objects.XString
   * @see de.lyca.xpath.objects.XRTreeFrag
   * 
   * @throws TransformerException
   */
  ...
  /**
   * Evaluate XPath string to an XObject. XPath namespace prefixes are resolved
   * from the namespaceNode. The implementation of this is a little slow, since
   * it creates a number of objects each time it is called. This could be
   * optimized to keep the same objects around, but then thread-safety issues
   * would arise.
   * 
   * @param contextNode
   *          The node to start searching from.
  ...
  public XObject eval(Node contextNode, String str, Node namespaceNode) throws TransformerException {
    ...
    final PrefixResolverDefault prefixResolver = new PrefixResolverDefault(
            namespaceNode.getNodeType() == Node.DOCUMENT_NODE ? ((Document) namespaceNode).getDocumentElement()
                    : namespaceNode);
