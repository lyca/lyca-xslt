...
abstract public class ToStream extends SerializerBase {
  ...
  int accumDefaultEntity(java.io.Writer writer, char ch, int i, char[] chars, int len, boolean fromTextNode,
    ...
    if (!escLF && CharInfo.S_LINEFEED == ch) {
      ...
      // or if this is a character from attribute value and a special one of
      // those
  ...
  public void characters(final char chars[], final int start, final int length) throws org.xml.sax.SAXException {
    ...
    // Section 7.6.1 of XSLT 1.0 (http://www.w3.org/TR/xslt#value-of) suggest no
    // text node
    ...
    try {
      ...
      // of isWhitespace(ch)
      final int end = start + length;
      int lastDirtyCharProcessed = start - 1; // last non-clean character that
                                              // was processed
      // that was processed
      ...
      for (; i < end; i++) {
        ...
        if (m_charInfo.shouldMapTextChar(ch)) {
          ...
          if (ch <= 0x1F) {
            ...
            // So, this is a (valid) surrogate pair XALANJ-2419
  ...
  private int accumDefaultEscape(Writer writer, char ch, int i, char[] chars, int len, boolean fromTextNode,
    ...
    if (i == pos) {
      if (Encodings.isHighUTF16Surrogate(ch)) {
        ...
        writer.write(Integer.toString(codePoint));
        writer.write(';');
        pos += 2; // count the two characters that went into writing out this
                  // entity
      } else {
        /*
    ...
    if (ns != null && ns.length() > 0) {
      ...
      if (index > 0) {
        ...
        if (uri != null && uri.equals(ns))
          // the prefix in the raw name is already maps to the given namespace
          // uri
