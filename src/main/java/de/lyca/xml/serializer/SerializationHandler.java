...
public interface SerializationHandler extends ExtendedContentHandler, ExtendedLexicalHandler, XSLOutputAttributes,
   ...
   * attributes or namespace nodes after a startElement().
   * <p>
   * If the SerializationHandler has a Writer or OutputStream, a call to this
   * method will flush such accumulated events as a closed start tag for an
   * element.
   * <p>
   * If the SerializationHandler wraps a ContentHandler, a call to this method
   * will flush such accumulated events as a SAX (not SAX-like) calls to
   * startPrefixMapping() and startElement().
   * <p>
   * If one calls endDocument() then one need not call this method since a call
   * to endDocument() will do what this method does. However, in some
   * circumstances, such as with document fragments, endDocument() is not called
   * and it may be necessary to call this method to flush any pending events.
   * <p>
