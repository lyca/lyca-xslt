...
public class ToXMLStream extends ToStream {
   ...
   */
  private final CharInfo m_xmlcharInfo = CharInfo.getCharInfo(CharInfo.XML_ENTITIES_RESOURCE, Method.XML);
  ...
  public void startDocumentInternal() throws SAXException {
    ...
    if (m_needToCallStartDocument) {
      ...
      /*
       * The call to getXMLVersion() might emit an error message and we should
       * emit this message regardless of if we are writing out an XML header or
       * not.
       */
      final String version = getXMLVersion();
      if (getOmitXMLDeclaration() == false) {
        final String encoding = Encodings.getMimeEncoding(getEncoding());
        String standalone;
   ...
   * <p>
   * The contents of the whitespace preserving section will be delivered through
   ...
   * @param xslAttribute
   *          true if this attribute is from an xsl:attribute, false if declared
   *          within the elements opening tag.
   * @throws SAXException
   */
  @Override
  public void addAttribute(String uri, String localName, String rawName, String type, String value, boolean xslAttribute)
          throws SAXException {
    if (m_elemContext.m_startTagOpen) {
      final boolean was_added = addAttributeAlways(uri, localName, rawName, type, value, xslAttribute);

      /*
       * We don't run this block of code if: 1. The attribute value was only
       * replaced (was_added is false). 2. The attribute is from an
       * xsl:attribute element (that is handled in the addAttributeAlways() call
       * just above. 3. The name starts with "xmlns", i.e. it is a namespace
       * declaration.
       */
       ...
       * 
       * Section: 7.1.3 Creating Attributes Adding an attribute to an element
       * after a PI (for example) has been added to it is an error. The
       * attributes can be ignored. The spec doesn't explicitly say this is
       * disallowed, as it does for child elements, but it makes sense to have
       * the same treatment.
       * 
      ...
      final String msg = Utils.messages.createMessage(MsgKey.ER_ILLEGAL_ATTRIBUTE_POSITION, new Object[] { localName });
      ...
      try {
        ...
        final SAXException se = new SAXException(e);
        throw se;
      }
    }
  }
  ...
  /**
   * From XSLTC Declare a prefix to point to a namespace URI. Inform SAX handler
   ...
   * of output document is not specified, then output document is of version XML
   * 1.0. If XML version of output doucment is specified, but it is not either
  ...
  private String getXMLVersion() {
    ...
    if (xmlVersion == null || xmlVersion.equals(XMLVERSION10)) {
      ...
      final String msg = Utils.messages.createMessage(MsgKey.ER_XML_VERSION_NOT_SUPPORTED, new Object[] { xmlVersion });
      try {
        // Prepare to issue the warning message
        final Transformer tran = super.getTransformer();
        final ErrorListener errHandler = tran.getErrorListener();
