...
import de.lyca.xml.serializer.utils.URI.MalformedURIException;
...
public final class SystemIDResolver {
   ...
   * which throws SecurityException) then we just return the relative path. The
   * space and backslash characters are also replaced to generate a good
   * absolute URI.
   * </p>
   * 
   * @param localPath
  ...
  public static boolean isAbsoluteURI(String systemId) {
    /**
     * http://www.ietf.org/rfc/rfc2396.txt Authors should be aware that a path
     * segment which contains a colon character cannot be used as the first
     * segment of a relative URI path (e.g., "this:that"), because it would be
     * mistaken for a scheme name.
     **/
    /**
     * %REVIEW% Can we assume here that systemId is a valid URI? It looks like
     * we cannot ( See discussion of this common problem in Bugzilla Bug 22777
     * ).
     **/
  ...
  /**
   * Replace spaces with "%20" and backslashes with forward slashes in the input
   * string to generate a well-formed URI string.
   * 
   * @param str
   *          The input string
      ...
      if (systemId.startsWith("file:")) {
        ...
        if (str != null && str.startsWith("/")) {
          if (str.startsWith("///") || !str.startsWith("//")) {
            ...
            final int secondColonIndex = systemId.indexOf(':', 5);
  ...
  public static String getAbsoluteURI(String urlString, String base) throws TransformerException {
    ...
    } catch (final MalformedURIException mue) {
