    ...
    try {
      ...
      if (method.equals(Method.XML)) {
        ...
        if (null == m_xhtml_properties) // double check
        {
          ...
          m_xhtml_properties = loadPropertiesFile(fileName, m_xml_properties);
        }
  ...
  static private Properties loadPropertiesFile(final String resourceName, Properties defaults) throws IOException {
    ...
    try {
      ...
      if (defaults == null)
        ...
        throw new WrappedRuntimeException(Utils.messages.createMessage(MsgKey.ER_COULD_NOT_LOAD_RESOURCE,
                new Object[] { resourceName }), ioe);
      ...
      if (defaults == null)
        ...
        throw new WrappedRuntimeException(Utils.messages.createMessage(MsgKey.ER_COULD_NOT_LOAD_RESOURCE,
                new Object[] { resourceName }), se);
      // "Could not load '"+resourceName+"' (check CLASSPATH, applet security), now using just the defaults ",
      // se);
    } finally {
      if (bis != null) {
        bis.close();
