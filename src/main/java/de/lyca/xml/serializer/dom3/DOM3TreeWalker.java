...
import de.lyca.xml.serializer.utils.XML11Char;
import de.lyca.xml.serializer.utils.XMLChar;

/**
 * Built on de.lyca.xml.serializer.TreeWalker and adds functionality to traverse
...
final class DOM3TreeWalker {
  ...
  /**
   * The SerializationHandler, it extends ContentHandler and when this class is
   * instantiated via the constructor provided, a SerializationHandler object is
   * passed to it.
   */
  private SerializationHandler fSerializer = null;

  /**
   * We do not need DOM2Helper since DOM Level 3 LS applies to DOM Level 2 or
   * newer
   */

  /** Locator object for this TreeWalker */
  private final LocatorImpl fLocator = new LocatorImpl();
  ...
  private final boolean fInEntityRef = false;

  /** Stores the version of the XML document to be serialize */
  private final String fXMLVersion = null;
   ...
   *          serialHandler The implemention of the SerializationHandler
   *          interface
   */
  DOM3TreeWalker(SerializationHandler serialHandler, DOMErrorHandler errHandler, LSSerializerFilter filter,
          String newLine) {
    fSerializer = serialHandler;
    // fErrorHandler = errHandler == null ? new DOMErrorHandlerImpl() :
    // errHandler; // Should we be using the default?
    ...
    } catch (final SecurityException se) { // user.dir not accessible from
                                           // applet
   ...
   * 
   * @param node
   *          The Docuemnt Type Node to serialize
   * @param bStart
  ...
  protected void serializeDocType(DocumentType node, boolean bStart) throws SAXException {
    ...
    final String docTypeName = node.getNodeName();
  ...
  protected void serializeElement(Element node, boolean bStart) throws SAXException {
    if (bStart) {
      ...
      // since endPrefixMapping was not used by SerializationHandler it was
      // removed
    ...
    for (int i = 0; i < nAttrs; i++) {
      ...
      if ((fFeatures & NAMESPACES) != 0 && !xmlnsAttr) {
        ...
        if (attrNS != null) {
          ...
          final String declAttrPrefix = fNSBinder.getPrefix(attrNS);
          final String declAttrNS = fNSBinder.getURI(attrPrefix);
          ...
          if (localName == null) {
            // DOM Level 1 node!
            final String msg = Utils.messages.createMessage(MsgKey.ER_NULL_LOCAL_ELEMENT_NAME,
                    new Object[] { attrName });

            if (fErrorHandler != null) {
              fErrorHandler.handleError(new DOMErrorImpl(DOMError.SEVERITY_ERROR, msg,
                      MsgKey.ER_NULL_LOCAL_ELEMENT_NAME, null, null, null));
            }
      ...
      if (applyFilter) {
        ...
        if (fFilter != null && (fFilter.getWhatToShow() & NodeFilter.SHOW_ATTRIBUTE) != 0) {
          ...
          if (!xmlnsAttr) {
            final short code = fFilter.acceptNode(attr);
            switch (code) {
              case NodeFilter.FILTER_REJECT:
              case NodeFilter.FILTER_SKIP:
                addAttr = false;
      ...
      if (addAttr && xmlnsAttr) {
        ...
        // If namespace-declarations=true, add the node with the Attr nodes
        // namespaceURI
        // else add the node setting it's namespace to null or else the
        // serializer will later
  ...
  protected void serializePI(ProcessingInstruction node) throws SAXException {
    final ProcessingInstruction pi = node;
    ...
    if ((fFeatures & CDATA) != 0) {
      ...
      if ((fFeatures & SPLITCDATA) != 0) {
        if (endIndex >= 0) {
          ...
          final String msg = Utils.messages.createMessage(MsgKey.ER_CDATA_SECTIONS_SPLIT, null);

          if (fErrorHandler != null) {
            fErrorHandler.handleError(new DOMErrorImpl(DOMError.SEVERITY_WARNING, msg, MsgKey.ER_CDATA_SECTIONS_SPLIT,
                    null, relatedData, null));
          }
        }
      } else {
        if (endIndex >= 0) {
  ...
  protected void serializeEntityReference(EntityReference node, boolean bStart) throws SAXException {
    if (bStart) {
      final EntityReference eref = node;
      // entities=true
   ...
   * @param refInvalidChar
   *          A reference to the character to be returned that was determined
   *          invalid.
   */
  ...
  /**
   * Checks if a XML character is well-formed. If there is a problem with the
   * character a non-null Character is returned else null is returned.
   * 
   * @param characters
   *          A String of characters to be checked for Well-Formedness
   * @return Character A reference to the character to be returned that was
   *         determined invalid.
   */
    ...
    if (fIsXMLVersion11) {
      ...
      while (i < datalength) {
        ...
        if (XML11Char.isXML11Invalid(c)) {
          ...
          final String msg = Utils.messages.createMessage(MsgKey.ER_WF_INVALID_CHARACTER_IN_COMMENT,
                  new Object[] { new Character(c) });

          if (fErrorHandler != null) {
            fErrorHandler.handleError(new DOMErrorImpl(DOMError.SEVERITY_FATAL_ERROR, msg,
                    MsgKey.ER_WF_INVALID_CHARACTER, null, null, null));
          }
        } else if (c == '-' && i < datalength && dataarray[i] == '-') {
          final String msg = Utils.messages.createMessage(MsgKey.ER_WF_DASH_IN_COMMENT, null);
  ...
  /**
   * Checks if an element node is well-formed, by checking its Name for
   * well-formedness.
   * 
   * @param data
   *          The contents of the comment node
  ...
  protected void isElementWellFormed(Node node) {
    ...
    if (!isNameWF) {
      final String msg = Utils.messages.createMessage(MsgKey.ER_WF_INVALID_CHARACTER_IN_NODE_NAME, new Object[] {
              "Element", node.getNodeName() });

      if (fErrorHandler != null) {
        fErrorHandler.handleError(new DOMErrorImpl(DOMError.SEVERITY_FATAL_ERROR, msg,
                MsgKey.ER_WF_INVALID_CHARACTER_IN_NODE_NAME, null, null, null));
      }
    }
  }

  /**
   * Checks if an attr node is well-formed, by checking it's Name and value for
   * well-formedness.
   * 
   * @param data
   *          The contents of the comment node
  ...
  protected void isAttributeWellFormed(Node node) {
    ...
    if (value.indexOf('<') >= 0) {
      final String msg = Utils.messages.createMessage(MsgKey.ER_WF_LT_IN_ATTVAL, new Object[] {
              ((Attr) node).getOwnerElement().getNodeName(), node.getNodeName() });

      if (fErrorHandler != null) {
        fErrorHandler.handleError(new DOMErrorImpl(DOMError.SEVERITY_FATAL_ERROR, msg, MsgKey.ER_WF_LT_IN_ATTVAL, null,
                null, null));
      }
    }

    // we need to loop through the children of attr nodes and check their values
    // for
    // well-formedness
    ...
    // WFC: Unique Att Spec
    // Perhaps pass a seen boolean value to this method. serializeAttList will
    // determine
    // if the attr was seen before.
  }

  /**
   * Checks if a PI node is well-formed, by checking it's Name and data for
   * well-formedness.
   * 
   * @param data
   *          The contents of the comment node
  ...
  protected void isPIWellFormed(ProcessingInstruction node) {
    ...
    final Character invalidChar = isWFXMLChar(node.getData());
    if (invalidChar != null) {
      final String msg = Utils.messages.createMessage(MsgKey.ER_WF_INVALID_CHARACTER_IN_PI,
              new Object[] { Integer.toHexString(Character.getNumericValue(invalidChar.charValue())) });

      if (fErrorHandler != null) {
        fErrorHandler.handleError(new DOMErrorImpl(DOMError.SEVERITY_FATAL_ERROR, msg, MsgKey.ER_WF_INVALID_CHARACTER,
                null, null, null));
      }
    }
  }
  ...
  /**
   * Checks if an CDATASection node is well-formed, by checking it's data for
   * well-formedness. Note that the presence of a CDATA termination mark in the
   * contents of a CDATASection is handled by the parameter spli-cdata-sections
   * 
   * @param data
   *          The contents of the comment node
   */
  protected void isCDATASectionWellFormed(CDATASection node) {
    ...
    if (invalidChar != null) {
      final String msg = Utils.messages.createMessage(MsgKey.ER_WF_INVALID_CHARACTER_IN_CDATA,
              new Object[] { Integer.toHexString(Character.getNumericValue(invalidChar.charValue())) });
  ...
  protected void isTextWellFormed(Text node) {
    ...
    final Character invalidChar = isWFXMLChar(node.getData());
    if (invalidChar != null) {
      final String msg = Utils.messages.createMessage(MsgKey.ER_WF_INVALID_CHARACTER_IN_TEXT,
              new Object[] { Integer.toHexString(Character.getNumericValue(invalidChar.charValue())) });
  ...
  /**
   * Checks if an EntityRefernece node is well-formed, by checking it's node
   * name. Then depending on whether it is referenced in Element content or in
   * an Attr Node, checks if the EntityReference references an unparsed entity
   * or a external entity and if so throws raises the appropriate
  ...
  protected void isEntityReferneceWellFormed(EntityReference node) {
    ...
    // of the EntityReference matches an Entity. If so, check the if the
    // notationName
        ...
        if (parent.getNodeType() == Node.ELEMENT_NODE) {
          if (entNamespaceURI.equals(nodeNamespaceURI) && entName.equals(nodeName)) {
            ...
            if (ent.getNotationName() != null) {
              final String msg = Utils.messages.createMessage(MsgKey.ER_WF_REF_TO_UNPARSED_ENT,
                      new Object[] { node.getNodeName() });

              if (fErrorHandler != null) {
                fErrorHandler.handleError(new DOMErrorImpl(DOMError.SEVERITY_FATAL_ERROR, msg,
                        MsgKey.ER_WF_REF_TO_UNPARSED_ENT, null, null, null));
              }
            }
          }
        ...
        if (parent.getNodeType() == Node.ATTRIBUTE_NODE) {
          if (entNamespaceURI.equals(nodeNamespaceURI) && entName.equals(nodeName)) {
            ...
            if (ent.getPublicId() != null || ent.getSystemId() != null || ent.getNotationName() != null) {
              final String msg = Utils.messages.createMessage(MsgKey.ER_WF_REF_TO_EXTERNAL_ENT,
                      new Object[] { node.getNodeName() });

              if (fErrorHandler != null) {
                fErrorHandler.handleError(new DOMErrorImpl(DOMError.SEVERITY_FATAL_ERROR, msg,
                        MsgKey.ER_WF_REF_TO_EXTERNAL_ENT, null, null, null));
              }
            }
          }
   ...
   * prefixes is referenced in a location where there are no bindings for the
   * namespace prefixes and if so raises a LSException with the error-type
   * "unbound-prefix-in-entity-reference"
   * 
   * @param Node
   *          , The EntityReference nodes whose children are to be checked
   */
    ...
    for (child = node.getFirstChild(); child != null; child = next) {
      ...
      if (child.getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (prefix != null && fNSBinder.getURI(prefix) == null) {
          final String msg = Utils.messages.createMessage(MsgKey.ER_ELEM_UNBOUND_PREFIX_IN_ENTREF,
                  new Object[] { node.getNodeName(), child.getNodeName(), prefix });

          if (fErrorHandler != null) {
            fErrorHandler.handleError(new DOMErrorImpl(DOMError.SEVERITY_FATAL_ERROR, msg,
                    MsgKey.ER_ELEM_UNBOUND_PREFIX_IN_ENTREF, null, null, null));
          }
        }
        ...
        for (int i = 0; i < attrs.getLength(); i++) {
          ...
          if (attrPrefix != null && fNSBinder.getURI(attrPrefix) == null) {
            final String msg = Utils.messages.createMessage(MsgKey.ER_ATTR_UNBOUND_PREFIX_IN_ENTREF, new Object[] {
                    node.getNodeName(), child.getNodeName(), attrs.item(i) });

            if (fErrorHandler != null) {
              fErrorHandler.handleError(new DOMErrorImpl(DOMError.SEVERITY_FATAL_ERROR, msg,
                      MsgKey.ER_ATTR_UNBOUND_PREFIX_IN_ENTREF, null, null, null));
            }
          }
        }
      }
   ...
   * 
   * @param Node
   *          , The element node, whose namespace declarations are to be
   *          recorded
   */
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (XMLNS_URI.equals(attrNS)) {
        ...
        if (XMLNS_URI.equals(attrValue)) {
          final String msg = Utils.messages.createMessage(MsgKey.ER_NS_PREFIX_CANNOT_BE_BOUND, new Object[] {
                  attrPrefix, XMLNS_URI });

          if (fErrorHandler != null) {
            fErrorHandler.handleError(new DOMErrorImpl(DOMError.SEVERITY_ERROR, msg,
                    MsgKey.ER_NS_PREFIX_CANNOT_BE_BOUND, null, null, null));
          }
        } else {
  ...
  protected void fixupElementNS(Node node) throws SAXException {
    ...
    if (namespaceURI != null) {
      ...
      final String inScopeNamespaceURI = fNSBinder.getURI(prefix);
      ...
      if (localName == null || "".equals(localName)) {
        ...
        if (fErrorHandler != null) {
          fErrorHandler.handleError(new DOMErrorImpl(DOMError.SEVERITY_ERROR, msg, MsgKey.ER_NULL_LOCAL_ELEMENT_NAME,
                  null, null, null));
        }
      } else {
   ...
   * 
   * @param properties
   *          DOMConfiguraiton properties that were set and which are to be used
   *          while serializing the DOM.
   */
  protected void initProperties(Properties properties) {
    ...
    for (final Enumeration keys = properties.keys(); keys.hasMoreElements();) {
      ...
      // caonical-form
      // Other features will be enabled or disabled when this is set to true or
      // false.
      ...
      if (iobj != null) {
        if (iobj instanceof Integer) {
          ...
          if ((DOMConstants.S_DOM3_PROPERTIES_NS + DOMConstants.DOM_FORMAT_PRETTY_PRINT).equals(key)) {
            // format-pretty-print; set internally on the serializers via
            // xsl:output properties in LSSerializer
            if (properties.getProperty(key).endsWith("yes")) {
            ...
            // omit-xml-declaration; set internally on the serializers via
            // xsl:output properties in LSSerializer
            if (properties.getProperty(key).endsWith("yes")) {
            ...
            // Retreive the value of the XML Version attribute via the
            // xml-version
