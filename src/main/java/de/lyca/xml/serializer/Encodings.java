...
public final class Encodings {
  ...
  static Writer getWriter(OutputStream output, String encoding) throws UnsupportedEncodingException {
    ...
    for (int i = 0; i < _encodings.length; ++i) {
      if (_encodings[i].name.equalsIgnoreCase(encoding)) {
        try {
          final String javaName = _encodings[i].javaName;
          final OutputStreamWriter osw = new OutputStreamWriter(output, javaName);
          return osw;
        } catch (final java.lang.IllegalArgumentException iae) // java 1.1.8
        {
          // keep trying
  ...
  /**
   * Returns the EncodingInfo object for the specified encoding, never null,
   * although the encoding name inside the returned EncodingInfo object will be
   * if we can't find a "real" EncodingInfo for the encoding.
   * <p>
   * This is not a public API.
   * 
   * @param encoding
  ...
  /**
   * Determines if the encoding specified was recognized by the serializer or
   * not.
   * 
   * @param encoding
   *          The encoding
   ...
   * should use either UTF-8 or UTF-16."
   * <p>
   * This is not a public API.
   * 
   * @param encoding
    ...
    if (null == encoding) {
      try {
        ...
        if (null != encoding) {
          ...
          /*
           * See if the mime type is equal to UTF8. If you don't do that, then
           * convertJava2MimeEncoding will convert 8859_1 to "ISO-8859-1", which
           * is not what we want, I think, and I don't think I want to alter the
           * tables to convert everything to UTF-8.
           */
          final String jencoding = encoding.equalsIgnoreCase("Cp1252") || encoding.equalsIgnoreCase("ISO8859_1")
                  || encoding.equalsIgnoreCase("8859_1") || encoding.equalsIgnoreCase("UTF8") ? DEFAULT_MIME_ENCODING
  ...
  private static EncodingInfo[] loadEncodingInfo() {
    try {
      ...
      for (int i = 0; i < totalEntries; ++i) {
        ...
        final int len = lengthOfMimeNames(val);
        ...
        if (len == 0) {
          ...
          mimeName = javaName;
          highChar = '\u0000'; // don't know the high code point, will need to
          ...
          final String mimeNames = val.substring(0, len);
          final StringTokenizer st = new StringTokenizer(mimeNames, ",");
          for (boolean first = true; st.hasMoreTokens(); first = false) {
            mimeName = st.nextToken();
            final EncodingInfo ei = new EncodingInfo(mimeName, javaName, highChar);
      ...
      final EncodingInfo[] ret_ei = new EncodingInfo[encodingInfo_list.size()];
      encodingInfo_list.toArray(ret_ei);
      return ret_ei;
    } catch (final java.net.MalformedURLException mue) {
      throw new de.lyca.xml.serializer.utils.WrappedRuntimeException(mue);
    } catch (final java.io.IOException ioe) {
      throw new de.lyca.xml.serializer.utils.WrappedRuntimeException(ioe);
    }
  }
   ...
   * 
   * @param val
   *          The value of the property, which should contain a comma separated
   *          list of Mime names, followed optionally by a space and the high
   *          char value
   * @return
   */
   ...
   * Code point values above this one may or may not be in the encoding, but
   * lower ones certainly are.
   * <p>
   * This is for performance.
   * 
   * @param encoding
   *          The encoding
   * @return The code point for which characters at or below this code point are
   *         in the encoding. Characters with higher code point may or may not
   *         be in the encoding. A value of zero is returned if the high code
   *         point is unknown.
   *         <p>
   *         This method is not a public API.
   * @xsl.usage internal
