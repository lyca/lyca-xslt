...
final class ElemContext {
  ...
  /**
   * The URI of the element. If this value is null it means that the URI is not
   * yet determined for the element. Valid values are the empty string "",
   * meaning that it is in no namespace, or a string of non-zero length.
   */
  ...
  /**
   * True if the current element has output escaping disabled. This is true for
   * SCRIPT and STYLE elements.
   */
  boolean m_isRaw = false;

  /**
   * The next element "stack frame". This value will only be set once as deeper
   * stack frames are not deleted when popped off, but are rather re-used when a
   * push is required.
   * 
   ...
   * re-use saves object creation but it also means that connections between the
   * frames via m_next and m_prev never changes either. Just the contents of the
   * frames change as they are re-used. Only the reference to the current stack
   * frame, which is held by the serializer is changed via a quick pop() or
   * push().
   */
  ...
  final ElemContext pop() {
     ...
     * deeper stack frames are still attached but dormant, just waiting to be
     * re-used.
     */
    return m_prev;
  }
   ...
   * when pushing a stack frame for an HTML "IMG" tag which has no children and
  ...
  final ElemContext push() {
    ...
    if (frame == null) {
      /*
       * We have never been at this depth yet, and there is no stack frame to
       * re-use, so we now make a new one.
       */
      frame = new ElemContext(this);
      m_next = frame;
    }
    /*
     * We shouldn't need to set this true because we should just be pushing a
     * dummy stack frame that will be instantly popped. Yet we need to be ready
     * in case this element does have unexpected children.
     */
   ...
   * 
   * @param uri
   *          The URI for the namespace for the element name, can be null if it
   *          is not yet known.
   * @param localName
