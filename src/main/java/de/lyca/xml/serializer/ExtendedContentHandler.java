...
public interface ExtendedContentHandler extends org.xml.sax.ContentHandler {
   ...
   * will be calculated from the prefix of qName. The local name will be derived
   * from qName and the type will be assumed to be "CDATA".
   * 
   * @param qName
   * @param value
   */
   ...
   * a character String rather than the standard character array.
   * 
   * @param chars
   *          the character data
   * @throws SAXException
  ...
  /**
   * This method is used to notify of a character event, but passing the data as
   * a DOM Node rather than the standard character array.
   * 
   * @param node
   ...
   * </pre>
   * 
   * only the last parameter is passed. If needed the serializer can derive the
   * localName from the qualified name and derive the namespaceURI from its
   * implementation.
   * 
   * @param elemName
   ...
   * not to the current element. This method applies to the current element. For
   * example one could make the calls in this order:
   * 
   * <pre>
   ...
   * @param shouldFlush
   *          if true this call is like the SAX startPrefixMapping(prefix,uri)
   *          call and the mapping applies to the element to come. If false the
   *          mapping applies to the current element.
   * @return boolean false if the prefix mapping was already in effect (in other
   *         words we are just re-declaring), true if this is a new, never
   ...
   * to be unique here. The serializer can write it out immediately without
   * saving it in a table first. The integer flag contains information about the
   * attribute, which helps the serializer to decide whether a particular
