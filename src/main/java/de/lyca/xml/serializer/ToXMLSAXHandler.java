...
public final class ToXMLSAXHandler extends ToSAXHandler {
  ...
  public boolean setEscaping(boolean escape) throws SAXException {
    final boolean oldEscapeSetting = m_escapeSetting;
  ...
  /**
   * Closes ane open cdata tag, and unlike the this.endCDATA() method (from the
   * LexicalHandler) interface, this "internal" method will send the endCDATA()
   * call to the wrapped handler.
   * 
   */
  @Override
  ...
  public void endElement(String namespaceURI, String localName, String qName) throws SAXException {
    ...
    /*
     * Pop all namespaces at the current element depth. We are not waiting for
     * official endPrefixMapping() calls.
     */
  ...
  public void endPrefixMapping(String prefix) throws SAXException {
    /*
     * poping all prefix mappings should have been done in endElement() already
     */
    return;
  }
   ...
   *          The URI for the namespace
   * @param shouldFlush
   *          a flag indicating if the mapping applies to the current element or
   *          an up coming child (not used).
   */
  ...
  public boolean startPrefixMapping(String prefix, String uri, boolean shouldFlush) throws org.xml.sax.SAXException {
    ...
    if (pushed) {
      ...
      if (getShouldOutputNSAttr()) {
        ...
        /*
         * I don't know if we really needto do this. The callers of this object
         * should have injected both startPrefixMapping and the attributes. We
        ...
        if (EMPTYSTRING.equals(prefix)) {
          ...
          { // that maps ns1 prefix to "" URI
            ...
            /*
             * for something like xmlns:abc="w3.pretend.org" the uri is the
             * value, that is why we pass it in the value, or 5th slot of
             * addAttributeAlways()
             */
  ...
  public void endCDATA() throws SAXException {
    /*
     * Normally we would do somthing with this but we ignore it. The neccessary
     * call to m_lexHandler.endCDATA() will be made in flushPending().
     * 
     * This is so that if we get calls like these: this.startCDATA();
     * this.characters(chars1, off1, len1); this.endCDATA(); this.startCDATA();
     * this.characters(chars2, off2, len2); this.endCDATA();
     * 
     * that we will only make these calls to the wrapped handlers:
     * 
     * m_lexHandler.startCDATA(); m_saxHandler.characters(chars1, off1, len1);
     * m_saxHandler.characters(chars1, off2, len2); m_lexHandler.endCDATA();
     * 
  ...
  public void characters(char[] ch, int off, int len) throws SAXException {
     ...
     * m_saxHandler worry about it, we've already warned them with the previous
     * call of m_lexHandler.startCDATA();
     */
  ...
  /**
   * 
   * @see org.xml.sax.ContentHandler#processingInstruction(String, String) Send
   *      a processing instruction to the output document
   */
  @Override
  ...
  public void startCDATA() throws SAXException {
    /*
     * m_cdataTagOpen can only be true here if we have ignored the previous call
     * to this.endCDATA() and the previous call this.startCDATA() before that is
     * still "open". In this way we merge adjacent CDATA. If anything else
     * happened after the ignored call to this.endCDATA() and this call then a
     * call to flushPending() would have been made which would have closed the
     * CDATA and set m_cdataTagOpen to false.
     */
    if (!m_cdataTagOpen) {
      flushPending();
      if (m_lexHandler != null) {
  ...
  /**
   * Adds the given attribute to the set of attributes, and also makes sure that
   * the needed prefix/uri mapping is declared, but only if there is a currently
   * open element.
   * 
   * @param uri
