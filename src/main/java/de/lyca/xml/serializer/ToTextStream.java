...
public class ToTextStream extends ToStream {
  ...
  public void characters(char ch[], int start, int length) throws org.xml.sax.SAXException {
    ...
    try {
      if (inTemporaryOutputState()) {
        /*
         * leave characters un-processed as we are creating temporary output,
         * the output generated by this serializer will be input to a final
         * serializer later on and it will do the processing in final output
         * state (not temporary output state).
         * 
         * A "temporary" ToTextStream serializer is used to evaluate attribute
         * value templates (for example), and the result of evaluating such a
         * thing is fed into a final serializer later on.
         */
   ...
   * SerializerToXML#writeNormalizedChars because it does not attempt to do XML
   * escaping at all.
   * 
   * @param ch
   *          The characters from the XML document.
    ...
    for (int i = start; i < end; i++) {
      ...
      if (S_LINEFEED == c && useLineSep) {
        ...
        if (codePoint != 0) {
          ...
          final String msg = Utils.messages.createMessage(MsgKey.ER_ILLEGAL_CHARACTER, new Object[] { integralValue,
                  encoding });
   ...
   * invoke the other version of this method. %REVIEW% In fact, is this one ever
   * needed, or was it a mistake?
   * 
   * @param data
   *          The comment data.
   * @throws org.xml.sax.SAXException
