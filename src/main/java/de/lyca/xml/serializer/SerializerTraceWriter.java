...
final class SerializerTraceWriter extends Writer implements WriterChain {
  ...
  /**
   * Creates or replaces the internal buffer, and makes sure it has a few extra
   * bytes slight overflow of the last UTF8 encoded character.
   * 
   * @param size
   */
   ...
   * SerializerTraceWriter will only signal trace events of what would have been
   * written to that writer. If the writer passed in is not null then the trace
   * events will mirror what is going to that writer. In this way tools, such as
   * a debugger, can gather information on what is being written out.
   * 
   * @param out
  ...
  public void write(final char chars[], final int start, final int length) throws IOException {
    ...
    if (lengthx3 >= buf_length) {
       ...
       * output buffer and make the buffer bigger to handle.
       */
