...
final class CharInfo {
  ...
  /**
   * The name of the XML entities file. If specified, the file will be resource
   * loaded with the default class loader.
   */
  ...
  /**
   * Array of values is faster access than a set of bits to quickly check ASCII
   * characters in attribute values, the value is true if the character in an
   * attribute value should be mapped to a String.
   */
  private final boolean[] shouldMapAttrChar_ASCII;

  /**
   * Array of values is faster access than a set of bits to quickly check ASCII
   * characters in text nodes, the value is true if the character in a text node
   ...
   * information in this array is complete, the isSpecialAttrASCII array is used
   * first because access to its values is common and faster.
   */
   ...
   * element its bit is stored in. 5 for 32 bit words (int) , 6 for 64 bit words
   * (long)
   */
  ...
  /*
   * This is used for optimizing the lookup of bits representing the integers in
   * the set. It is the index of the first element in the array array_of_bits[]
   * that is not used.
   */
  private int firstWordNotUsed;

  /**
   * A base constructor just to explicitly create the fields, with the exception
   * of m_charToString which is handled by the constructor that delegates base
   * construction to this one.
   * <p>
   * m_charToString is not created here only for performance reasons, to avoid
   * creating a Hashtable that will be replaced when making a mutable copy,
   * {@link #mutableCopyOf(CharInfo)}.
   * 
   */
  private CharInfo() {
    array_of_bits = createEmptySetOfIntegers(65535);
    firstWordNotUsed = 0;
    shouldMapAttrChar_ASCII = new boolean[ASCII_MAX];
    shouldMapTextChar_ASCII = new boolean[ASCII_MAX];
  ...
  private CharInfo(String entitiesResource, String method, boolean internal) {
    ...
    if (entities != null) {
      ...
      try {
        ...
        if (is == null)
          throw new RuntimeException(Utils.messages.createMessage(MsgKey.ER_RESOURCE_COULD_NOT_FIND, new Object[] {
        ...
        throw new RuntimeException(Utils.messages.createMessage(MsgKey.ER_RESOURCE_COULD_NOT_LOAD, new Object[] {
    ...
    /*
     * Now that we've used get(ch) just above to initialize the two arrays we
     * will change by adding a tab to the set of special chars for XML (but not
     * HTML!). We do this because a tab is always a special character in an XML
     * attribute, but only a special character in XML text if it has an entity
     * defined for it. This is the reason for this delay.
     */
...
/**
  ...
  private boolean defineEntity(String name, char value) {
    ...
    final boolean extra = defineChar2StringMapping(entityString, value);
    return extra;
  }

  /**
   * A utility object, just used to map characters to output Strings, needed
   * because a HashMap needs to map an object as a key, not a Java primitive
   * type, like a char, so this object gets around that and it is reusable.
   */
   ...
   * Unfortunately, that introduces a threading risk. Simplest fix for now is to
   * make it a synchronized method, or to give up the reuse; I see very little
   * performance difference between them. Long-term solution would be to replace
   * the hashtable with a sparse array keyed directly from the character's
   * integer value; see DTM's string pool for a related solution.
   * 
   * @param value
   *          The character that should be resolved to a String, e.g. resolve
   *          '>' to "&lt;".
   * 
  ...
  /**
   * Tell if the character argument that is from an attribute value has a
   * mapping to a String.
   * 
   * @param value
...
/**
  ...
  private static CharInfo mutableCopyOf(CharInfo charInfo) {
    final CharInfo copy = new CharInfo();
  ...
  /**
   * Table of user-specified char infos. The table maps entify file names (the
   ...
   * 
   * @param i
   *          the integer to add to the set, valid values are 0, 1, 2 ... up to
   *          the maximum that was specified at the creation of the set.
   */
  private final void set(int i) {
    setASCIItextDirty(i);
    setASCIIattrDirty(i);

    final int j = i >> SHIFT_PER_WORD; // this word is used
    final int k = j + 1;
   ...
   * 
   * This implementation uses an array of integers with 32 bits per integer. If
   * a bit is set to 1 the corresponding integer is in the set of integers.
   * 
   * @param i
   *          an integer that is tested to see if it is the set of integers, or
   *          not.
   */
  private final boolean get(int i) {

    boolean in_the_set = false;
    final int j = i >> SHIFT_PER_WORD; // wordIndex(i)
...
/**
  ...
  /**
   * If the character is in the ASCII range then mark it as needing replacement
   * with a String on output if it occurs in a text node.
   * 
   * @param ch
   */
  ...
  /**
   * If the character is in the ASCII range then mark it as needing replacement
   * with a String on output if it occurs in a attribute value.
   * 
   * @param ch
   */
...
/**
  ...
  boolean defineChar2StringMapping(String outputString, char inputChar) {
    ...
    final boolean extraMapping = extraEntity(outputString, inputChar);
    return extraMapping;
