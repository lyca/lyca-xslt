 ...
 * </pre>
 * <p>
 * As an alternate to supplying a series of SAX events as input through the ContentHandler interface, the input to
 * serialize may be given as a DOM.
 * </p>
 * <p>
 * For example:
 * </p>
...
public interface Serializer {
   ...
   * <p>
   * The encoding specified in the output {@link Properties} is used, or if no encoding was specified, the default for
   ...
   * serializer is in the process of serializing a document.
   * <p>
   * The encoding specified for the output {@link Properties} must be identical to the output format used with the
   * writer.
   * 
   * <p>
   * Only one of setWriter() or setOutputStream() should be called.
   ...
   * <p>
   * The standard property keys supported are: "method", "version", "encoding", "omit-xml-declaration", "standalone",
   * doctype-public", "doctype-system", "cdata-section-elements", "indent", "media-type". These property keys and their
   * values are described in the XSLT recommendation, see <a href="http://www.w3.org/TR/1999/REC-xslt-19991116"> XSLT
   ...
   * 
   * <p>
   * This method can be called multiple times before a document is serialized. Each time it is called more, or
   * over-riding property values, can be specified. One property value that can not be changed is that of the "method"
   * property key.
   * </p>
   * <p>
   * The value of the "cdata-section-elements" property key is a whitespace separated list of elements. If the element
   * is in a namespace then value is passed in this format: {uri}localName
   * </p>
   * <p>
   * If the "cdata-section-elements" key is specified on multiple calls to this method the set of elements specified in
   * the value is not replaced from one call to the next, but it is cumulative across the calls.
   * </p>
   * 
  ...
  /**
   * Return a {@link ContentHandler} interface to provide SAX input to. Through the returned object the document to be
   * serailized, as a series of SAX events, can be provided to the serialzier. If the serializer does not support the
   * {@link ContentHandler} interface, it will return null.
   * <p>
  ...
  /**
   * Return a {@link DOMSerializer} interface into this serializer. Through the returned object the document to be
   * serialized, a DOM, can be provided to the serializer. If the serializer does not support the {@link DOMSerializer}
  ...
  /**
   * This method resets the serializer. If this method returns true, the serializer may be used for subsequent
   * serialization of new documents. It is possible to change the output format and output stream prior to serializing,
   * or to reuse the existing output format and output stream or writer.
   * 
  ...
  /**
   * Return an Object into this serializer to be cast to a DOM3Serializer. Through the returned object the document to
   * be serialized, a DOM (Level 3), can be provided to the serializer. If the serializer does not support casting to a
   ...
   * 
   * @return An Object to be cast to a DOM3Serializer interface into this serializer, or null if the serializer is not
   *         DOM capable
   * @throws IOException An I/O exception occured
   */
