...
public interface Serializer {
   ...
   * a document.
   * <p>
   * The encoding specified for the output {@link Properties} must be identical
   * to the output format used with the writer.
   * 
   * <p>
   * Only one of setWriter() or setOutputStream() should be called.
   * 
   ...
   * {@link <a href="http://www.w3.org/TR/1999/REC-xslt-19991116"> XSLT 1.0 recommendation</a>}
   * <p>
   ...
   * <p>
   * If the "cdata-section-elements" key is specified on multiple calls to this
   * method the set of elements specified in the value is not replaced from one
   * call to the next, but it is cumulative across the calls.
   * 
   * @param format
   ...
   * the returned object the document to be serailized, as a series of SAX
   * events, can be provided to the serialzier. If the serializer does not
   ...
   * returned object the document to be serialized, a DOM, can be provided to
   * the serializer. If the serializer does not support the
   ...
   * serializer may be used for subsequent serialization of new documents. It is
   * possible to change the output format and output stream prior to
   ...
   * Through the returned object the document to be serialized, a DOM (Level 3),
   * can be provided to the serializer. If the serializer does not support
   * casting to a {@link DOM3Serializer} interface, it should return null.
   * <p>
   * In principle only one of asDOM3Serializer() or asContentHander() should be
   * called.
   * 
   * @return An Object to be cast to a DOM3Serializer interface into this
   *         serializer, or null if the serializer is not DOM capable
   * @throws IOException
   *           An I/O exception occured
   */
