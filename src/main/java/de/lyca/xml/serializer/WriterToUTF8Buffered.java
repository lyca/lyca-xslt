...
final class WriterToUTF8Buffered extends Writer implements WriterChain {
  ...
  /**
   * number of bytes that the byte buffer can hold. This is a fixed constant is
   * used rather than m_outputBytes.lenght for performance.
   */
  private static final int BYTES_MAX = 16 * 1024;
  /**
   * number of characters that the character buffer can hold. This is 1/3 of the
   * number of bytes because UTF-8 encoding can expand one unicode character by
   * up to 3 bytes.
   */
  ...
  /**
   * The internal buffer where data is stored. (sc & sb remove final to compile
   * in JDK 1.1.8)
   */
   ...
   * <tt>0</tt> through <tt>m_outputBytes.length</tt>; elements
  ...
  public WriterToUTF8Buffered(OutputStream out) {
    ...
    // the old body of this constructor, before the buffersize was changed to a
    // constant
    // this(out, 8*1024);
  }
  ...
  public void write(final char chars[], final int start, final int length) throws java.io.IOException {
    ...
    if (lengthx3 >= BYTES_MAX - count) {
      ...
      if (lengthx3 > BYTES_MAX) {
        /*
         * The requested length exceeds the size of the buffer. Cut the buffer
         * up into chunks, each of which will not cause an overflow to the
         * output buffer m_outputBytes, and make multiple recursive calls. Be
         * careful about integer overflows in multiplication.
         */
        final int split = length / CHARS_MAX;
        final int chunks;
        ...
        for (int chunk = 1; chunk <= chunks; chunk++) {
          ...
          if (c >= 0xD800 && c <= 0xDBFF) {
            ...
            if (end_chunk < start + length) {
              ...
              /*
               * This is the last char of the last chunk, and it is the high
               * char of a high/low pair with no low char provided. TODO: error
               * message needed. The char array incorrectly ends in a high char
               * of a high/low surrogate pair, but there is no corresponding low
               * as the high is the last char
               */
              end_chunk--;
            }
          }
    ...
    {
      /*
       * This block could be omitted and the code would produce the same result.
       * But this block exists to give the JIT a better chance of optimizing a
       * tight and common loop which occurs when writing out ASCII characters.
       */
      char c;
    ...
    for (; i < n; i++) {
      ...
      /**
       * The following else if condition is added to support XML 1.1 Characters
       * for UTF-8: [1111 0uuu] [10uu zzzz] [10yy yyyy] [10xx xxxx]* Unicode:
       * [1101 10ww] [wwzz zzyy] (high surrogate) [1101 11yy] [yyxx xxxx] (low
       * surrogate) * uuuuu = wwww + 1
       */
  ...
  public void write(final String s) throws IOException {
    ...
    if (lengthx3 >= BYTES_MAX - count) {
      ...
      if (lengthx3 > BYTES_MAX) {
        /*
         * The requested length exceeds the size of the buffer, so break it up
         * in chunks that don't exceed the buffer size.
         */
        final int start = 0;
        final int split = length / CHARS_MAX;
        final int chunks;
        ...
        for (int chunk = 1; chunk <= chunks; chunk++) {
          ...
          if (c >= 0xD800 && c <= 0xDBFF) {
            ...
            if (chunk == chunks) {
              /*
               * TODO: error message needed. The String incorrectly ends in a
               * high char of a high/low surrogate pair, but there is no
               * corresponding low as the high is the last char Recover by
               * ignoring this last char.
               */
            }
          }
