...
public class ToHTMLStream extends ToStream {
  ...
  static void initTagReference(Trie m_elementFlags) {
    ...
    m_elementFlags.put("HR", new ElemDesc(0 | ElemDesc.BLOCK | ElemDesc.BLOCKFORM | ElemDesc.BLOCKFORMFIELDSET
            | ElemDesc.EMPTY));
    ...
    m_elementFlags
            .put("BLOCKQUOTE", new ElemDesc(0 | ElemDesc.BLOCK | ElemDesc.BLOCKFORM | ElemDesc.BLOCKFORMFIELDSET));
   ...
   * 
   * This method can be called multiple times before starting the serialization
   * of a particular result-tree. In principle all serialization parameters can
   * be changed, with the exception of method="html" (it must be method="html"
   * otherwise we shouldn't even have a ToHTMLStream object here!)
   * 
   * @param format
  ...
  public void setOutputFormat(Properties format) {
    ...
    /*
     * If "format" does not contain the property S_OMIT_META_TAG, then don't set
     * this value at all, just leave as-is rather than explicitly setting it.
     */
   ...
   * 
   * @return non-null reference to ElemDesc, which may be m_dummy if no element
   *         description matches the given name.
   */
  ...
  /**
   * A Trie that is just a copy of the "static" one. We need this one to be able
   * to use the faster, but not thread-safe method Trie.get2(name)
   */
  private final Trie m_htmlInfo = new Trie(m_elementFlags);

  /**
   * Calls to this method could be replaced with calls to getElemDesc(name), but
  ...
  /**
   * This method should only get called once. If a DOCTYPE declaration needs to
   * get written out, it will be written out. If it doesn't need to be written
   * out, then the call to this method has no effect.
   */
  ...
  public void startElement(String namespaceURI, String localName, String name, Attributes atts)
    ...
    try {
      ...
      final ElemDesc elemDesc = getElemDesc2(name);
      ...
      if ((elemFlags & ElemDesc.EMPTY) != 0) {
        ...
        /*
         * XSLTC sometimes calls namespaceAfterStartElement() so we need to
         * remember the name
         */
  ...
  public final void endElement(final String namespaceURI, final String localName, final String name)
    ...
    try {
      ...
      if (!elemContext.m_startTagOpen) {
        ...
        // the starting tag was still open when we received this endElement()
        // call
        // so we need to process any gathered attributes NOW, before they go
        // away.
  ...
  public void writeAttrURI(final java.io.Writer writer, String string, boolean doURLEscaping) throws IOException {
    ...
    for (int i = 0; i < end; i++) {
      ...
      if (ch < 32 || ch > 126) {
        ...
        if (doURLEscaping) {
          ...
          } else if (Encodings.isHighUTF16Surrogate(ch)) // high surrogate
          {
            ...
            // Clear high 6 bits.
            final int highSurrogate = ch & 0x03FF;
            ...
            final int wwww = (highSurrogate & 0x03C0) >> 6;
            final int uuuuu = wwww + 1;

            // next 4 bits
            final int zzzz = (highSurrogate & 0x003C) >> 2;
            ...
            // Clear high 6 bits.
            final int lowSurrogate = ch & 0x03FF;

            // put the middle 4 bits into the bottom of yyyyyy (byte 3)
            yyyyyy = yyyyyy | (lowSurrogate & 0x03C0) >> 6;

            // bottom 6 bits.
            final int xxxxxx = lowSurrogate & 0x003F;

            final int byte1 = 0xF0 | uuuuu >> 2; // top 3 bits of uuuuu
            final int byte2 = 0x80 | (uuuuu & 0x03) << 4 & 0x30 | zzzz;
            ...
            final int high = ch >> 12 | 0xE0; // top 4 bits
            final int middle = (ch & 0x0FC0) >> 6 | 0x80;
            // middle 6 bits
            final int low = ch & 0x3F | 0x80;
            // First 6 bits, + high bit
            writer.write('%');
        ...
        // In this character range we have first written out any previously
        // accumulated
        // "clean" characters, then processed the current more complicated
        // character,
      ...
      // identifier, we believe that it's OK to double escape quotes.
        ...
        // Dmitri Ilyin: to check if '%' number number is invalid. It must be
        // checked if %xx is a sign, that would be encoded
        // The encoded signes are in Hex form. So %xx my be in form %3C that is
        // "<" sign. I will try to change here a little.
        ...
        // We have written out any clean characters, then the escaped '%' and
        // now we
        // We now we reset the next possible clean character.
        cleanStart = i + 1;
      } else if (ch == '&') {
  ...
  public void writeAttrString(final java.io.Writer writer, String string, String encoding) throws IOException {
    ...
    for (int i = 0; i < end; i++) {
      ...
      if (escapingNotNeeded(ch) && !m_charInfo.shouldMapAttrChar(ch)) {
        ...
        final int pos = accumDefaultEntity(writer, ch, i, chars, end, false, true);
        ...
        if (i != pos) {
          ...
          /*
           * else if ((ch < m_maxCharacter) && (m_maxCharacter == 0xFFFF) && (ch
           * != 160)) { writer.write(ch); // no escaping in this case } else
           */
  ...
  public void processingInstruction(String target, String data) throws org.xml.sax.SAXException {
    ...
    if (target.equals(Result.PI_DISABLE_OUTPUT_ESCAPING)) {
      ...
      try {
        ...
        /*
         * Perhaps processing instructions can be written out in HTML before the
         * DOCTYPE, in which case this could be emitted with the startElement
  ...
  protected void closeStartTag() throws SAXException {
    try {
      ...
      /*
       * At this point we have the prefix mappings now, so lets determine if the
  ...
  /**
   * This method is used to add an attribute to the currently open element. The
   * caller has guaranted that this attribute is unique, which means that it not
   * been seen before and will not be seen again.
   * 
   * @param name
   *          the qualified name of the attribute
   * @param value
   *          the value of the attribute which can contain only ASCII printable
   *          characters characters in the range 32 to 127 inclusive.
   * @param flags
  ...
  public void comment(char ch[], int start, int length) throws SAXException {
    ...
    /*
     * Perhaps comments can be written out in HTML before the DOCTYPE. In this
     * case we might delete this call to writeOutDOCTYPE, and it would be
     * handled within the startElement() call.
     */
    if (m_needToOutputDocTypeDecl) {
      outputDocTypeDecl("html"); // best guess for the upcoming element
    }
  ...
  static class Trie {
     ...
     * java.util.Hashtable The key must be a 7-bit ASCII string The value may be
     * any Java Object One can get an object stored in a trie from its key, but
     * the search is either case sensitive or case insensitive to the characters
     * in the key, and this choice of sensitivity or insensitivity is made when
     ...
     * 
     * @param lowerCaseOnly
     *          true if the search keys are to be loser case only, not case
     *          insensitive.
     */
    ...
    /**
     * Construct the trie from another Trie. Both the existing Trie and this new
     * one share the same table for lookup, and it is assumed that the table is
     * fully populated and not changing anymore.
     * 
     * @param existingTrie
    ...
    /**
     * Get an object that matches the key. This method is faster than get(), but
     * is not thread-safe.
     * 
     * @param key
     *          must be a 7-bit ASCII string
    ...
    public Object get2(final String key) {
      ...
      // and a fair bit faster. Don't know why.
        ...
        default: {
          /*
           * Copy string into array. This is not thread-safe because it modifies
           * the contents of m_charBuffer. If multiple threads were to use this
           * Trie they all would be using this same array (not good). So this
           * method is not thread-safe, but it is faster because converting to a
           * char[] and looping over elements of the array is faster than a
           * String's charAt(i).
           */
