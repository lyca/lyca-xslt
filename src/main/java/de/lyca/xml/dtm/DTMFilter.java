...
public interface DTMFilter {
   ...
   * this case, it means that the attribute node will appear in the first
   * position of the iteration or traversal. Since attributes are never children
   * of other nodes, they do not appear when traversing over the main document
   * tree.
   */
   ...
   * have been fully expanded while the tree was being constructed, these nodes
   * will not appear and this mask has no effect.
   */
   ...
   * will appear in the first position of the traversal. Since entities are not
   * part of the document tree, they do not appear when traversing over the main
   * document tree.
   */
  ...
  /**
   * Show <code>Document</code> nodes. (Of course, as with Attributes and such,
   * this is meaningful only when the iteration root is the Document itself,
   * since Document has no parent.)
   */
  ...
  /**
   * Show <code>Notation</code> nodes. This is meaningful only when creating an
   ...
   * are not part of the document tree, they do not appear when traversing over
  ...
  /**
   * 
   * This bit instructs the iterator to show namespace nodes, which are modeled
   * by DTM but not by the DOM. Make sure this does not conflict with
   * {@link org.w3c.dom.traversal.NodeFilter}.
   * <p>
   * %REVIEW% Might be safer to start from higher bits and work down, to leave
   * room for the DOM to expand its set of constants... Or, possibly, to create
   * a DTM-specific field for these additional bits.
   */
   ...
   * considered are just as easy and just about as efficient if the name test is
   * performed in the DTMIterator... -- Joe
   * </p>
   * 
   * <p>
   * %REVIEW% Should that 0xFFFF have a mnemonic assigned to it? Also: This
   * representation is assuming the expanded name is indeed split into high/low
   * 16-bit halfwords. If we ever change the balance between namespace and
   * localname bits (eg because we decide there are many more localnames than
   * namespaces, which is fairly likely), this is going to break. It might be
   * safer to encapsulate the details with a makeExpandedName method and make
   ...
   *          one of SHOW_XXX values.
   * @param expandedName
   *          a value defining the exanded name as defined in the DTM interface.
   *          Wild cards will be defined by 0xFFFF in the namespace and/or
   *          localname portion of the expandedName.
   * @return one of FILTER_ACCEPT, FILTER_REJECT, or FILTER_SKIP.
   */
