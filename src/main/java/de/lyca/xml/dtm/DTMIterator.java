...
public interface DTMIterator {
   ...
   * created. Note the root node is not the root node of the document tree, but
   * the context node from where the iteration begins and ends.
   * 
   ...
   *          below)</li>
   *          <li>a pair of non-zero positive integers (the context position and
   *          the context size)</li>
   ...
   *          application dependent. Probably a proper interface will be created
   *          fairly soon.
   *          </p>
   * 
   */
   ...
   * will be the root node -- or, if that's filtered out, the first node within
   * the root's subtree which is _not_ skipped by the filters.
   */
  public void reset();

  /**
   * This attribute determines which node types are presented via the iterator.
   * The available set of constants is defined above. Nodes not accepted by
   ...
   * entity expansion, use the <code>whatToShow</code> flags to show the entity
   ...
   * in the set. After a <code>DTMIterator</code> has setRoot called, the first
   * call to <code>nextNode()</code> returns that root or (if it is rejected by
   * the filters) the first node within its subtree which is not filtered out.
   * 
  ...
  /**
   * Get the current node in the iterator. Note that this differs from the DOM's
   * NodeIterator, where the current position lies between two nodes (as part of
   * the maintain-relative-position semantic).
   * 
  ...
  /**
   * If setShouldCacheNodes(true) is called, then nodes will be cached, enabling
   * random access, and giving the ability to do sorts and the like. They are
  ...
  /**
   * Get the current position within the cached list, which is one less than the
   * next nextNode() call will retrieve. i.e. if you call getCurrentPos() and
   * the return is 0, the next fetch will take place at index 1.
   * 
   ...
   * 
   * @param index
   *          The index to run to, or -1 if the iterator should be run to the
   *          end.
   */
   ...
   * to <code>length-1</code> inclusive. Note that this requires running the
   ...
   * so that it may be used from the current position.
   * 
