...
public abstract class DTMAxisTraverser {
   ...
   * returned or the iteration will go into an infinate loop. So to traverse an
   * axis, the first function must be used to get the first node.
   * 
   * <p>
   * This method needs to be overloaded only by those axis that process the self
   * node. <\p>
   * 
   * @param context
   ...
   *          for the traversal -- its "root node" or starting point.
   * @param current
   *          The current node of the traversal. This is the last known location
   *          in the traversal, typically the node-handle returned by the
