...
final class ChunkedIntArray {
  ...
  /**
   * Create a new CIA with specified record size. Currently record size MUST be
   * a power of two... and in fact is hardcoded to 4.
   */
  ChunkedIntArray(int slotsize) {
    if (ChunkedIntArray.slotsize < slotsize)
      throw new ArrayIndexOutOfBoundsException(XMLMessages.createXMLMessage(
              XMLErrorResources.ER_CHUNKEDINTARRAY_NOT_SUPPORTED, new Object[] { Integer.toString(slotsize) })); // "ChunkedIntArray("+slotsize+") not currently supported");
  ...
  /**
   * Append a 4-integer record to the CIA, starting with record 1. (Since arrays
   * are initialized to all-0, 0 has been reserved as the "unknown" value in
   * DTM.)
   * 
  ...
  int appendSlot(int w0, int w1, int w2, int w3) {
    /*
     * try { int newoffset = (lastUsed+1)*slotsize; fastArray[newoffset] = w0;
     * fastArray[newoffset+1] = w1; fastArray[newoffset+2] = w2;
   ...
   * record, both 0-based (though position 0 is reserved for special purposes).
   * 
   * @param position
   *          int Record number
   * @param slotpos
  ...
  int readEntry(int position, int offset) throws ArrayIndexOutOfBoundsException {
     ...
     */
    {
      ...
      if (offset >= slotsize)
        throw new ArrayIndexOutOfBoundsException(XMLMessages.createXMLMessage(
                XMLErrorResources.ER_OFFSET_BIGGER_THAN_SLOT, null)); // "Offset bigger than slot");
      position *= slotsize;
  ...
  /**
   * Disard the highest-numbered record. This is used in the string-buffer CIA;
   * when only a single characters() chunk has been recieved, its index is moved
   * into the Text node rather than being referenced by indirection into the
   * text accumulator.
   */
  void discardLast() {
    --lastUsed;
  ...
  /**
   * Overwrite an entire (4-integer) record at the specified index. Mostly used
   * to create record 0, the Document node.
   * 
   * @param position
   ...
   * to reduce addressing overhead when code will access several columns of the
   * record.
   * 
   * @param position
   *          int Record number
   * @param buffer
   *          int[] Integer array provided by user, must be large enough to hold
   *          a complete record.
   */
