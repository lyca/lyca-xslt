...
public class DTMNamedNodeMap implements NamedNodeMap {
  ...
  /**
   * Returns the <code>index</code>th item in the map. If <code>index</code> is
  ...
  /**
   * Adds a node using its <code>nodeName</code> attribute. If a node with that
   * name is already present in this map, it is replaced by the new one. <br>
   * As the <code>nodeName</code> attribute is used to derive the name which the
   * node must be stored under, multiple nodes of certain types (those that have
   ...
   * 
   * @param newNode
   *          node to store in this map. The node will later be accessible using
   *          the value of its <code>nodeName</code> attribute.
   * 
   ...
   * well as the corresponding namespace URI, local name, and prefix when
   * applicable.
   * 
   * @param name
   ...
   * 
   * @return A <code>Node</code> (of any type) with the specified local name and
   *         namespace URI, or <code>null</code> if they do not identify any
   *         node in this map.
   * @since DOM Level 2
   */
  @Override
   ...
   * this map, it is replaced by the new one. <br>
   ...
   * of the <code>Node</code> interface. If so, an attribute immediately appears
