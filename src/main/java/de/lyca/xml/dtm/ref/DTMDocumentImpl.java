...
public class DTMDocumentImpl implements DTM, ContentHandler, LexicalHandler {
  ...
  // nodes array: integer array blocks to hold the first level reference of the
  // nodes,
  ...
  private final ExpandedNameTable m_expandedNames = new ExpandedNameTable();
   ...
   * 
   * @param source
   *          The IncrementalSAXSource that we want to recieve events from on
   *          demand.
   */
  ...
  private final int appendNode(int w0, int w1, int w2, int w3) {
    ...
    final int slotnumber = nodes.appendSlot(w0, w1, w2, w3);
   ...
   * this really be Public? Changing it while DTM is in use would be a disaster.
   * 
   * @param poolRef
  ...
  /**
   * Set a reference pointer to the namespace URI symbol table. %REVIEW% Should
   * this really be Public? Changing it while DTM is in use would be a disaster.
   * 
   * @param poolRef
   *          DTMStringPool reference to an instance of table.
  ...
  public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
    ...
    for (int i = nAtts - 1; i >= 0; --i) {
      ...
      if (qName.startsWith("xmlns:") || "xmlns".equals(qName)) {
        ...
        appendNSDeclaration(m_prefixNames.stringToIndex(prefix), m_nsNames.stringToIndex(atts.getValue(i)), atts
                .getType(i).equalsIgnoreCase("ID"));
      }
    }
    ...
    for (int i = nAtts - 1; i >= 0; --i) {
      ...
      if (!(qName.startsWith("xmlns:") || "xmlns".equals(qName))) {
        ...
        final int contentEnd = m_char.length();

        if (!("xmlns".equals(prefix) || "xmlns".equals(qName))) {
          appendAttribute(m_nsNames.stringToIndex(atts.getURI(i)), m_localNames.stringToIndex(localName),
                  m_prefixNames.stringToIndex(prefix), atts.getType(i).equalsIgnoreCase("ID"), m_char_current_start,
  ...
  // application
  // /**
  // * Factory method; creates an Element node not associated with a given name
  ...
  // interface.
  // *
  // * <p>The XML content handler or application will invoke endElement() method
  // after all
  // * of the element's content are processed in order to give DTM the
  // indication
  // * to prepare and patch up parent and sibling node pointers.</p>
  // *
  // * <p>The following parameters for createElement contains raw string values
  // for name
  ...
  // //w0 = ATTRIBUTE_NODE |
  // // (de.lyca.xalan.templates.Constants.S_XMLNAMESPACEURI << 16);
  // } else {
  // //w0 = ATTRIBUTE_NODE |
  // }
  // } else {
  ...
  // // Counters to keep track of the current text string being accumulated with
  // respect
  // // to the text/comment string table: charStringStart should point to the
  // starting
  // // offset of the string in the table and charStringLength the acccumulated
  // length when
  // // appendAccumulatedText starts, and reset to the end of the table and 0 at
  // the end
   ...
   * testing this without having to create a proxy node. It's less useful in the
   * DTM API, where (dtm.getFirstChild(nodeHandle)!=DTM.NULL) is just as fast
   * and almost as self-evident. But it's a convenience, and eases porting of
   * DOM code to DTM.
   * </p>
   * 
   * @param nodeHandle
  ...
  public int getFirstChild(int nodeHandle) {
    ...
    final short type = (short) (gotslot[0] & 0xFFFF);
    ...
    if (type == ELEMENT_NODE || type == DOCUMENT_NODE || type == ENTITY_REFERENCE_NODE) {
      ...
      if (gotslot[1] == nodeHandle) {
        final int firstChild = kid | m_docHandle;
  ...
  /**
   * Given a node handle, advance to its last child. If not yet resolved, waits
   * for more nodes to be added to the document and tries again.
   * 
   * @param nodeHandle
   *          int Handle of the node.
  ...
  public int getAttributeNode(int nodeHandle, String namespaceURI, String name) {
    final int nsIndex = m_nsNames.stringToIndex(namespaceURI), nameIndex = m_localNames.stringToIndex(name);
    nodeHandle &= NODEHANDLE_MASK;
    nodes.readSlot(nodeHandle, gotslot);
    final short type = (short) (gotslot[0] & 0xFFFF);
   ...
   * 
   * @param nodeHandle
   *          handle to node, which should probably be an element node, but need
   *          not be.
   * 
   * @param inScope
   *          true if all namespaces in scope should be returned, false if only
   *          the namespace declarations should be returned.
   ...
   * layer provided the additional logic for the public view. If we're rewriting
   * for XPath emulation, that test must be done here.
   * 
   * %TBD% CODE INTERACTION WITH INCREMENTAL PARSE - If not yet resolved, should
   * wait for more nodes to be added to the document and tries again.
   * 
   * @param nodeHandle
   *          int Handle of the node.
  ...
  public int getNextSibling(int nodeHandle) {
    ...
    final short type = (short) (nodes.readEntry(nodeHandle, 0) & 0xFFFF);
    if (type == ELEMENT_NODE || type == ATTRIBUTE_NODE || type == ENTITY_REFERENCE_NODE) {
      final int nextSib = nodes.readEntry(nodeHandle, 2);
      if (nextSib == NULL)
        return NULL;
      if (nextSib != 0)
    ...
    final int thisParent = nodes.readEntry(nodeHandle, 1);
  ...
  public int getPreviousSibling(int nodeHandle) {
    ...
    final int parent = nodes.readEntry(nodeHandle, 1);
    int kid = NULL;
   ...
   * advance to its first attribute; if an attr, we advance to the next attr on
   * the same node.
   * 
   * @param nodeHandle
   *          int Handle of the node.
      ...
      if (nodeHandle > subtreeRootHandle) {
        ...
        if (gotslot[2] != 0) {
          ...
          if (type == ATTRIBUTE_NODE) {
            ...
            final int nextParentPos = gotslot[1];
   ...
   * @param nodeHandle
   *          the id of the node.
   * @return int Node-number of preceding sibling, or DTM.NULL to indicate none
   *         exists.
   */
  ...
  /**
   * Given a node handle, find the owning document node. This has the exact same
   * semantics as the DOM Document method of the same name, in that if the
   * nodeHandle is a document node, it will return NULL.
   * 
   * <p>
   ...
   * @param nodeHandle
   *          the id of the node.
   * @return int Node handle of owning document, or NULL if the nodeHandle is a
   *         document.
   */
  @Override
  ...
  public XMLString getStringValue(int nodeHandle) {
    // ###zaj - researching
    nodes.readSlot(nodeHandle, gotslot);
    final int nodetype = gotslot[0] & 0xFF;
    String value = null;
   ...
   * http://www.w3.org/TR/xpath#data-model for the definition of a node's
   * string-value). Note that a single text node may have multiple text chunks.
   * 
   * EXPLANATION: This method is an artifact of the fact that the underlying
   * m_chars object may not store characters in a single contiguous array -- for
   * example,the current FastStringBuffer may split a single node's text across
   * multiple allocation units. This call tells us how many separate accesses
   * will be required to retrieve the entire content. PLEASE NOTE that this may
   * not be the same as the number of SAX characters() events that caused the
   * text node to be built in the first place, since m_chars buffering may be on
   ...
   * m_chars object may not store characters in a single contiguous array -- for
   * example,the current FastStringBuffer may split a single node's text across
   * multiple allocation units. This call retrieves a single contiguous portion
   * of the text -- as much as m-chars was able to store in a single allocation
   * unit. PLEASE NOTE that this may not be the same granularityas the SAX
   * characters() events that caused the text node to be built in the first
   * place, since m_chars buffering may be on different boundaries than the
   * parser's buffers.
   * 
   * @param nodeHandle
   *          The node ID.
  ...
  public int getExpandedTypeID(int nodeHandle) {
    nodes.readSlot(nodeHandle, gotslot);
    final String qName = m_localNames.indexToString(gotslot[3]);
    ...
    // Get NS
    final String namespace = m_nsNames.indexToString(gotslot[0] << 16);
    // Create expanded name
    final String expandedName = namespace + ":" + localName;
    final int expandedNameID = m_nsNames.stringToIndex(expandedName);
  ...
  public String getLocalNameFromExpandedNameID(int ExpandedNameID) {
    ...
    // Get expanded name
    final String expandedName = m_localNames.indexToString(ExpandedNameID);
   ...
   * 
   * %REVIEW% What's the local name of something other than Element/Attr? Should
   * this be DOM-style (undefined unless namespaced), or other?
   * 
   * @param nodeHandle
   *          the id of the node.
   * @return String Local name of this node.
  ...
  public String getNodeValue(int nodeHandle) {
    nodes.readSlot(nodeHandle, gotslot);
    final int nodetype = gotslot[0] & 0xFF; // ###zaj use mask to get node type
    String value = null;
  ...
  /**
   * Tests whether DTM DOM implementation implements a specific feature and that
   * feature is supported by this node.
   * 
   * @param feature
   *          The name of the feature to test.
   * @param version
   *          This is the version number of the feature to test. If the version
   *          is not specified, supporting any version of the feature will cause
   *          the method to return <code>true</code>.
   * @return Returns <code>true</code> if the specified feature is supported on
  ...
  /**
   * Return the base URI of the document entity. If it is not known (because the
   * document was parsed from a socket connection or from standard input, for
   * example), the value of this property is unknown.
   * 
  ...
  /**
   * Return an indication of the standalone status of the document, either "yes"
   * or "no". This property is derived from the optional standalone document
   * declaration in the XML declaration at the beginning of the document entity,
   * and has no value if there is no standalone document declaration.
   * 
   * @param nodeHandle
   *          The node id, which can be any valid node handle.
  ...
  /**
   * Return a string representing the XML version of the document. This property
   * is derived from the XML declaration optionally present at the beginning of
   * the document entity, and has no value if there is no XML declaration.
   * 
   ...
   * Its value is a boolean. If it is false, then certain properties (indicated
   * in their descriptions below) may be unknown. If it is true, those
   * properties are never unknown.
   * 
   ...
   * document than nodeHandle1, in Document Order as defined by the XPath model.
  ...
  /**
   * 2. [element content whitespace] A boolean indicating whether the character
   * is white space appearing within element content (see [XML], 2.10
   * "White Space Handling"). Note that validating XML processors are required
   * by XML 1.0 to provide this information. If there is no declaration for the
   * containing element, this property has no value for white space characters.
   ...
   * unread declaration), then the value of this property is unknown for white
   * space characters. It is always false for characters that are not white
   * space.
   * 
   * @param nodeHandle
   *          the node ID.
   ...
   * part of the infoset of the document. Rather it is an indication of whether
   * the processor has read the complete DTD. Its value is a boolean. If it is
   * false, then certain properties (indicated in their descriptions below) may
  ...
  public void appendChild(int newChild, boolean clone, boolean cloneDepth) {
    final boolean sameDoc = (newChild & DOCHANDLE_MASK) == m_docHandle;
    if (clone || !sameDoc) {
  ...
  /**
   * Append a text node child that will be constructed from a string, to the end
   * of the document.
   * 
   * <p>
   * %REVIEW% "End of the document" needs to be defined more clearly. Does it
  ...
  /**
   * Append a text child at the current insertion point. Assumes that the actual
   * content of the text has previously been appended to the m_char buffer
  ...
  void appendTextChild(int m_char_current_start, int contentLength) {
    ...
    // %TBD% may be possible to combine with appendNode()to replace the next
    // chunk of code
    final int w0 = TEXT_NODE;
    // W1: Parent
    final int w1 = currentParent;
    // W2: Start position within m_char
    final int w2 = m_char_current_start;
    // W3: Length of the full string
    final int w3 = contentLength;

    final int ourslot = appendNode(w0, w1, w2, w3);
    previousSibling = ourslot;
  }
   ...
   * 
   * Note that this _only_ handles the Element node itself. Attrs and namespace
   * nodes are unbundled in the ContentHandler layer and appended separately.
   * 
  ...
  void appendStartElement(int namespaceIndex, int localNameIndex, int prefixIndex) {
    ...
    // W0 High: Namespace Low: Node Type
    final int w0 = namespaceIndex << 16 | ELEMENT_NODE;
    // W1: Parent
    final int w1 = currentParent;
    // W2: Next (initialized as 0)
    final int w2 = 0;
    // W3: Tagname high: prefix Low: local name
    final int w3 = localNameIndex | prefixIndex << 16;
   ...
   * Assumes that the symbols (namespace URI and prefix) have already been added
   * to the pools
   * 
   * @param prefixIndex
   *          : Index within the prefix string pool
   ...
   *          declared as being a node ID. I don't really want to support that
   *          stupidity, but I'm not sure we can refuse to accept it.
   * */
  void appendNSDeclaration(int prefixIndex, int namespaceIndex, boolean isID) {
    ...
    // W0 High: Namespace Low: Node Type
    final int w0 = NAMESPACE_NODE | m_nsNames.stringToIndex("http://www.w3.org/2000/xmlns/") << 16;
  ...
  /**
   * Append an Attribute child at the current insertion point. Assumes that the
   * symbols (namespace URI, local name, and prefix) have already been added to
   * the pools, and that the content has already been appended to m_char. Note
   * that the attribute's content has been flattened into a single string; DTM
   * does _NOT_ attempt to model the details of entity references within
   * attribute values.
   * 
   * @param namespaceIndex
   ...
   * @param isID
   *          boolean True if this attribute was declared as an ID (for use in
   *          supporting getElementByID).
   * @param m_char_current_start
   *          int Starting offset of node's content in m_char.
   * @param contentLength
   *          int Length of node's content in m_char.
   * */
  void appendAttribute(int namespaceIndex, int localNameIndex, int prefixIndex, boolean isID, int m_char_current_start,
          int contentLength) {
    // %TBD% isID is not currently honored.
  ...
  /**
   * This returns a stateless "traverser", that can navigate over an XPath axis,
   * though not in document order.
   * 
   * @param axis
  ...
  /**
   * This is a shortcut to the iterators that implement the supported XPath axes
   * (only namespace::) is not supported. Returns a bare-bones iterator that
  ...
  /**
   * A dummy routine to satisify the abstract interface. If the DTM
   * implememtation that extends the default base requires notification when the
   * document is being released, they can override this method
   */
  @Override
