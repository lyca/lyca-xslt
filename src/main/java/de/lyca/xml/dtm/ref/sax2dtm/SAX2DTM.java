...
public class SAX2DTM extends DTMDefaultBaseIterators implements EntityResolver, DTDHandler, ContentHandler,
  ...
  public SAX2DTM(DTMManager mgr, Source source, int dtmIdentity, DTMWSFilter whiteSpaceFilter,
    ...
    m_data.addElement(0); // Need placeholder in case index into here must be
                          // <0.

    // m_dataOrQName = new SuballocatedIntVector(blocksize);

    // m_useSourceLocationProperty=de.lyca.xalan.processor.TransformerFactoryImpl.m_source_location;
  ...
  public void dispatchCharactersEvents(int nodeHandle, ContentHandler ch, boolean normalize) throws SAXException {
    ...
    if (isTextType(type)) {
      ...
      final int offset = m_data.elementAt(dataIndex);
      final int length = m_data.elementAt(dataIndex + 1);
      ...
      if (DTM.NULL != firstChild) {
        ...
        final String str = m_valuesOrPrefixes.indexToString(dataIndex);
  ...
  public String getNodeName(int nodeHandle) {
    ...
    final int expandedTypeID = getExpandedTypeID(nodeHandle);
    // If just testing nonzero, no need to shift...
    final int namespaceID = m_expandedNameTable.getNamespaceID(expandedTypeID);
  ...
  public String getNodeNameX(int nodeHandle) {
    ...
    if (0 == namespaceID) {
      final String name = m_expandedNameTable.getLocalName(expandedTypeID);
  ...
  protected boolean nextNode() {
    ...
    final Object gotMore = m_incrementalSAXSource.deliverMoreNodes(true);
  ...
  public void migrateTo(DTMManager manager) {
    ...
    final int numDTMs = m_dtmIdent.size();
  ...
  protected void setSourceLocation() {
    ...
    if (m_sourceSystemId.size() != m_size) {
      final String msg = "CODING ERROR in Source Location: " + m_size + " != " + m_sourceSystemId.size();
  ...
  public String getPrefix(int nodeHandle) {
    ...
    if (DTM.ELEMENT_NODE == type) {
      final int prefixIndex = _dataOrQName(identity);

      if (0 == prefixIndex)
        return "";
      else {
        final String qname = m_valuesOrPrefixes.indexToString(prefixIndex);
  ...
  public int getAttributeNode(int nodeHandle, String namespaceURI, String name) {
    ...
    for (int attrH = getFirstAttribute(nodeHandle); DTM.NULL != attrH; attrH = getNextAttribute(attrH)) {
      final String attrNS = getNamespaceURI(attrH);
      final String attrName = getLocalName(attrH);
  ...
  protected void charactersFlush() {
    ...
    if (m_textPendingStart >= 0) // -1 indicates no-text-in-progress
    {
      ...
      if (doStrip) {
        ...
        if (length > 0) {
          final int exName = m_expandedNameTable.getExpandedTypeID(DTM.TEXT_NODE);
  ...
  public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
    ...
    if (!m_pastFirstElement) {
      ...
      final int val = m_valuesOrPrefixes.stringToIndex(declURL);
  ...
  public void processingInstruction(String target, String data) throws SAXException {
    ...
    final int exName = m_expandedNameTable.getExpandedTypeID(null, target, DTM.PROCESSING_INSTRUCTION_NODE);
    final int dataIndex = m_valuesOrPrefixes.stringToIndex(data);
  ...
  public void comment(char ch[], int start, int length) throws SAXException {
    ...
    final int exName = m_expandedNameTable.getExpandedTypeID(DTM.COMMENT_NODE);

    // For now, treat comments as strings... I guess we should do a
    // seperate FSB buffer instead.
    final int dataIndex = m_valuesOrPrefixes.stringToIndex(new String(ch, start, length));
