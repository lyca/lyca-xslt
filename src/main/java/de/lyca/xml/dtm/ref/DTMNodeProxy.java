...
public class DTMNodeProxy implements Node, Document, Text, Element, Attr, ProcessingInstruction, Comment,
  ...
  public final boolean equals(Node node) {
    ...
    try {
      ...
      return dtmp.node == this.node && dtmp.dtm == dtm;
  ...
  public final boolean sameNodeAs(Node other) {
    ...
    return dtm == that.dtm && node == that.node;
  }
   ...
   * should always return false. Or we could say that we support DOM Core Level
   * 2 but all nodes are read-only. Unclear which answer is least misleading.
   * 
   ...
   */
  private final void traverseChildren(List<Node> listVector, Node tempNode, String tagname, boolean isTagNameWildCard) {
    if (tempNode == null)
      return;
    else {
  ...
  static class DTMNodeProxyImplementation implements DOMImplementation {
     ...
     * 
     * In fact, we do not _fully_ support any DOM feature -- we're a read-only
     * subset -- so arguably we should always return false. On the other hand,
     * it may be more practically useful to return true and simply treat the
     * whole DOM as read-only, failing on the methods we can't support. I'm not
     * sure which would be more useful to the caller.
     */
    @Override
     ...
     *          version is <code>null</code> or the empty string, supporting any
  ...
  /**
   * This method returns a specialized object which implements the specialized
   * APIs of the specified feature and version. The specialized object may also
   * be obtained by using binding-specific casting methods but is not
   * necessarily expected to, as discussed in Mixed DOM implementations.
   * 
   * @param feature
   *          The name of the feature requested (case-insensitive).
   ...
   *          is <code>null</code> or the empty string, supporting any version
   *          of the feature will cause the method to return an object that
   *          supports at least one version of the feature.
   * @return Returns an object which implements the specialized APIs of the
   *         specified feature and version, if any, or <code>null</code> if
   ...
   * The two nodes are of the same type.The following string attributes are
   * equal: <code>nodeName</code>, <code>localName</code>,
   ...
   * <code>attributes</code> <code>NamedNodeMaps</code> are equal. This is: they
   * are both <code>null</code>, or they have the same length and for each node
   * that exists in one map there is a node that exists in the other map and is
   * equal, although not necessarily at the same index.The
   * <code>childNodes</code> <code>NodeLists</code> are equal. This is: they are
   * both <code>null</code>, or they have the same length and contain equal
   * nodes at the same index. This is true for <code>Attr</code> nodes as for
   * any other type of node. Note that normalization can affect equality; to
   * avoid this, nodes should be normalized before being compared. <br>
  ...
  public boolean isEqualNode(Node arg) {
    ...
    if (getNodeValue() == null) {
      ...
      return false;
    /*
     * if (getBaseURI() == null) { if (((NodeImpl) arg).getBaseURI() != null) {
     * return false; } } else if (!getBaseURI().equals(((NodeImpl)
  ...
  public String lookupNamespaceURI(String specifiedPrefix) {
    ...
    switch (type) {
      case Node.ELEMENT_NODE: {
         ...
         * != null) { return ancestor.lookupNamespaceURI(specifiedPrefix); }
         */
        return null;
      }
      /*
  ...
  public boolean isDefaultNamespace(String namespaceURI) {
     ...
     * 
     * NodeImpl ancestor = (NodeImpl)getElementAncestor(this); if (ancestor !=
     * null) { return ancestor.isDefaultNamespace(namespaceURI); } return false;
     * } case Node.DOCUMENT_NODE:{
     * return((NodeImpl)((Document)this).getDocumentElement
  ...
  public String lookupPrefix(String namespaceURI) {
     ...
     */
      ...
      default: {
        /*
         * NodeImpl ancestor = (NodeImpl)getElementAncestor(this); if (ancestor
         * != null) { return ancestor.lookupPrefix(namespaceURI); }
         */
        return null;
      }
    }
   ...
   * two <code>Node</code> references are references to the same object, even if
   ...
   * contain any markup. No whitespace normalization is performed, the returned
   * string does not contain the element content whitespaces . Similarly, on
   * setting, no parsing is performed either, the input string is taken as pure
   * textual content. <br>
   ...
   * supports the feature "HTML" , the base URI is computed using first the
   * value of the href attribute of the HTML BASE element if any, and the value
   * of the <code>documentURI</code> attribute from the <code>Document</code>
   ...
   * properties [base URI] defined in . When the node is a <code>Notation</code>
   * , an <code>Entity</code>, or an <code>EntityReference</code>, this
   * attribute represents the properties [declaration base URI] in the . How
   * will this be affected by resolution of relative namespace URIs issue?It's
   * not.Should this only be on Document, Element, ProcessingInstruction,
   * Entity, and Notation nodes, according to the infoset? If not, what is it
   * equal to on other nodes? Null? An empty string? I think it should be the
   * parent's.No.Should this be read-only and computed or and actual read-write
   * attribute?Read-only and computed (F2F 19 Jun 2000 and teleconference 30 May
   * 2001).If the base HTML element is not yet attached to a document, does the
   * insert change the Document.baseURI? Yes. (F2F 26 Sep 2001)
   * 
   * @since DOM Level 3
   */
  @Override
  ...
  public Text replaceWholeText(String content) throws DOMException {
     ...
     * check if node in the tree parent.removeChild(this); return null; } } Text
     * currentNode = null; if (isReadOnly()){ Text newNode =
     * this.ownerDocument().createTextNode(content); if (parent !=null) { //
     * check if node in the tree parent.insertBefore(newNode, this);
     * parent.removeChild(this); currentNode = newNode; } else { return newNode;
     * } } else { this.setData(content); currentNode = this; } Node sibling =
     * currentNode.getNextSibling(); while ( sibling !=null) {
  ...
  /**
   * DOM Level 3 Returns all text of <code>Text</code> nodes logically-adjacent
