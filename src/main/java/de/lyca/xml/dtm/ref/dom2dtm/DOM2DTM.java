...
public class DOM2DTM extends DTMDefaultBaseIterators {
   ...
   * (for example, if logically-adjacent Text/CDATASection nodes in the DOM have
   ...
   * @param forceNodeType
   *          If not DTM.NULL, overrides the DOM node type. Used to force nodes
   *          to Text rather than CDATASection when their coalesced value
   *          includes ordinary Text nodes (current DTM behavior).
   * 
  ...
  protected int addNode(Node node, int parentIndex, int previousSibling, int forceNodeType) {
    ...
    final int expandedNameID = null != localName ? exnt.getExpandedTypeID(nsURI, localName, type) : exnt
            .getExpandedTypeID(type);
  ...
  /**
   * This method iterates to the next node that will be added to the table. Each
   * call to this method adds a new node to the table, unless the end is
      ...
      // If that fails, look up and right (but not past root!)
        ...
        while (m_last_parent != NULL) {
          ...
          if (pos != null && ENTITY_REFERENCE_NODE == pos.getNodeType()) {
            ...
            // Fix and pop DTM
            if (m_last_kid == NULL) {
              m_firstch.setElementAt(NULL, m_last_parent); // Popping from an
                                                           // element
            } else {
              m_nextsib.setElementAt(NULL, m_last_kid); // Popping from anything
                                                        // else
            }
    ...
    if (!suppressNode) {
      ...
      if (ELEMENT_NODE == nexttype) {
        ...
        if (!m_processedFirstElement) {
          ...
          attrIndex = addNode(new DOM2DTMdefaultNamespaceDeclarationNode((Element) next, "xml", NAMESPACE_DECL_NS,
                  makeNodeHandle((attrIndex == NULL ? nextindex : attrIndex) + 1)), nextindex, attrIndex, NULL);
          m_firstch.setElementAt(DTM.NULL, attrIndex);
          m_processedFirstElement = true;
        }
        if (attrIndex != NULL) {
   ...
   * large documents. That might also imply improving this call to handle nodes
   * which would be in this DTM but have not yet been built, which might or
   ...
   * doesn't work. DOM Level 3 will provide the isSameNode() method to fix that,
   * but until then this is going to be flaky.
   * 
   * @param node
  ...
  public int getHandleOfNode(Node node) {
    if (null != node) {
      ...
      if (m_root == node || m_root.getNodeType() == DOCUMENT_NODE && m_root == node.getOwnerDocument()
              || m_root.getNodeType() != DOCUMENT_NODE && m_root.getOwnerDocument() == node.getOwnerDocument()) {
        // If node _is_ in m_root's tree, find its handle
        //
        // %OPT% This check may be improved significantly when DOM
        // Level 3 nodeKey and relative-order tests become
        // available!
        for (Node cursor = node; cursor != null; cursor = cursor.getNodeType() != ATTRIBUTE_NODE ? cursor
                .getParentNode() : ((org.w3c.dom.Attr) cursor).getOwnerElement()) {
          if (cursor == m_root)
   ...
   * <p>
   * %REVIEW% Actually, since this method operates on the DOM side of the fence
   * rather than the DTM side, it SHOULDN'T do any special handling. The DOM
   * does what the DOM does; if you want DTM-level abstractions, use DTM-level
   * methods.
   * 
   * @param node
  ...
  /**
   * Utility function: Given a DOM Text node, determine whether it is logically
   * followed by another Text or CDATASection node. This may involve traversing
   * into Entity References.
   * 
   * %REVIEW% DOM Level 3 is expected to add functionality which may allow us to
   * retire this.
   */
  ...
  public int getElementById(String elementId) {
    ...
    final Document doc = m_root.getNodeType() == Node.DOCUMENT_NODE ? (Document) m_root : m_root.getOwnerDocument();
  ...
  public boolean isAttributeSpecified(int attributeHandle) {
    final int type = getNodeType(attributeHandle);

    if (DTM.ATTRIBUTE_NODE == type) {
      final Attr attr = (Attr) getNode(attributeHandle);
   ...
   *         object has a built-in SAX ContentHandler, the IncrmentalSAXSource
  ...
  /**
   * No source information is available for DOM2DTM, so return <code>null</code>
   * here.
   * 
   * @param node
