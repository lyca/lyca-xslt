...
public class SAX2RTFDTM extends SAX2DTM {
   ...
   * don't think number of NS sets is ever different from number of NS elements.
   * We can probabably reduce these to a single stack and save some storage.
   * */
  IntStack mark_nsdeclset_size = new IntStack();
  /**
   * Tail-pruning mark: Number of naespace declaration elements in use %REVIEW%
   * I don't think number of NS sets is ever different from number of NS
  ...
  public SAX2RTFDTM(DTMManager mgr, Source source, int dtmIdentity, DTMWSFilter whiteSpaceFilter,
    ...
    // de.lyca.xalan.processor.TransformerFactoryImpl.m_source_location;
   ...
   * 
   * (I'm counting on the fact that getOwnerDocument() is implemented on top of
   * this call, in the superclass, to avoid having to rewrite that one. Be
   ...
   * this considers the owningDocument of a Document to be itself. Note that in
   * shared DTMs this may not be zero.
   * 
   ...
   * 
   * %REVIEW% I have no idea how to rewind m_elemIndexes. However, RTFs will not
   * be indexed, so I can simply panic if that case arises. Hey, it works...
   * */
   ...
   * time. We expect that this will allow us to reuse storage more effectively.
   * 
   ...
   * 
   * %REVIEW% WARNING: This is the first use of some of the truncation methods.
   * If Xalan blows up after this is called, that's a likely place to check.
   * 
   * %REVIEW% Our original design for DTMs permitted them to share string pools.
   * If there any risk that this might be happening, we can _not_ rewind and
   * recover the string storage. One solution might to assert that DTMs used for
   * RTFs Must Not take advantage of that feature, but this seems excessively
   * fragile. Another, much less attractive, would be to just let them leak...
   * Nah.
   * 
  ...
  public boolean popRewindMark() {
    final boolean top = mark_size.empty();
