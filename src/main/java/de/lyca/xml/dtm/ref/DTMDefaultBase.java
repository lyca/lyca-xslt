...
/**
 * The <code>DTMDefaultBase</code> class serves as a helper base for DTMs. It sets up structures for navigation and
 * type, while leaving data management and construction to the derived classes.
 */
public abstract class DTMDefaultBase implements DTM {
  ...
  /**
   * These hold indexes to elements based on namespace and local name. The base lookup is the the namespace. The second
   * lookup is the local name, and the last array contains the the first free element at the start, and the list of
   * element handles following.
   */
  ...
  /**
   * The document identity number(s). If we have overflowed the addressing range of the first that was assigned to us,
   * we may add others.
   */
  protected SuballocatedIntVector m_dtmIdent;

  /**
   * The mask for the identity. %REVIEW% Should this really be set to the _DEFAULT? What if a particular DTM wanted to
   * use another value?
   */
  ...
  /**
   * The table for exandedNameID lookups. This may or may not be the same table as is contained in the
   * DTMManagerDefault.
   */
   ...
   * 
   * @return The index in the list of the slot that is higher or identical to the identity argument, or -1 if no node is
   *         higher or equal.
   */
   ...
   * 
   * @return The first node that is greater than or equal to the firstPotential argument, or DTM.NOTPROCESSED if not
   *         found.
   */
  ...
  public void dumpDTM(OutputStream os) {
    try {
      if (os == null) {
        final File f = new File("DTMDump" + ((Object) this).hashCode() + ".txt");
   ...
   * 
   * %REVIEW% KNOWN GLITCH: If you pass it a node index rather than a node handle, it works just fine... but the
   * displayed identity number before the colon is different, which complicates comparing it with nodes printed the
   * other way. We could always OR the DTM ID into the value, to suppress that distinction...
   * 
   * %REVIEW% This might want to be moved up to DTMDefaultBase, or possibly DTM itself, since it's a useful diagnostic
   * and uses only DTM's public APIs.
   * 
   ...
   * <p>
   * %REVIEW% This is obviously useful at the DOM layer, where it would permit testing this without having to create a
   * proxy node. It's less useful in the DTM API, where (dtm.getFirstChild(nodeHandle)!=DTM.NULL) is just as fast and
   * almost as self-evident. But it's a convenience, and eases porting of DOM code to DTM.
   * </p>
   * 
  ...
  /**
   * Given a node identity, return a node handle. If extended addressing has been used (multiple DTM IDs), we need to
   * map the high bits of the identity into the proper DTM ID.
   * 
   * This has been made FINAL to facilitate inlining, since we do not expect any subclass of DTMDefaultBase to ever
   * change the algorithm. (I don't really like doing so, and would love to have an excuse not to...)
   * 
   * %REVIEW% Is it worth trying to specialcase small documents? %REVIEW% Should this be exposed at the package/public
   * layers?
   * 
  ...
  /**
   * Given a node handle, return a node identity. If extended addressing has been used (multiple DTM IDs), we need to
   * map the high bits of the identity into the proper DTM ID and thence find the proper offset to add to the low bits
   * of the identity
   * 
   * This has been made FINAL to facilitate inlining, since we do not expect any subclass of DTMDefaultBase to ever
   * change the algorithm. (I don't really like doing so, and would love to have an excuse not to...)
  ...
  final public int makeNodeIdentity(int nodeHandle) {
    ...
    return whichDTMid == NULL ? NULL
        : (whichDTMid << DTMManager.IDENT_DTM_NODE_BITS) + (nodeHandle & DTMManager.IDENT_NODE_DEFAULT);
  }

  /**
   * Given a node handle, get the handle of the node's first child. If not yet resolved, waits for more nodes to be
   ...
   * @param nodeType TODO
   * @return DTM node-number of first child, or DTM.NULL to indicate none exists.
   */
  public int getTypedFirstChild(int nodeHandle, int nodeType) {

    int firstChild, eType;
    if (nodeType < DTM.NTYPES) {
      for (firstChild = _firstch(makeNodeIdentity(nodeHandle)); firstChild != DTM.NULL; firstChild = _nextsib(
          firstChild)) {
  ...
  /**
   * Given a node handle, advance to its last child. If not yet resolved, waits for more nodes to be added to the
   * document and tries again.
   * 
   * @param nodeHandle int Handle of the node.
  ...
  /**
   * Given a node handle, advance to its next sibling. If not yet resolved, waits for more nodes to be added to the
   * document and tries again.
   * 
   * @param nodeHandle int Handle of the node.
  ...
  /**
   * Given a node handle, find its preceeding sibling. WARNING: DTM is asymmetric; this operation is resolved by search,
  ...
  /**
   * Given a node handle, advance to the next attribute. If an attr, we advance to the next attr on the same node. If
   * not an attribute, we return NULL.
   * 
   * @param nodeHandle int Handle of the node.
  ...
  /**
   * Build table of namespace declaration locations during DTM construction. Table is a List of SuballocatedIntVectors
   * containing the namespace node HANDLES declared at that ID, plus an SuballocatedIntVector of the element node
   * INDEXES at which these declarations appeared.
   * 
   * NOTE: Since this occurs during model build, nodes will be encountered in doucment order and thus the table will be
   * ordered by element, permitting binary-search as a possible retrieval optimization.
   * 
  ...
  /**
   * Retrieve list of namespace declaration locations active at this node. List is an SuballocatedIntVector whose
  ...
  /**
   * Subroutine: Locate the specified node within m_namespaceDeclSetElements, or the last element which preceeds it in
   * document order
   * 
   ...
   * @param vector TODO
   * @param lookfor TODO
   * @return If positive or zero, the index of the found item. If negative, index of the point at which it would have
   *         appeared, encoded as -1-index and hence reconvertable by subtracting it from -1. (Encoding because I don't
   *         want to recompare the strings but don't want to burn bytes on a datatype to hold a flagged value.)
   */
  ...
  /**
   * Given a node handle, get the index of the node's first child. If not yet resolved, waits for more nodes to be added
   * to the document and tries again
   * 
   * @param nodeHandle handle to node, which should probably be an element node, but need not be.
   * 
   * @param inScope true if all namespaces in scope should be returned, false if only the namespace declarations should
   *        be returned.
   ...
   * 
   * @param baseHandle handle to original node from where the first namespace was relative to (needed to return nodes in
   *        document order).
   * @param nodeHandle A namespace handle for which we will find the next node.
   * @param inScope true if all namespaces that are in scope should be processed, otherwise just process the nodes in
   *        the given element handle.
  ...
  /**
   * Find the Document node handle for the document currently under construction. PLEASE NOTE that most people should
   * use getOwnerDocument(nodeHandle) instead; this version of the operation is primarily intended for use during
   * negotiation with the DTM Manager.
   * 
  ...
  /**
   * Given a node handle, find the owning document node. This has the exact same semantics as the DOM Document method of
   * the same name, in that if the nodeHandle is a document node, it will return NULL.
   * 
   * <p>
   * %REVIEW% Since this is DOM-specific, it may belong at the DOM binding layer. Included here as a convenience
   * function and to aid porting of DOM code to DTM.
   * </p>
   * 
   * @param nodeHandle the id of the node.
  ...
  /**
   * Given a node handle, find the owning document node. Unlike the DOM, this considers the owningDocument of a Document
   * to be itself.
   * 
   * @param nodeHandle the id of the node.
  ...
  /**
   * Get number of character array chunks in the string-value of a node. (see http://www.w3.org/TR/xpath#data-model for
   * the definition of a node's string-value). Note that a single text node may have multiple text chunks.
   * 
   * @param nodeHandle The node ID.
   * 
  ...
  public int getStringValueChunkCount(int nodeHandle) {
    ...
    // %TBD%
    // ("getStringValueChunkCount not yet supported!");
  ...
  /**
   * Get a character array chunk in the string-value of a node. (see http://www.w3.org/TR/xpath#data-model for the
   * definition of a node's string-value). Note that a single text node may have multiple text chunks.
   * 
   * @param nodeHandle The node ID.
  ...
  public char[] getStringValueChunk(int nodeHandle, int chunkIndex, int[] startAndLen) {
    ...
    // %TBD%
    // "getStringValueChunk not yet supported!");
    error(Messages.get().methodNotSupported());
  ...
  public int getExpandedTypeID(int nodeHandle) {
    // %REVIEW% This _should_ only be null if someone asked the wrong DTM about
    // the node...
  ...
  /**
   * Given an expanded name, return an ID. If the expanded-name does not exist in the internal tables, the entry will be
   * created, and the ID will be returned. Any additional nodes that are created that have this expanded name will use
   * this ID.
   * 
   * @param type The simple type, i.e. one of ELEMENT, ATTRIBUTE, etc.
   * 
   * @param namespace The namespace URI, which may be null, may be an empty string (which will be the same as null), or
   *        may be a namespace URI.
  ...
  public int getNamespaceType(final int nodeHandle) {
    ...
    final int identity = makeNodeIdentity(nodeHandle);
    final int expandedNameID = _exptype(identity);
   ...
   * <p>
   * %REVIEW% Generally, returning short is false economy. Return int? %REVIEW% Make assumption that node has already
   * arrived. Is OK?
   * 
   * @param nodeHandle The node id.
  ...
  public int getNodeIdent(int nodeHandle) {
    /*
     * if (nodeHandle != DTM.NULL) return nodeHandle & m_mask; else return DTM.NULL;
     */
  ...
  public int getNodeHandle(int nodeId) {
    /*
     * if (nodeId != DTM.NULL) return nodeId | m_dtmIdent; else return DTM.NULL;
     */
   ...
   * @param version This is the version number of the feature to test. If the version is not specified, supporting any
   *        version of the feature will cause the method to return <code>true</code>.
  ...
  /**
   * Return the base URI of the document entity. If it is not known (because the document was parsed from a socket
   * connection or from standard input, for example), the value of this property is unknown.
   * 
  ...
  /**
   * Return an indication of the standalone status of the document, either "yes" or "no". This property is derived from
   * the optional standalone document declaration in the XML declaration at the beginning of the document entity, and
   * has no value if there is no standalone document declaration.
   * 
   * @param nodeHandle The node id, which can be any valid node handle.
  ...
  /**
   * Return a string representing the XML version of the document. This property is derived from the XML declaration
   * optionally present at the beginning of the document entity, and has no value if there is no XML declaration.
   * 
  ...
  /**
   * Return an indication of whether the processor has read the complete DTD. Its value is a boolean. If it is false,
   * then certain properties (indicated in their descriptions below) may be unknown. If it is true, those properties are
   * never unknown.
   * 
  ...
  /**
   * Figure out whether nodeHandle2 should be considered as being later in the document than nodeHandle1, in Document
   * Order as defined by the XPath model. This may not agree with the ordering defined by other XML applications.
   * <p>
  ...
  /**
   * 2. [element content whitespace] A boolean indicating whether the character is white space appearing within element
   * content (see [XML], 2.10 "White Space Handling"). Note that validating XML processors are required by XML 1.0 to
   * provide this information. If there is no declaration for the containing element, this property has no value for
   * white space characters. If no declaration has been read, but the [all declarations processed] property of the
   * document information item is false (so there may be an unread declaration), then the value of this property is
   * unknown for white space characters. It is always false for characters that are not white space.
   * 
   * @param nodeHandle the node ID.
   ...
   * it is an indication of whether the processor has read the complete DTD. Its value is a boolean. If it is false,
   * then certain properties (indicated in their descriptions below) may be unknown. If it is true, those properties are
   * never unknown.
   * 
  ...
  /**
   * Append a child to the end of the document. Please note that the node is always cloned if it is owned by another
   * document.
   * 
   * <p>
   * %REVIEW% "End of the document" needs to be defined more clearly. Does it become the last child of the Document? Of
   * the root element?
   * </p>
   * 
  ...
  public void appendTextChild(String str) {
    // "appendTextChild not yet supported!");
    error(Messages.get().methodNotSupported());
  }
  ...
  /**
   * A dummy routine to satisify the abstract interface. If the DTM implememtation that extends the default base
   * requires notification of registration, they can override this method.
   */
  @Override
  ...
  /**
   * A dummy routine to satisify the abstract interface. If the DTM implememtation that extends the default base
   * requires notification when the document is being released, they can override this method
   */
  @Override
