...
public abstract class DTMDefaultBase implements DTM {
  ...
  /**
   * These hold indexes to elements based on namespace and local name. The base
   * lookup is the the namespace. The second lookup is the local name, and the
   * last array contains the the first free element at the start, and the list
   * of element handles following.
   */
  ...
  /**
   * The document identity number(s). If we have overflowed the addressing range
   * of the first that was assigned to us, we may add others.
   */
  ...
  /**
   * The table for exandedNameID lookups. This may or may not be the same table
   * as is contained in the DTMManagerDefault.
   */
  ...
  /**
   * Find the first index that occurs in the list that is greater than or equal
   * to the given value.
   * 
   * @param list
   *          A list of integers.
   ...
   * @param value
   *          Find the slot that has a value that is greater than or identical
   *          to this argument.
   * 
   ...
   * 
   * @return The first node that is greater than or equal to the firstPotential
   *         argument, or DTM.NOTPROCESSED if not found.
   */
   ...
   * handle, it works just fine... but the displayed identity number before the
   * colon is different, which complicates comparing it with nodes printed the
   * other way. We could always OR the DTM ID into the value, to suppress that
   * distinction...
   * 
   ...
   * been used (multiple DTM IDs), we need to map the high bits of the identity
   * into the proper DTM ID.
   * 
   * This has been made FINAL to facilitate inlining, since we do not expect any
   * subclass of DTMDefaultBase to ever change the algorithm. (I don't really
   * like doing so, and would love to have an excuse not to...)
   * 
   * %REVIEW% Is it worth trying to specialcase small documents? %REVIEW% Should
   * this be exposed at the package/public layers?
   * 
   * @param nodeIdentity
   ...
   * been used (multiple DTM IDs), we need to map the high bits of the identity
   * into the proper DTM ID and thence find the proper offset to add to the low
   * bits of the identity
   * 
   * This has been made FINAL to facilitate inlining, since we do not expect any
   * subclass of DTMDefaultBase to ever change the algorithm. (I don't really
   * like doing so, and would love to have an excuse not to...)
  ...
  final public int makeNodeIdentity(int nodeHandle) {
    ...
    return whichDTMid == NULL ? NULL : (whichDTMid << DTMManager.IDENT_DTM_NODE_BITS)
  ...
  /**
   * Given a node handle, advance to the next attribute. If an attr, we advance
   * to the next attr on the same node. If not an attribute, we return NULL.
   * 
   * @param nodeHandle
   *          int Handle of the node.
   ...
   * 
   * @param identity
   *          int identity of the attribute node. This <strong>must</strong> be
   *          an attribute node.
   * 
   * @return int DTM node-identity of the resolved attr, or DTM.NULL to indicate
   *         none exists.
   * 
   */
  ...
  /**
   * Retrieve list of namespace declaration locations active at this node. List
   * is an SuballocatedIntVector whose entries are the namespace node HANDLES
   * declared at that ID.
   * 
   * %REVIEW% Directly managed arrays rather than vectors? %REVIEW% Handles or
   * IDs? Given usage, I think handles.
  ...
  /**
   * Subroutine: Locate the specified node within m_namespaceDeclSetElements, or
   * the last element which preceeds it in document order
   * 
   ...
   * @param localName
   *          The local name string, which must be a valid <a
   ...
   * <p>
   * %REVIEW% Generally, returning short is false economy. Return int? %REVIEW%
   * Make assumption that node has already arrived. Is OK?
   * 
   * @param nodeHandle
   *          The node id.
