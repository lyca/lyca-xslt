...
public abstract class DTMAxisIteratorBase implements DTMAxisIterator {
   ...
   * iterating over a stream that can not be re-scanned, or if the iterator was
   * produced by cloning another iterator.)
   */
  ...
  /**
   * @return A DTMAxisIterator which has been reset to the start node, which may
   *         or may not be the same as this iterator.
   * */
  @Override
   ...
   * which this iterator will yield. In a reverse iterator, I believe it should
   * return 1 (since the "last" is the first produced.)
   * 
   * This may be an expensive operation when called the first time, since it may
   * have to iterate through a large part of the document to produce its answer.
   * 
