...
public class DTMNodeIterator implements org.w3c.dom.traversal.NodeIterator {
   ...
   * stateful XPath-based filtering directly into the traversal object. We could
   * return something which supports the NodeFilter interface and allows
   * querying whether a given node would be permitted if it appeared as our next
   * node, but in the current implementation that would be very complex -- and
   * just isn't all that useful.
   * 
   * @throws DOMException
