...
public interface DTM {
  ...
  /**
   * The node is a <code>namespace node</code>. Note that this is not currently
   * a node type defined by the DOM API.
   */
   ...
   */
  DTMAxisTraverser getAxisTraverser(final Axis axis);
   ...
   *          the node's own namespace declarations should be returned.
   ...
   * advance to its first attribute; if an attr, we advance to the next attr of
   * the same element.
   * 
   * @param nodeHandle
   *          int Handle of the node.
  ...
  /**
   * Given a DTM which contains only a single document, find the Node Handle of
   * the Document node. Note that if the DTM is configured so it can contain
   * multiple documents, this call will return the Document currently under
   * construction -- but may return null if it's between documents. Generally,
   * you should use getOwnerDocument(nodeHandle) or getDocumentRoot(nodeHandle)
   * instead.
   * 
  ...
  /**
   * Given a node handle, find the owning document node. This version mimics the
   ...
   * @param nodeHandle
   *          the id of the node.
   * @return int Node handle of owning document, or the node itself if it was a
   *         Document. (Note difference from DOM, where getOwnerDocument returns
   ...
   *          Which chunk to get.
   * @param startAndLen
   *          A two-integer array which, upon return, WILL BE FILLED with values
   *          representing the chunk's start position within the returned
   *          character buffer and the length of the chunk.
   ...
   * that our caller can map it to one. %REVIEW% Resolve Public Identifiers...
   * or consider changing function name.
   * <p>
   ...
   * didn't store the document's base URI.) %REVIEW% Consider resolving Relative
   * URIs.
   * <p>
   ...
   * must assume that the whitespace could be meaningful and return false. If no
   * declaration has been read, but the [all declarations processed] property of
   * the document information item is false (so there may be an unread
   * declaration), then the value of this property is indeterminate for white
   * space characters and should probably be reported as false. It is always
   * false for text nodes that contain anything other than (or in addition to)
   * white space.
   * <p>
   ...
   *         <code>false</code> if it was defaulted or the handle doesn't refer
   *         to an attribute node.
   */
   ...
   * always cloned in a base DTM, since our basic behavior is immutable so nodes
   * can't be removed from their previous location.
   * 
   * <p>
   ...
   * insert point is the last element that was appended (or the last one popped
   * back to by an end-element operation).
   * </p>
   * 
   * @param newChild
   ...
   * of the document. Behavior is otherwise like appendChild().
   * 
   * @param str
   ...
   * This will give the DTM implementation a chance to initialize any subsystems
   * that are required to build the DTM
   */
  void documentRegistration();

  /**
   * As documents are released from the DTMManager, the DTM implementation will
   * be notified of the event. This will allow the DTM implementation to
   * shutdown any subsystem activity that may of been assoiated with the active
   * DTM Implementation.
   */
