...
public class FastStringBuffer {
   ...
   * normalize-to-SAX is called for the first chunk of a multi-chunk output, or
   * one following unsuppressed whitespace in a previous chunk.
   * 
   ...
   * should be used when normalize-to-SAX is called for a complete string. (I'm
   * not wild about the name of this one. Ideas welcome.)
   * 
   * @see #sendNormalizedSAXcharacters(org.xml.sax.ContentHandler,int,int)
   */
   ...
   * chunk. For example, if m_chunkbits is set to 15, each chunk can contain up
   * to 2^15 (32K) characters
   */
  int m_chunkBits = 15;

  /**
   * Field m_maxChunkBits affects our chunk-growth strategy, by saying what the
   * largest permissible chunk size is in this particular FastStringBuffer
   * hierarchy.
   */
   ...
   * chunks should be allocated at one size before we encapsulate them into the
   * first chunk of the next size up. For example, if m_rechunkBits is set to 3,
   * then after 8 chunks at a given size we will rebundle them as the first
   * element of a FastStringBuffer using a chunk size 8 times larger (chunkBits
   * shifted left three bits).
   */
  int m_rebundleBits = 2;

  /**
   * Field m_chunkSize establishes the maximum size of one chunk of the array as
   * 2**chunkbits characters. (Which may also be the minimum size if we aren't
   * tuning for storage)
   */
  int m_chunkSize; // =1<<(m_chunkBits-1);

  /**
   * Field m_chunkMask is m_chunkSize-1 -- in other words, m_chunkBits worth of
   * low-order '1' bits, useful for shift-and-mask addressing within the chunks.
   */
   ...
   * reallocated when necessary in order to allow the buffer to grow; references
   * to them should be considered to be invalidated after any append. However,
   * the only time these arrays are directly exposed is in the sendSAXcharacters
   * call.
   */
  char[][] m_array;

  /**
   * Field m_lastChunk is an index into m_array[], pointing to the last chunk of
   * the Chunked Array currently in use. Note that additional chunks may
  ...
  /**
   * Field m_firstFree is an index into m_array[m_lastChunk][], pointing to the
   * first character in the Chunked Array which is not part of the
   ...
   * hierarchy of FastStringBuffers, where early appends use a smaller chunkSize
   * (for less wasted memory overhead) but later ones use a larger chunkSize
   * (for less heap activity overhead).
   */
   ...
   * <p>
   * For coding convenience, I've expressed both allocation sizes in terms of a
   * number of bits. That's needed for the final size of a chunk, to permit fast
   * and efficient shift-and-mask addressing. It's less critical for the inital
   ...
   * @param maxChunkBits
   *          Number of character-offset bits that should be used for addressing
   *          within a chunk. Maximum length of a chunk is 2^chunkBits
   *          characters.
   * @param rebundleBits
   ...
   * values.
   * <p>
   * ISSUE: Should this call assert initial size, or fixed size? Now configured
   * as initial, with a default for fixed.
   * 
  ...
  /**
   * Directly set how much of the FastStringBuffer's storage is to be considered
   * part of its content. This is a fast but hazardous operation. It is not
   * protected against negative values, or values greater than the amount of
   * storage currently available... and even if additional storage does exist,
   * its contents are unpredictable. The only safe use for our setLength() is to
   * truncate the FastStringBuffer to a shorter string.
   * 
   * @param l
  ...
  /**
   * Append a single character onto the FastStringBuffer, growing the storage if
   * necessary.
   * <p>
  ...
  /**
   * Append part of the contents of a Character Array onto the FastStringBuffer,
  ...
  /**
   * Append the contents of another FastStringBuffer onto this FastStringBuffer,
   * growing the storage if necessary.
   * <p>
   * NOTE THAT after calling append(), previously obtained references to
   * m_array[] may no longer be valid.
  ...
  public final void append(FastStringBuffer value) {
    ...
    while (strlen > 0) {
      ...
      if (runlength != available) {
        System.arraycopy(value.m_array[sourcechunk + 1], 0, m_array[m_lastChunk], m_firstFree + runlength, available
                - runlength);
      }
   ...
   * events. Note that the buffer reference passed to the ContentHandler may be
   ...
   * <p>
   * Note too that there is no promise that the output will be sent as a single
   * call. As is always true in SAX, one logical string may be split across
   ...
   *          output) whenever the previous chunk ended in retained whitespace.</dd>
   *          <dt>SUPPRESS_TRAILING_WS</dt>
   *          <dd>When false, causes trailing whitespace to be converted to a
   *          single space; when true, causes it to be discarded entirely.
  ...
  /**
   * Encapsulation c'tor. After this is called, the source FastStringBuffer will
   * be reset to use the new object as its m_innerFSB, and will have had its
   * chunk size reset appropriately. IT SHOULD NEVER BE CALLED EXCEPT WHEN
