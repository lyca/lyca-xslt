...
public class DefaultErrorHandler implements ErrorHandler, ErrorListener {
   ...
   * directed.
   * 
   * @return The <code>PrintWriter</code> installed via the constructor or the
  ...
  public static void printLocation(PrintWriter pw, Throwable exception) {
    ...
    if (null != locator) {
      ...
      final String id = null != locator.getPublicId() ? locator.getPublicId() : null != locator.getSystemId() ? locator
              .getSystemId() : XMLMessages.createXMLMessage(XMLErrorResources.ER_SYSTEMID_UNKNOWN, null); // "SystemId Unknown";
