...
public class StringComparable implements Comparable {
  ...
  public final static Comparable getComparator(final String text, final Locale locale, final Collator collator,
    ...
    while (true) {
      ...
      if (patternElement == done || targetElement == done)
        ...
        if (startPatt < endPatt && startTarg < endTarg) {
          ...
          if (m_collator.compare(subTextUp, subPattUp) != 0) { // not case
                                                               // diffference
            continue;
          }

          final int diff[] = { UNKNOWN_CASE, UNKNOWN_CASE };
