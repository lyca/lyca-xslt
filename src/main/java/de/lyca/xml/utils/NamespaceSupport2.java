 ...
 * 
 * <p>
 * This class is an attempt to rewrite the SAX NamespaceSupport "helper" class
 * for improved efficiency. It can be used to track the namespace declarations
 * currently in scope, providing lookup routines to map prefixes to URIs and
 * vice versa.
 * </p>
 * 
 * <p>
 * ISSUE: For testing purposes, I've extended NamespaceSupport even though I'm
 * completely reasserting all behaviors and fields. Wasteful.... But SAX did not
 * put an interface under that object and we seem to have written that SAX class
 * into our APIs... and I don't want to argue with it right now.
 * </p>
 * 
...
public class NamespaceSupport2 extends NamespaceSupport {
   ...
   * element: the new context will automatically inherit the declarations of its
   * parent context, but it will also keep track of which declarations were made
   * within this context.
   * </p>
   * 
   * <p>
  ...
  public void pushContext() {
    ...
    final Context2 parentContext = currentContext;
  ...
  public void popContext() {
    final Context2 parentContext = currentContext.getParent();
   ...
   * will remain in force until this context is popped, unless it is shadowed in
   * a descendant context.
   * </p>
   * 
   * <p>
   * To declare a default Namespace, use the empty string. The prefix must not
   * be "xml" or "xmlns".
   * </p>
   * 
   * <p>
   ...
   * 
   * <p>
   * Note that there is an asymmetry in this library: while {@link #getPrefix
   * getPrefix} will not return the default "" prefix, even if you have declared
   * one; to check for a default prefix, you have to look it up explicitly using
   * {@link #getURI getURI}. This asymmetry exists to make it easier to look up
   * prefixes for attribute names, where the default prefix is not allowed.
   * </p>
   * 
   * @param prefix
   ...
   * the prefix and looking it up among the prefixes currently declared. The
   * return value will be the array supplied by the caller, filled in as
   * follows:
   * </p>
   * 
   * <dl>
   ...
   * 
   * @param qName
   *          The raw XML 1.0 name to be processed.
   * @param parts
   *          A string array supplied by the caller, capable of holding at least
   *          three members.
   * @param isAttribute
   *          A flag indicating whether this is an attribute name (true) or an
   *          element name (false).
   * @return The supplied array holding three internalized strings representing
   *         the Namespace URI (or empty string), the local name, and the raw
   *         XML 1.0 name; or null if there is an undeclared prefix.
   * @see #declarePrefix
   * @see java.lang.String#intern
   */
  @Override
  public String[] processName(String qName, String[] parts, boolean isAttribute) {
    final String[] name = currentContext.processName(qName, isAttribute);
    if (name == null)
      return null;
   ...
   *         except for the empty (default) prefix.
   ...
   * {@link #getPrefixes} method instead.
   * </p>
   * 
   * <p>
   ...
   * this behaviour differs from that of {@link #getPrefix} and
   * {@link #getPrefixes}.
   * </p>
   * 
   * @return An enumeration of all prefixes declared in this context.
 ...
 * version of the operation. This is NOT necessarily the most efficient
 ...
 * will be equal to the element depth of the document, not to the total number
 * of elements (i.e. 5-10 rather than tens of thousands).
 * </p>
 */
final class Context2 {
  ...
  /**
   * @returns The child Namespace context object, or null if this is the last
   *          currently on the chain.
   */
  Context2 getChild() {
    return child;
  }
  ...
  /**
   * (Re)set the parent of this Namespace context. This is separate from the
   * c'tor because it's re-applied when a Context2 is reused by push-after-pop.
   * 
   * @param context
   ...
   * 
   * @param qName
   *          The raw XML 1.0 name.
   * @param isAttribute
   *          true if this is an attribute name.
   * @return An array of three strings containing the URI part (or empty
   *         string), the local part, and the raw name, all internalized, or
   *         null if there is an undeclared prefix.
   ...
   * 
   * <p>
   * Since many prefixes may be mapped to the same URI, the return value may be
   * unreliable.
   * </p>
   * 
   * @param uri
   ...
   * 
   * <p>
   * The default prefix, if in force, is <em>not</em> returned, and will have to
   ...
   * 
   * <p>
   * This class is optimized for the normal case where most elements do not
   * contain Namespace declarations. In that case, the Context2 will share data
   * structures with its parent. New tables are obtained only when new
   * declarations are issued, so they can be popped off the stack.
   * </p>
   * 
   * <p>
   * JJK: **** Alternative: each Context2 might declare _only_ its local
   * bindings, and delegate upward if not found.
   * </p>
   */
